...
class Sledilnik:
    ...
    def kje_zivi(self, oseba):
        for kraj, ime in self.seznam:
            if oseba == ime:
                return kraj

    def prebivalci(self, kraj):
        mnozica = set()
        for plac, ime in self.seznam:
            if plac == kraj:
                mnozica.add(ime)
        return mnozica

    def preseli(self, oseba, kraj):
        for i, ime in enumerate(self.seznam):
            if ime in self.seznam[i] and ime == oseba:
                self.seznam[i] = (kraj, oseba)
