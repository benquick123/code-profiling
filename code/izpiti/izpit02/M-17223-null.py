...
def bingo(listki,vrstni_red):
    ...
    for stevilka in vrstni_red:
        for vrstica in novo:
            if stevilka in vrstica:
                vrstica.remove(stevilka)
            if vrstica == []:
                return
"""
class Sledilnik:
    def __init__(self):
        self.razdelitev =  # vneses iz 3 naloge kraje
    def kje_zivi(self, oseba):
        for kraj, ime in self.razdelitev:
            if ime == oseba:
                return kraj
    def prebivalci(self, kraj): #vrne mnozico prebivalcev podanega kraja
    def preseli(self, oseba, kraj): #preseli podano osebo v podani kraj
    def preseli_vse(self, odkod, kam): #prseli vse prebivalce kraja odkod v kraj kam
    def selitev(self):#vrne stevilo vseh selitev. ce se seba seli v kraj, v katerem ze zivi, se to steje za selitev.
                      # ce iz nekega kraja preselijo tri osebe v drug kraj, so to tri selitve
