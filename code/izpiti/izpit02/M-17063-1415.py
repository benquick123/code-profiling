...
def nogavice(s):
    ...
    mnozica2 = set()
    for nogavica in s:
        if not nogavica in mnozica:
            mnozica |= {nogavica}
        else:
            mnozica = mnozica - {nogavica}
    for a in s:
        if a in mnozica and a not in mnozica2:
            seznam += [a]
            mnozica2 |= {a}
    return seznam

def brez_para(nogavica,nogavice):
    n = nogavice.count(nogavica)
    if n > 2:
        nogavice.remove(nogavica)
        nogavice.remove(nogavica)
...
def bingo(listki,vrstni_red):
    ...
    for stevilka in vrstni_red:
        for n,seznam in enumerate(listki):
            if stevilka in seznam:
                if n in slovar:
                    slovar[n] += 1
                else:
                    slovar[n] = 1
                if len(seznam) == slovar[n]:
                    return listki[n]

class Sledilnik:
    def __init__(self, zacetek):
        self.kje = {}
        for kraj, ime in zacetek:
            self.kje[ime] = kraj
        self.selitve = 0

    def kje_zivi(self, oseba):
        return self.kje[oseba]

    def prebivalci(self, kraj):
        mapa = self.kje
        mnozica = set()
        for ime, kra in mapa.items():
            if kraj == kra:
                mnozica |= {ime}
        return mnozica
...
def selitve(zacetek, datoteka_selitev, kraj):
    ...
    for vrstica in open(datoteka_selitev):
        ...
        seznam1 = []
        seznam2 = []
        for n, cr in enumerate(seznam):
            cas = True
            if cr == '"' and nn == 0 and cas:
                seznam1 = seznam[(n+1):]
                nn += 1
                cas = False
                stevec = 0
            if cr == '"' and nn == 1 and cas:
                seznam1 = seznam1[:(stevec - 1)]
                nn += 1
                cas = False
            if cr == '"' and nn == 2 and cas:
                seznam2 = seznam[(n+1):]
                nn += 1
                cas = False
                stevec = 0
            if cr == '"' and nn == 3 and cas:
                seznam2 = seznam2[:(stevec -1)]
                nn += 1
                cas = False
                break
        ...
        for crka in seznam1:
            kraj1 += crka
        for crka2 in seznam2:
            kraj2 += crka2

        sledilo.preseli_vse(kraj1, kraj2)
    return sledilo.prebivalci(kraj)

    return seznam1, seznam
