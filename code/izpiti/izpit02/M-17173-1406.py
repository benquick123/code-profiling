...
def selitve(zacetek, datotoeka_selitev, kraj):
    datoteka = open(datotoeka_selitev)
    for vrstica in datoteka.read().splitlines():
        return 0

def bingo(listki, vrstni_red):
    from collections import defaultdict
    seznam = defaultdict(list)
    k = 0
    for sezname in listki:
        seznam[k] = []
        for i in sezname:
            if i in vrstni_red:
                seznam[k].append(i)
...
def brez_para(nogavica, nogavice):
    if sum(nogavice) == len(nogavice):
        if len(nogavice) % 2 == 0:
            return True
    elif nogavice[0] == nogavica:
        return [True] + nogavice[1:]
    else:
        return nogavice[1:]
...
class Sledilnik():
    def __init__(self, razdelitev):
        razdel = {}
        for kraj, oseba in razdelitev:
            razdel[oseba] = kraj
        self.razdelitev = razdel
        self.selitve = 0

    def kje_zivi(self, oseba):
        return self.razdelitev[oseba]

    def prebivalci(self, kraj):
        seznam = set()
        for i in self.razdelitev:
            if self.razdelitev[i] == kraj:
                seznam.add(i)
        return seznam

    def preseli(self, oseba, kraj):
        self.razdelitev[oseba] = kraj
        self.selitve += 1

    def preseli_vse(self, odkod, kam):
        for oseba in self.razdelitev:
            if self.razdelitev[oseba] == odkod:
                self.razdelitev[oseba] == kam
                self.selitve += 1
