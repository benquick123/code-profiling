...
class Sledilnik:
    def __init__(self):
        self.oseba=""
        self.krajevi=collections.defaultdict()
        self.kraj=""
        self.selitev=0
    def kje_zivi(self,oseba,kraj):
        return kraj[oseba]
    def prebivalci(self,kraj):
        ...
        for i in kraj:
            mnozica.add(i)
        return mnozica
    def preseli(self,oseba,kraj,selitev,krajevi):
        krajevi[kraj]=oseba
        selitev=selitev+1
    def preseli_vse(self,odkod,kam,selitev,krajevi):
        kdo=krajevi[odkod].items()
        stevilo=len(kdo)
        krajevi[kam]=kdo
        selitev=selitev+stevilo
    def selitev(self,selitev):
        return selitev
def brez_para(nogavica,nogavice):
    for i in nogavice:
        stevilo=nogavice.count(nogavica)
        if stevilo%2==0:
            return False
        else:
            brez_para(i,nogavice)
    return True
