        ...
        for b in vrstni_red:
            ...
            if b in a:
                ...
                if not a:
                    ...
                    print(i)
    print(stev)
    return listki[stev.index(min(stev))]

"""def selitve(zacetek, datoteka_selitev, kraj):
    datoteka = open(datoteka_selitev)
    for line in datoteka:
        print(line.split())

"""
#to je tok lahka naloga sam nimam pojma kako naj bi vrstico splitou glede na narekovaje
#


def brez_para(nogavica, nogavice):
    return nogavice.count(nogavica) % 2 == 0
...
class Sledilnik:
    def __init__(self, zacetek):
        ...
        self.trenutno = zacetek
        self.selitve = 0

    def kje_zivi(self, oseba):
        for mesto in self.trenutno:
            if oseba in mesto:
                return mesto[0]
    ...
    def preseli_vse(self, odkod, kam):
        for mesto in self.trenutno:
            if odkod in mesto:
                sez = [oseba for oseba in mesto[1:]]
                for ime in mesto:
                    if ime in sez:
                        mesto.remove(ime)
        for mesto in self.trenutno:
            if kam in mesto:
                for a in sez:
                    kam.append(sez)
        self.selitve += len(sez)

    def selitev(self):
        return self.selitve

#nisem vedel kako naj drugače naredim ker mi je zmanjkalo casa zaradi 3. naloge
#problem je ker so vsa mesta in prebivalci nasteti v terkah
#najprej sem mislil da so v seznamih
#ce bi imel vec casa bi stvar naredil s slovarji, kjer so kljuci imena mest in vrednosti imena prebivalcev
#tako bi lahko prosto spreminjal kdo je kje
