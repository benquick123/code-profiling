...
def bingo(listki, vrstni_red):
    ...
    c_st = 0
    i = 0
    while i < len(vrstni_red):
        if vrstni_red[i] in listki[0]:
            a_st +=1
            if a_st == len(listki[0]):
                return listki[0]
        if vrstni_red[i] in listki[1]:
            b_st += 1
            if b_st == len(listki[1]):
                return listki[1]
        if vrstni_red[i] in listki[2]:
            c_st += 1
            if c_st == len(listki[2]):
                return listki[2]
        i += 1
