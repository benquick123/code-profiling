...
def bingo(listki, stevilke):
    stevke = [1, 2, 3, 4, 5, 6, 7, 8, 9]
    listkicopy = []
    for listek in listki:
        temp = []
        for stevka in listek :
            if stevka in stevke:
                temp.append(stevka)
        listkicopy.append(temp)
    for stevilka in stevilke:
        for n in range(0,3):
            if stevilka in listkicopy[n]:
                listkicopy[n].remove(stevilka)
            if listkicopy[n] == []:
                return listki[n]

def selitve(zacetek, datoteka_selitev, kraj):
    file = open(datoteka_selitev)
    osebe = set()
    try:
        while True:
            ...
            x = x[1:]
            for e in x:
                if e[0] in ["A", "B", "C", "Č", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O", "P", "R", "S", "Š", "T", "U", "V", "Z", "Ž", ]:
                    kraja.append(e)
            print(kraja)

            i = 0
            for (mesto, oseba) in zacetek:
                if mesto == kraja[0]:
                    zacetek[i] = (kraja[1], oseba)
    except:
        for (mesto, oseba) in zacetek:
            if mesto == kraj:
                osebe.add(oseba)
...
def brez_para(nogavica, nogavice):
    count = 0
    for stunf in nogavice:
        brez_para(nogavica, stunf)
    if stunf == nogavica:
        count += 1
    return count
...
class Sledilnik():
    def __init__(self, zacetek):
        self.podatki = zacetek
        self.selitve = 0

    def kje_zivi(self, oseba):
        for (kraj, clovek) in self.podatki:
            if clovek == oseba:
                return kraj
    def prebivalci(self, kraj):
        preb = []
        for (mesto, clovek) in self.podatki:
            if kraj == mesto:
                preb.append(clovek)
        return set(preb)
    ...
    def preseli(self, oseba, kraj):
        ...
        for (mesto, clovek) in self.podatki:
            if clovek == oseba:
                self.podatki[index] = (kraj, clovek)
                self.selitve += 1
            index += 1
