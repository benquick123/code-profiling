...
def nogavice(s):
    nepar = []
    stej = Counter(s)
    for st, pojavitve in stej.items():
        if pojavitve % 2 == 1:
            nepar.append(st)
    return nepar
...
def selitve(zacetek, datoteka_selitev, kraj):
    for vrsta in open(datoteka_selitev).split("."):
        besede = vrsta.split('"')
...
def brez_para(nogavica, nogavice):
    count = 0
    stej = Counter(nogavice)
    if stej[nogavica] % 2 == 1:
        return True
    else:
        return False
    """""
        for x in nogavice:
            if x == nogavica:
                count += 1
                if count % 2 == 1:
                    return x + brez_para(nogavica, nogavice[1::])
        """

class Sledilnik:
    def __init__(self, zacetno):
        mesta = {}
        osebe = {}
        for mesto, ime in zacetno:
            mesta[mesto] = ime
            osebe[ime] = mesto
        self.osebe = osebe
        self.mesta = mesta
    ...
    def prebivalci(self, kraj):
        return set(self.mesta[kraj])

    def preseli(self, oseba, kraj):
        kopija = self.mesta.copy()

        if oseba in self.mesta[kraj]:
            self.selitve += 1
        else:
            for mesto, ime in self.mesta.items():
                if ime == oseba:
                    krajos = mesto
                    nov = list(self.mesta[mesto]).append(ime)
                    self.mesta[mesto] = nov
                    if self.mesta[krajos]:
                        self.mesta[krajos].remove(ime)
                    self.selitve += 1
                    break
    ...
    def preseli_vse(self, odkod, kam):
        kopija = self.mesta
        vsi = self.mesto[odkod]
        self.mesto[kam].update(vsi)
        self.mesto[odkod].clear()
        self.selitve += len(vsi)
