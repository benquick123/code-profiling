...
def bingo(listki, vrstni_red):
    a = []
    for x in range(len(listki)):
        a.append(0)
    for x in vrstni_red:
        for i,y in enumerate(listki):
            if x in y:
                a[i] += 1
                if a[i] == 5:
                    return listki[i]
        ...
        for x in vrstica:
            if '"' in x:
                if st == 0:
                    ...
                    priseli = x[1:-1]
        print(izseli,priseli)
        for i, x in enumerate(zacetek):
            kr, os = x
            if kr == izseli:
                zacetek[i] = (priseli,os)
    osebe = set()
