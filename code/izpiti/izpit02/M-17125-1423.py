...
def nogavice(s):
    ns = []
    for i in s:
        if s.count(i)%2 != 0 and i not in ns:
            ns.append(i)
    return ns
...
def bingo(listki, vrstni_red):
    ...
    s = [st1, st2, st3]
    for i in vrstni_red:
        for ix, listek in enumerate(listki):
            for st in listek:
                if st == i:
                    s[ix] -= 1
                for x in listki:
...
def brez_para(nogavica, nogavice):
    if nogavice[:1] == nogavica:
        if nogavice.count(nogavice[:1]) % 2 != 0:
            return True
    else:
        brez_para(nogavice[1:], nogavice)
    return False

def brez_para(nogavica, nogavice):
    for i in nogavice:
        if i == nogavica and nogavice.count(i) %2 != 0:
            return True
        else:
            return False
...
class Sledilnik:
    ...
    def preseli(self, oseba, kraj):
        for i in self.zacetek:
            if i[1] == oseba:
                self.zacetek.remove(i)
                self.zacetek.append((kraj, oseba))
    def preseli_vse(self, odkod, kam):
        for i in self.zacetek:
            if i[0] == odkod:
                self.zacetek.remove(i)
                self.zacetek.append(kam, i[1])
