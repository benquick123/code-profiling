...
def nogavice(s):
    ...
    for nogavica,stevilo in t.items():
        if stevilo % 2 != 0:
            u.append(nogavica)
    return u
...
def bingo(listki,vrstni_red):
    ...
    if len(u)>len(v) and len(u)>len(z):
        return u
    if len(z) > len(u) and len(z) > len(v):
        return z
    if len(v) > len(u) and len(v) > len(z):
        return v
