...
def nogavice(s):
    ...
    for i in range(len(s)):
            if s.count(s[i]) % 2 != 0 and s[i] not in seznam:
                seznam.append(s[i])
    return seznam
...
def bingo(listki, vrstni_red):
    ...
    for i in range(len(vrstni_red)):
        if vrstni_red[i] in l1:
            player1 += 1
            if player1 == 5:
                return l1
        if vrstni_red[i] in l2:
            player2 += 1
            if player2 == 5:
                return l2
        if vrstni_red[i] in l3:
            player3 += 1
            if player3 == 5:
                return l3
...
def selitve(zacetek, datoteka_selitev, kraj):
    ...
    for x,y in zacetek:
        kraji.append('"'+x+'"')
        ljudje.append(y)
    zacetek = dict(zacetek)
    for vrstica in open(datoteka_selitev):
        vrstica = vrstica.split()
        for i in range(len(vrstica)):
            if vrstica[i] in kraji:
                selitve1.append(vrstica[i])
    for i in selitve1:
        i = i.strip('"')

    while cnt <= (len(selitve1)/2):
        while iq < len(selitve1):
            if selitve1[i] in zacetek:
                zacetek[selitve1[i]].add(ljudje[i])
            iq+=2
        cnt +=1
    return set(zacetek[kraj])
...
class Sledilnik:
    ...
    def preseli(self, oseba, kraj):
        ...
        for x,y in self.zacetek:
            if y == oseba1:
                del y
        while cnt <= len(self.zacetek):
            x,y = self.zacetek[cnt]
            if x == kraj:
                self.zacetek[cnt].append(oseba1)
