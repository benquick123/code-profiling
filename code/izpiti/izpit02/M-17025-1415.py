...
def nogavice(s):
    seznam = [x for x in s if (s.count(x)%2) is not 0]
    seznam2 = []
    for a in seznam:
        if a not in seznam2:
            seznam2.append(a)
...
def bingo(listki, vrstni_red):
    seznam = listki.copy()
    listek1 = seznam[0].copy()
    listek2 = seznam[1].copy()
    listek3 = seznam[2].copy()
    for a, item in enumerate(vrstni_red):
        if item in listek1:
            listek1.remove(item)
            if listek1 is []:
                return listki[0]
            elif item in listek2:
                    listek2.remove(item)
                    if listek2 is []:
                        return listki[1]
            elif item in listek3:
                    listek3.remove(item)
                    if listek3 is []:
                        return listki[2]

def selitve(zacetek, datoteka_selitev, kraj):
    selitev = open(datoteka_selitev)
    seznam = [a for a in selitev.split()]
...
class Sledilnik:
    ...
    def kje_zivi(self, oseba):
        return [a[0] for a in self.zacet if a[1] is oseba]

    def prebivalci(self, kraj):
        return set(a[1] for a in self.zacet if a[0] is kraj)
