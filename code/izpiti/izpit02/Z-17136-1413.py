...
def nogavice(s):
    sez = []
    for nogavi in s:
        if nogavi not in sez:
            sez.append(nogavi)

    slovar = dict()
    for x in range(1, len(sez) + 1):
        slovar[x] = list()
    for nogavica in s:
        if nogavica in slovar.keys():
            slovar[nogavica].append(nogavica)
    brez = []
    for st, noga in slovar.items():
        if len(noga) % 2 != 0:
            brez.append(noga[0])
    return brez
...
def brez_para(nogavica, nogavice):
    if nogavica not in nogavice:
        return False
    return True

class Sledilnik:
    def __init__(self, zacetek):
        self.seznam_zacetkov = zacetek
        self.st_selitev = 0

    def kje_zivi(self, oseba):
        for kraj, ime in self.seznam_zacetkov:
            return kraj

    def perbivalci(self, kraj):
        mnozica = set()
        for kr, ime in self.seznam_zacetkov:
            if kr == kraj:
                mnozica.add(ime)

    def preseli(self, oseba, kraj):
        for prostor, prebivalec in self.seznam_zacetkov:
            if prebivalec == oseba:
                self.seznam_zacetkov.replace(prostor, kraj)

    def preseli_vse(self, odkod, kam):
        for kraj, oseba in self.seznam_zacetkov:
            if kraj == odkod:
                self.seznam_zacetkov.replace(kraj, kam)
                self.st_selitev += 1
