def nogavice(s):
    seznam = []
    for el in s:
        if s.count(el) % 2 == 1 and el not in seznam:
            seznam.append(el)
    return seznam
...
def bingo(listki, vrstni_red):
    ...
    for el in vrstni_red:
        ...
        for d in dolzina:
            if d == 0:
                break
    indeks = dolzina.index(0)
    return listki[indeks]
...
class Sledilnik:
    def __init__(self, delitev):
        self.delitev = delitev
        self.s = {}
        for el in self.delitev:
            self.s[el[0]] = el[1]
    ...
    def kje_zivi(self,oseba):
        for kljuc, vrednost in self.s.items():
            if vrednost == oseba:
                return kljuc
    ...
    def preseli_vse(self, odkod, kam):
        self.s[odkod] = self.s[kam]
        self.stselitev += len(self.s[odkod])
