...
def nogavice(s):
   t = [e for e in s if s.count(e) % 2 == 1]
   c = []
   for e in t:
       if e not in c:
           c.append(e)
    ...
    for listek in listki:
        ...
        for stevilka in vrstni_red:
             ...
             if stevilka in listek:
                 precrtan.append(stevilka)
                 menjave +=1
             if sorted(precrtan) == sorted(listek):
                 a.append(listek)
                 break
        seznam_korakov.append(koraki-menjave)
    najmin = seznam_korakov.index(min(seznam_korakov))
    if len(a) == 1:
        return a[0]
    else:
        return listki[najmin]

def selitve(zacetek, datoteka_selitev, kraj):
    slovar_krajev = {}
    for e in zacetek:
        kraj, oseba = e
        slovar_krajev[kraj]={oseba}
    for vrstica in open(datoteka_selitev):
        seznamcek = []
        t = vrstica.split()
        for e in t:
            if e in slovar_krajev:
                seznamcek.append(e)
        prejsnji = seznamcek[0]
        novi = seznamcek [1]

        slovar_krajev[novi] |= slovar_krajev[prejsnji]
        slovar_krajec[prejsnji] = set()
    return  slovar_krajev[kraj]
...
def brez_para(nogavica, nogavice):
    if len(nogavice) == 1:
        return True
    elif nogavice[0] == nogavica:
        if nogavica not in nogavice[1:]:
            return True
        elif(nogavice[1:].count(nogavica)) % 2 == 0:
         return True
        else:
         return False
    else:
        brez_para(nogavica, nogavice[1:])
