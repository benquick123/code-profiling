...
class Sledilnik:
    ...
    def kje_zivi(self, oseba):
        rez="neki"
        for kraj, noseba in self.zacetna:
            if oseba == noseba:
                rez= kraj
                break
    ...
    def prebivalci(self, kraj):
        sez= []
        for nkraj, oseba in self.zacetna:
            if nkraj == kraj:
                sez.append(oseba)
        mno= set(sez)
        return mno
    ...
    def preseli(self, oseba, kraj):
        for nkraj, noseba in self.zacetna:
            if noseba==oseba:
                nkraj = kraj
                self.stevec+=1

    def preseli_vse(self, odkod, kam):
        for k, o in self.zacetna:
            if k == odkod:
                k= kam
                self.stevec+=1
...
def brez_para(nogavica, nogavice):
    if nogavica == nogavice[0]:
        pass # = return brez_para(nogavica, nogavice[1:])
