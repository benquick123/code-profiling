...
def bingo(listki, vrstni_red):
    ...
    dic = defaultdict(int)
    konc = False
    for i, e in enumerate(vrstni_red):
        for j, igralec in enumerate(listki):
            if e in igralec:
                dic[j] += 1
                if dic[j] == duzina:
                    konc = True
...
class Sledilnik:
    def __init__(self, zacetno):
        ...
        self.kdo_kje = {}
        for kraj, ime in zacetno:
            self.kdo_kje[ime] = kraj

        self.stevilo = 0

    def kje_zivi(self, oseba):
        return self.kdo_kje[oseba]

    def prebivalci(self, kraj):
        mnoz = set()
        for k, v in self.kdo_kje.items():
            if v == kraj:
                mnoz.add(k)
