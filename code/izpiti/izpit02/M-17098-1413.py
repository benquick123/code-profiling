...
def nogavice(s):
    ...
    for i in s:
        ...
        if stistih % 2 != 0:
            retseznam.append(i)
    for i in retseznam:
        if retseznam.count(i) > 1:
            retseznam.pop(retseznam.index(i))
    return retseznam
...
def bingo(listki, vrstni_red):
    ...
    indeks = 0
    for i in listki:
        maxindeksvrste.append(0)
        for ii in i:
            if ii not in vrstni_red:
                break
            else:
                ind = vrstni_red.index(ii)
                if ind > maxindeksvrste[indeks]:
                    maxindeksvrste[indeks] = ind
        indeks += 1

    return listki[maxindeksvrste.index(min(maxindeksvrste))]


def brez_para(nogavica, nogavice): #ne vem kako bi vpeljav rekurzijo
    if nogavice.count(nogavica) % 2 == 0:
        return False
    else: return True
    ...
    def preseli_vse(self, odkod, kam):
        ...
        for i in self.polje:
            if kam == i[0]:
                for i in poljeodkod:
                    i[1].append(i)
        self.stselitev = self.stselitev + poljeodkod.len()

    def selitev(self):
        return self.stselitev
