...
class Sledilnik:
    def __init__(self, kraji):
        self.kraji = kraji
        self.slovar = defaultdict(str)
        for kraj in kraji:
            self.slovar[kraj[0]] = kraj[1]
        self.selitve = 0

    def kje_zivi(self, oseba):
        for a, b in self.slovar.items():
            if oseba in b:
                return a
