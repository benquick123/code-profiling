...
def nogavice(s):
    t = []
    for nogavica in s:
        if s.count(nogavica) % 2 != 0 and nogavica not in t:
            t.append(nogavica)
    return t
        ...
        for beseda in vrstica:
            if beseda[1:-1] in slovar:
                if v == 0:
                    ...
                    continue
                if v == 1:
                    slovar[beseda[1:-1]] |= iz
    return slovar[kraj]

def brez_para(nogavica, nogavice):
    if nogavice[0] != nogavica:
        brez_para(nogavica, nogavice[1:])
    return nogavice.count(nogavica) % 2 == 1

class Sledilnik:
    def __init__(self, razporeditev):
        self.stevilo = 0
        self.razporeditev = razporeditev
        self.slovar = collections.defaultdict(set)
        for a, b in self.razporeditev:
            self.slovar[a].add(b)

    def kje_zivi(self, oseba):
        for a in self.slovar:
            if oseba in self.slovar[a]:
                return a
    ...
    def preseli(self, oseba, kraj):
        kje = self.kje_zivi(oseba)
        self.slovar[kje] -= set(oseba)
        self.slovar[kraj] |= set(oseba)
        self.stevilo += 1
