...
import collections
def nogavice(s):
    videni = collections.defaultdict(int)
    for nogavica in s:
        videni[nogavica] += 1
    sami = [stumf for stumf in s if videni[stumf]%2!=0]
    return sami


def bingo(listki, vrstni_red):
    enake_stevilke = collections.defaultdict(list)
    for i, stevilka in enumerate(vrstni_red):
        for n, listek in enumerate(listki):
            if stevilka in listek:
                enake_stevilke[n] += stevilka
            if len(enake_stevilke[n]) == len(listki[n]):
                return enake_stevilke[n]

def selitve(zacetek, datoteke_selitev, kraj):
    datoteka = open(datoteke_selitev)
    navodila = []
    for vrstica in datoteka.readlines():
        vrstica =vrstica.split()
        kraj1, kraj2 = None, None
        for beseda in vrstica:
            if "" in beseda and kraj2 is None:
                kraj1 = beseda
            elif "" in beseda:
                kraj2 = beseda
        navodila.append((kraj1, kraj2))
    for (iz_kraj, v_kraj) in navodila:
        for i, (kraj, ime) in enumerate(zacetek):
            if iz_kraj == kraj:
                zacetek[i] = v_kraj, ime
    return set(ime for nov_kraj, ime in navodila if nov_kraj == kraj)

def presteti(nogavica, nogavice):
    presteti = 0
    if nogavice[0] == nogavica:
        presteti = 1
    for zokn in nogavice[1:]:
        presteti = brez_para(zokn, nogavice)
    return presteti
def brez_para(nogavica, nogavice):
    return bool(1 if presteti(nogavica, nogavice)%2!=0 else 0)
class Sledilnik:
    def __init__(self, zacetek):
        self.doma = {ime: kraj for kraj, ime in zacetek}
        self.selitve = 0

    def kje_zivi(self, oseba):
        return self.doma[oseba]
    def prebivalci(self, kraj):
        return {ime for ime, ta_kraj in self.doma.items() if ta_kraj == kraj}
    def preseli(self, oseba, kraj):
        self.doma[oseba] = kraj
        self.selitve += 1
    def preseli_vse(self, odkod, kam):
        for ime, ta_kraj in self.doma.items():
            if ta_kraj == odkod:
                self.doma[ime] = kam
                self.selitve += 1
    def selitev(self):
