...
def nogavice(s):
    ...
    for nogavica in range(len(s)):
        for nogavica2 in s:
            if s[nogavica] == nogavica2:
                e[nogavica] += 1

    for neki in range(len(e)):
        if e[neki]%2 != 0:
            if s[neki] not in r:
                r.append(s[neki])
...
def bingo(listki, vrstni_red):
    daup = []
    zmagovalni = []
    for k in range(len(listki)):
        zmagovalni.append("".join(str(listki[k])))
    ...
    for korak in vrstni_red:
        for listek in range(len(listki)):
            if len(listki[listek]) == 0:
                print(zmagovalni[listek])
                zm = zmagovalni[listek]
                h = []
                for z in list(zm):
                    if z.isdigit():
                        h.append(int(z))
            ...
            if korak in listki[listek]:
                listki[listek].remove(korak)
...
class Sledilnik:
    def __init__(self, razporeditev):
        self.razporeditev = razporeditev
        self.selitve = 0

    def kje_zivi(self, oseba):
        for kraj, pers in self.razporeditev:
            if pers == oseba:
                nk = kraj
        return (nk)
    ...
    def prebivalci(self, kraj):
        m = set()
        for place, pers in self.razporeditev:
            if kraj == place:
                m.add(pers)
    ...
    def preseli(self, oseba, kraj):
        for place in range(len(self.razporeditev)):
            if self.razporeditev[place][1] == oseba:
                self.razporeditev[place] = (kraj, self.razporeditev[place][1])
                self.selitve += 1

    def preseli_vse(self, odkod, kam):
        for place in range(len(self.razporeditev)):
            if self.razporeditev[place][0] == odkod:
                self.razporeditev[place] = (kam, self.razporeditev[place][1])
                self.selitve += 1
