...
def nogavice(s):
    ...
    for n, c in d.items():
        if c % 2 != 0:
            seznam_brezpara.append(n)
    return seznam_brezpara
...
def nogavice(s):
    ...
    if not s:
        return None
    stevci = Counter(s)
    for n, c in stevci.items():
        if n % 2 != 0:
            seznam_brezpara.append(c)
    return seznam_brezpara
...
def bingo(listki, vrstni_red):
    listki1 = listki.copy()
    for listek1 in listki1:
        for stevilka in listek1:
            for klicana in vrstni_red:
                if stevilka == klicana:
                    listek1.remove(stevilka)
                    if len(listek1) == 0:
                        pass
...
def selitve(zacetek, datoteka_selitev, kraj):
    kraja = []
    premesceni = set()
    f = open(datoteka_selitev)
    for vrstica in f:
        element = vrstica.split()
        if element[0] == '"':
            kraja.append(element)
    for k, i in zacetek:
        if k in kraja:
            konec = kraja[1]
            if konec == kraj:
