...
def nogavice(s):
    sez_brez_para = []
    for i in s:
        if i in sez_brez_para:
            sez_brez_para.remove(i)
        else:
            sez_brez_para.append(i)
    return sez_brez_para

def bingo(listki, vrstni_red):
    listek_1, listek_2, listek_3 = listki[0], listki[1], listki[2]
    st = 0
    for i in range(len(vrstni_red)):
        for j in listek_1:
            if j == vrstni_red[st]:
                del listek_1[vrstni_red[st]]
                break
        for k in listek_2:
            if k == vrstni_red[st]:
                del listek_2[vrstni_red[st]]
                break
        for z in listek_3:
            if z == vrstni_red[st]:
                del listek_3[vrstni_red[st]]
                break
        if len(listek_1) == 0:
            return listki[0]
            break
