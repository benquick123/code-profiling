def nogavice(s):
    ...
    for x in s:
        if x not in nov:
            nov.append(x)
    return [x for x in nov if kosara[x] % 2 != 0]

def bingo(listki, vrstni_red):
    stki = listki
    for x in vrstni_red:
        for i, y in enumerate(stki):
            if x in y:
                y.remove(x)
                if len(y) == 0:
                    return listki[i]
...
def selitve(zacetek, datoteka_selitev, kraj):
    d = dict(zacetek)
    for x in open(datoteka_selitev, encoding="utf-8"):
        mesto = []
        for y in x[x.find('"'):].strip().split('"'):
            if y and y[0]:
                mesto.append(y)
        d[mesto[-1]] += d[mesto[0]]
        d[mesto[0]] = 0
...
def brez_para(nogavica, nogavice):
    prva = [nogavice[0]]
    if prva[-1] == nogavica:
        if nogavice[1:].count(nogavica) % 2 != 0:
            return False
        else:
            return True
    else:
