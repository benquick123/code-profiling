    ...
    for a in vrstni_red:
        for b in list:
            if a in b:
                b.pop(b.index(a))
            if len(b) == 0:
                return listki[list.index(b)]


def selitve(zacetek, datoteka_selitve,kraj):
    a = Sledilnik(zacetek)
    for line in open(datoteka_selitve):
        list = []
        for word in line.split():
            if word[0] == '"':
                list.append(word[1:len(word)-1])
        a.preseli_vse(list[0],list[1])

    return a.prebivalci(kraj)



def brez_para(nogavica,nogavice):
    return False if nogavice.count(nogavica) % 2 == 0 else True
...
class Sledilnik:
    def __init__(self, razdelitev):
        ...
        for a in razdelitev:
            if a not in seta:
                seta += a
                self.slovar = {a:{}}
        for terka in razdelitev:
            if terka[0] in self.slovar:
                self.slovar[terka[0]] = self.slovar[terka[0]] + {terka[1]}
            else:
                self.slovar[terka[0]] = {terka[1]}
    ...
    def preseli(self,oseba,kraj):
        self.slovar[oseba] = kraj
        self.st_selitev +=1

    def preseli_vse(self,odkod,kam):
        list = self.prebivalci(odkod)
        for a in list:
            self.slovar[a] = kam
            self.st_selitev += 1
