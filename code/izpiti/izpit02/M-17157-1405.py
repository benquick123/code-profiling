...
def nogavice(s):
    l = []
    for num in s:
        if s.count(num) % 2 != 0 and num not in l:
            l.append(num)
    return l
...
def bingo(listki,vrstni_red):
    ...
    for glavna in vrstni_red:
        for index,seznam in enumerate(listki):
            ind = str(index % l)
            if glavna in seznam:
                d[ind].append(glavna)
            if d[ind] == seznam:
...
def selitve(zacetek, datoteka_selitev, kraj):
    for vrstica in open(datoteka_selitev):
        for bes in vrstica:
            if bes == isinstance(bes,str):
                odkod = bes
            if bes != odkod and bes == isinstance(bes,str):
                kam = bes
...
class Sledilnik:
    ...
    def prebivalci(self,kraj):
        ...
        for tup in self.seznam:
            if tup[0] == kraj:
                self.mnozica.add(tup[0])
        return self.mnozica

    def preseli(self,oseba,kraj):
        self.tuple = (kraj, oseba)
        for tup in self.seznam:
            if tup[1] == oseba:
                tup = self.tuple
                self.st_selitev += 1
