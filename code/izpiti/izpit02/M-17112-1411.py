...
def bingo(listki, vrstni_red):
    ll = [i[:] for i in listki]
    for st in vrstni_red:
        for st_l, l in enumerate(ll):
            if st in l:
                l.remove(st)
            if len(l) == 0:
                return listki[st_l]
...
def selitve(zacetek, datoteka_selitev, kraj):
    imenik = dict()
    for lok, oseba in zacetek:
        imenik[lok] = {oseba}

    dat = open(datoteka_selitev)
    for vrst in dat:
        bes = vrst.split("\"")
        imenik[bes[3]] |= imenik[bes[1]]
        imenik[bes[1]].clear()
...
class Sledilnik():
    def __init__(self, zacetek):
        self.imenik = dict()
        for lok, oseba in zacetek:
            self.imenik[lok] = {oseba}
        self.migracije = 0

    def kje_zivi(self, oseba):
        for kraj, osebe in self.imenik.items():
            print(kraj, osebe)
            if oseba in osebe:
                return kraj

    def prebivalci(self, kraj):
        return self.imenik[kraj]

    def preseli(self, oseba, kraj):
        for a, osebe in self.imenik.items():
            if oseba in osebe:
                izvor = a
        self.imenik[izvor].remove(oseba)
        self.imenik[kraj].add(oseba)
        self.migracije += 1

    def preseli_vse(self, odkod, kam):
        self.imenik[kam] |= self.imenik[odkod]
        self.migracije += len(self.imenik[odkod])
        self.imenik[odkod].clear()

    def selitev(self):
        return self.migracije
