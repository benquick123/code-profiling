...
def bingo(listki,vrstni_red):
    ...
    for listek in listki:
        igralci[igralec(listek,vrstni_red)]=listek

    return igralci[min(igralci)]



def igralec(listek,vrstni_red):
    i=0
    tester=[]
    for stevilka in listek:
        tester.append(stevilka)
    for stevilka in vrstni_red:
        if len(tester)==0:
            return i
            break
        else:
            if stevilka in tester:
                tester.remove(stevilka)
            i += 1
    return i
...
def selitve(zacetek,datoteka_selitev,kraj):
    sez=[]
    output=set()
    datoteka = open(datoteka_selitev, "r")
#da izločimo kraje iz vrstice
    for vrstica in datoteka:
        razclenjena_vrstica = vrstica.split(" ")
        test=[]
        for elementi in razclenjena_vrstica:
            if elementi.startswith('"') and elementi.endswith('"'):
                test.append(elementi)
            elif elementi.startswith('"') and not elementi.endswith('"'):
                for elementi2 in razclenjena_vrstica:
                    if not elementi2.startswith('"') and elementi2.endswith('"'):
                        test.append((elementi+elementi2))
            elif not elementi.startswith('"') and elementi.endswith('"'):
                for elementi2 in razclenjena_vrstica:
                    if elementi2.startswith('"') and not elementi2.endswith('"'):
                        test.append(elementi+elementi2)
        for par1,par2 in test:
            sez.append((par1,par2)) #dobimo terko z krajema
    return output
""""   for kraj,oseba in zacetek:
        for mesto, stanovalec in sez:
"""
...
def brez_para(nogavica,nogavice):
    if len(nogavice)==2:
        if nogavice[0] == nogavica and nogavice[1] == nogavica:
            return False
        elif nogavice[0] != nogavica and nogavice[1] != nogavica:
            return False
        else:
            return True
    elif len(nogavice)<2:
        if nogavica in nogavice:
            return True
        else:
            return False
    else:
        return brez_para(nogavica,nogavice[2:])
...
class Sledilnik:
    ...
    def kje_zivi(self,oseba):
        for kraj,stanovalec in self.zacetek:
            if stanovalec == oseba:
                return kraj

    def prebivalci(self,kraj):
        a=set()
        for mesto,stanovalec in self.zacetek:
            if kraj == mesto:
                a.add(stanovalec)
        return a
