...
def bingo(listki, vrstni_red):
    ...
    for listek in listki:
        for stevilke in listek:
            if stevilke in vrstni_red and stevec < len(listek):
                seznam.append(stevilke)
                stevec += 1
    return seznam
...
class Sledilnik():
    ...
    def kje_zivi(self, oseba):
        for kraj, ime in self.z:
            if ime == oseba:
                return kraj

    def prebivalci(self, kraj):
        mnozica = set()
        for vas, ime in self.z:
            if kraj == vas:
                mnozica.add(ime)
