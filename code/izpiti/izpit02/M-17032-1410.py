...
def selitve(initial, migration, place):
    s = []
    for i, j in initial:
        for x in open(migration):
            a, b, c, d = x.split("\"") # b = from d = to
...
def brez_para(sock, socks):
    if len(socks) == 1 and sock in socks:
        return False
    elif (sock == socks[0]) and (socks[0] == socks[1]):
        return brez_para(sock, socks[2:])
    else:
        return True
