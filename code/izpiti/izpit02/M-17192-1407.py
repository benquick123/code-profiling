...
def selitve(zacetek,datoteka_selitev,kraj):
    ...
    for vrstica in datoteka:
        kraj1=vrstica.split('"')[1]
        kraj2=vrstica.split('"')[3]
        for x in zacetek: #vsak kraj posebej
            if x[0]==kraj1:#ce se kraj ujema z izseljevanjem
                imena=x[0][1:]
            if x[0]== kraj2:
                x|set(imena)
            if x[0]== kraj:
