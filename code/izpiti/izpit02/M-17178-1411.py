def nogavice(s):
    ...
    prejsnji = s[0]
    naprej = s[1:]
    for indeks, element in enumerate(naprej):
        if element == prejsnji:
            pari.append(element)
            prejsnji = element
        else:
...
"""def brez_para(nogavica, nogavice):
    for indeks, element in enumerate(nogavice):"""


class Sledilnik:
    def __init__(self, zacetna):
        self.zacetka = set(zacetna)

    def kje_zivi(self, oseba):
        for plac, clovek in self.zacetka:
            if clovek == oseba:
                return plac
...
sledilnik = Sledilnik([("Ljubljana", "Jana"), ("Šentvid", "Vid"), ("Šentvid", "Blaž")])
print(sledilnik.kje_zivi("Vid"))
print(sledilnik.prebivalci("Šentvid"))

print(set([("Ljubljana", "Jana"), ("Šentvid", "Vid"), ("Šentvid", "Blaž")]))
