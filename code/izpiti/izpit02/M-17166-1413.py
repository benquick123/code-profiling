...
def nogavice (s):
    ...
    nepari = []
    for stevilka in s:
        if stevilka not in seznam:
            seznam[stevilka] = 1
        else:
            seznam[stevilka] += 1
    for stumpf, stevilo in seznam.items():
        if stevilo % 2:
            nepari.append(stumpf)
    return nepari
"""
...
def selitve(zacetek, datoteka_selitev, kraj):
    datoteka = open(datoteka_selitev,  "r",  encoding = "utf-8", )
