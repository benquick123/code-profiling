...
def bingo(listki, vrstni_red):
    vrstni_red_dict=defaultdict()
    for i, el in enumerate(vrstni_red):
        vrstni_red_dict[el]=int(i)

    listki_dict=defaultdict()

    for listek in listki:
        for el in listek:
            if el in vrstni_red_dict:
                listki_dict[listek]+=vrstni_red_dict[el]
...
def selitve(zacetek, datoteka_selitev, kraj):
    for el in zacetek:
        kje_kdo_zivi={el[0]:el[1]}

    po_selitvi=defaultdict()
...
class Sledilnik:
    def __init__(self,zacetno_stanje,mesto,ime):
        self.mesto=zacetno_stanje.key()
        self.ime=zacetno_stanje
