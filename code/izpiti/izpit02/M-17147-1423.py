...
"""def selitve(zacetek, datoteka_selitev, kraj):
    ...
    for vrstica in datoteka_selitev:
        vrstica.split(""):
"""
def brez_para(nogavica, nogavice):

    if nogavice[0] ==nogavica:
        return (nogavica[0] + brez_para(nogavica, nogavice[1:])%2==1)
    else:
        return (brez_para(nogavica, nogavice[1:]))%2==1


class Sledilnik:
    """    self.slovar== collections.defaultdict(int)
    mnozica{}
    selitev=0
"""
    ...
    def kje_zivi(self, oseba):
        return self.slovar[oseba]

    """    def prebivalci(self, kraj):
        for e in self.slovar:
            if self.slovar[e]==kraj
                mnozica.add(e)
        return mnozica
"""
