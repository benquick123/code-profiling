...
def nogavice(s):
    ...
    for i in s:
        counter[i] += 1
    pairless = [type for type, count in counter.items() if count%2 == 1]
    out = []
    for i in s:
        if i in pairless and i not in out:
            out.append(i)
    return out


def bingo(lists, order):
...
def brez_para(sock, socks):
    socknum = socks.count(sock)
    relevant_socks = [sock]*socknum
    if socknum%2 == 0:
        return False
    else:
        return True
...
class Sledilnik:
    def __init__(self, initial_loc):
        ...
        for loc, name in initial_loc:
            self.register[loc] += [name]
        self.nummove = 0
        print (self.register)

    def kje_zivi(self, person):
        for loc, name in self.register.items():
            if person in name:
                return loc

    def prebivalci(self, place):
        return set(self.register[place])

    def preseli(self, person, to_place):
        for loc, ppl in self.register.items():
            if person in ppl:
                self.register[loc].remove(person)
                self.register[to_place] += [person]
                self.nummove += 1

    def preseli_vse(self, from_place, to_place):

        temp = self.register[from_place]
        self.register[from_place] = []
        self.register[to_place] += temp
        self.nummove += len(temp)

    def selitev(self):
        return self.nummove

def selitve(initial, migrations, place):
    helper = Sledilnik(initial)

    migr_moves = []
    r = open(migrations)
    for line in r:
        junk1, from_place, junk2, to_place, junk3 = line.strip().split("\"")
        migr_moves.append((from_place, to_place))

    for from_place, to_place in migr_moves:
        helper.preseli_vse(from_place, to_place)

    return set(helper.register[place])
