...
def nogavice(s):
    a=set()
    for nogavica in s:
        if not s.count(nogavica)%2==0:
            a.add(nogavica)
    return [x for x in a]

def bingo(listki,vrstni_red):
    listki_b=listki.copy()
    for stevilka in vrstni_red:
        for x,listek in enumerate(listki):
            for y,st in enumerate(listek):
                if st==stevilka:
                    del listki[x][y]
            if listek==[]:
                return listki_b[x]
...
class Sledilnik:
    def __init__(self,prebivalci):
        self.prebivalci_st=prebivalci
        self.selitve=0
    def kje_zivi(self,oseba):
        for mesto,ime in self.prebivalci_st:
            if ime==oseba:
                return mesto
    ...
    def preseli(self,oseba,kraj):
        for i, mesto_ime in enumerate(self.prebivalci_st):
            if mesto_ime[1] == oseba:
                self.prebivalci_st[i][0]=kraj
                self.selitve+=1
    def preseli_vse(self,odkod,kam):
        for i,mesto_ime in enumerate(self.prebivalci_st):
            if mesto_ime[1]==odkod:
                self.prebivalci_st[i][0]=kam
                self.selitve += 1
    def selitev(self):
        return self.selitve
...
def selitve(zacetek,datoteka_selitev,kraj):
    ...
    for vrstica in open(datoteka_selitev):
        tab=vrstica.split('"')
        selitev.append((tab[1],tab[3]))
    x=Sledilnik(zacetek)
    for iz,v in selitev:
        x.preseli_vse(iz,v)
    return x.prebivalci(kraj)


def brez_para(nogavica,nogavice):
    i=0
    for x,nog in enumerate(nogavice):
        if nog==nogavica:
            i+=1
            del nogavice[x]
            print(nogavice)
