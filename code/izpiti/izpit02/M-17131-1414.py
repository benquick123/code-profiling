...
def nogavice(s):
    ...
    for a in stevci:
        if a % 2 == 0:
            ...
            brez_para.append(x[stevci.index(a)])
...
def brez_para(nogavica, nogavice):
    stunfi = nogavice[nogavica]
    #for stunf in stunfi[nogavice]:

class Sledilnik:
    def __init__(self, zacetek):
        self.vrednosti = dict(zacetek)
        self.kraj = zacetek.get(zacetek.values())
        self.oseba = zacetek.get(zacetek.keys())



    def kje_zivi(self, oseba):
        return self.kraj(oseba)

    def prebivalci(self, kraj):
        return self.oseba(kraj)
