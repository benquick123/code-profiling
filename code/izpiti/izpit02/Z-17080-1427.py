...
def bingo(listki, vrstni_red):
    novi_listki = defaultdict(list)
    for i, e in enumerate(listki):
        novi_listki[i] += e
    for izžrebana in vrstni_red:
        for listek in novi_listki:
            if izžrebana in novi_listki[listek]:
                novi_listki[listek].remove(izžrebana)
                if len(novi_listki[listek]) == 0:
                    return listki[listek]


def selitve(zacetek, datoteka, kraj):
    mnozica = {}
    for vrstica in datoteka.split("\n"):
...
def brez_para(nogavica, nogavice):
    ...
    if nogavica == nogavice[0]:
        pojavitve.append(nogavica)
    else:
        pojavitve.append(brez_para(nogavice[1:]))
    return len(pojavitve) % 2 == 1
