    ...
    for listek in listki:
        ...
        for n in vrstni_red:
            if n in listek:
                listek.remove(n)
                if len(listek) == 0:
                    slovar[st_krogov] = kopija_lista
                    break
            st_krogov += 1
    return slovar[max(slovar)]

def selitve(zacetek, datoteka_selitev, kraj):
    with open(datoteka_selitev) as d:
        for vrstica in d:
            kraj_doma = vrstica.split("iz")[1]
            kraj_selitev = vrstica.split("v")[1]

def brez_para(nogavica,nogavice):
    if not nogavice:
        return 0
    if nogavica == nogavice[0]:
        return 1 + brez_para(nogavica,nogavice[1:])
    else:
        return brez_para(nogavica,nogavice[1:])

class Sledilnik:
    def __init__(self,zacetek,datoteka_selitev,kraj):
        self.kraj, self.vkraj = open(datoteka_selitev).readlines().split("")

    def kje_zivi(self,oseba):
        return self.kraj

    def prebivalci(self, kraj):
        return set(self.prebivalci)
