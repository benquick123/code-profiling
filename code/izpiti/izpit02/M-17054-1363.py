...
class Sledilnik():
    def __init__(self, zacetek):
        self.start = {}
        for pair in zacetek:
            self.start[pair[0]] = [pair[1]]
        self.moves = 0

    def kje_zivi(self, oseba):
        for city in self.start:
            if oseba in self.start[city]:
                return city

    def prebivalci(self, kraj):
        return set(self.start[kraj])

    def preseli(self, oseba, kraj):
        self.moves += 1
        self.start[kraj].append(self.start[self.kje_zivi(oseba)].pop(self.start[self.kje_zivi(oseba)].index(oseba)))

    def preseli_vse(self, odkod, kam):
        self.start[kam] += self.start[odkod]
        self.moves += len(self.start[odkod])
        self.start[odkod] = []
