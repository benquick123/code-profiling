class Carovnik:
    ...
    def caraj(self, s):
        t = []
        kaj_spreminja = self.niz[:self.niz.index("-")].strip()
        v_kaj = self.niz[self.niz.index(">") + 1:].strip()

        for e in s:
            if e == kaj_spreminja:
                t.append(v_kaj)
                self.zasluzeno += self.cena
            else:
                t.append(e)
        return t

    def zasluzek(self):
        return self.zasluzeno
...
def najpogostejse_polje(navodila):
    ...
    for znak in navodila:
        if znak == "D":
            ...
            robot.naprej(int(znak))
            seznam_polj.append(robot.koordinate())

    max = 0
    for e in seznam_polj:
        if seznam_polj.count(e) > max:
            max = seznam_polj.count(e)
            koordinat = e
    return koordinat
...
def navodila(zaporedje):
    ...
    while i < len(zaporedje):
        if zaporedje[i] != "D" and zaporedje[i] != "L":
            t.append(zaporedje[i])
            while i + 1 < len(zaporedje) and zaporedje[i + 1] != "D" and zaporedje[i + 1] != "L":
                t[-1] += zaporedje[i+1]
                i += 1
        else:
            t.append(zaporedje[i])
...
def zamenjano(s, menjave):
    return [menjave[e] if e in menjave else e for e in s]
...
def sprazni(s):
    return [sprazni(e) for e in s if e is not None]
