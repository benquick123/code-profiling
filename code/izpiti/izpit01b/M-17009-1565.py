def najpogostejse_polje(navodila):
    ...
    najpogostejsa = ""
    pogostost = 0
    for trenutna in seznam:
        counter = 0
        for koordinata in seznam:
            if trenutna == koordinata:
                counter = counter + 1
            if counter > pogostost:
                najpogostejsa = trenutna
                pogostost = counter
...
def zamenjano(s, menjave):
    novseznam = []
    for oseba in s:
        if oseba in menjave.keys():
            novseznam.append(menjave[oseba])
        else:
            novseznam.append(oseba)
    return novseznam

def zamenjaj(s, menjave):
    i = 0
    while i < len(menjave):
        if s[0] in menjave.keys():
            s.append(menjave[s[0]])
        else:
            s.append(s[0])
        s.pop(0)
        i = i + 1
...
class Carovnik:
    def __init__(self,magija,cena):
        self.a = magija.split(" -> ")[0]
        self.b = magija.split(" -> ")[1]
        self.cena = cena
        self.zasluzek1 = 0
    def caraj(self,s):
        seznam = []
        for element in s:
            if element == self.a:
                seznam.append(self.b)
                self.zasluzek1 = self.zasluzek1 + self.cena
            else:
