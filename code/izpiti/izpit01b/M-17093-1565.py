def najpogostejse_polje(navodila):
    ...
    for x in navodila:
        if x == "D":
            ...
            a.naprej(int(x))
            if a.koordinate() not in koord:
                koord[a.koordinate()] = 0
            else:
                koord[a.koordinate()] += 1
    return max(koord.keys())
...
class Carovnik():
    def __init__(self, zacaraj, cena):
        self.zacaraj = zacaraj
        self.cena = cena
        self.zasluzi = 0
    ...
    def caraj(self, s):
        a = []
        for x in s:
            if x == self.zacaraj.split("->")[0].strip():
                a.append(self.zacaraj.split("->")[1].strip())
                self.zasluzi += self.cena
            else:
                a.append(x)
        return a
