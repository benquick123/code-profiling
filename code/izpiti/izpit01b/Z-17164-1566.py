def najpogosteje_polje(navodila):
    l = []
    koordinate = (0,0)
    for e in navodila:
        if e.isalpha() == True:
...
def navodila(zaporedje):
    ...
    for e in zaporedje:
        if e.isalpha() == True:
            ...
            e = int(e)
            l.append(e)
    for e1, e2 in zip(zaporedje, zaporedje[1:]):
        if e1 is int() and e2 is int():
            e = ''.join((e1, e2))
    return (l)
...
class Carovnik:
    def __init__(self, zna, zasluzek ):
        self.zna = zna.split(' -> ')
    ...
    def caraj(self, s):
        l = []
        for e in s:
            if e not in self.zna[0]:
                l.append(e)
            else:
                e.replace(self.zna[1])
                l.append(e)
        return l
