...
def najpogostejse_polje(navodila):
    r = Robot
    najpogostejsePolje = (0,0)
    navodila = navodila.strip()
    koordinate = []
    koordinati = (0,0)
    counter = 0
    for i in navodila:
    ...
    for i in koordinate:
        if koordinate.count(i) > counter:
            counter = koordinate.count(i)
            koordinati = i
    return koordinati

def navodila(zaporedje):
    seznam = []
    zaporedje = zaporedje.strip()
    stevke = ""
    seznamstevk = ['1','2','3','4','5','6','7','8','9']
    counter = 0
    j = 0
    i = 0
    while i < len(zaporedje):
        while j == i:
            if zaporedje[i] in seznamstevk:
                stevke += zaporedje[i]
                counter += 1
                j+= 1
            else:
        ...
        if stevke == "":
            ...
            if counter > len(stevke) or zaporedje[i] not in seznamstevk:
...
def zamenjano(s,menjave):
    ...
    for i in range(len(s)):
        j = s[i]
        if s[i] in menjave:
            seznam.append(menjave[j])
        else: seznam.append(s[i])
    return seznam
