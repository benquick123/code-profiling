...
class Carovnik:
    def __init__(self, kaj_zna, cena):
        l = kaj_zna.split(" -> ")
        self.iz_cesa = l[0]
        self.v_kaj = l[1]
        self.cena = cena
...
def najpogostejse_polje(navodila):
    ...
    for navodilo in navodila:
        if navodilo == "D":
            ...
            r.naprej(int(navodilo))
            d[(r.x, r.y)] += 1
    return max((ponovitev, koord) for koord, ponovitev in d.items())[1]
