...
def najpogostejse_polje(navodila):
    ...
    prestej = Counter(polja)
    najvec = sorted((value, key) for key, value in prestej.items())
    return najvec[-1][1]
...
def navodila(zaporedje):
    s = []
    stevila = []
    for indeks, e in enumerate(zaporedje):
        if e.isalpha():
            s.append(e)
...
class Carovnik:
    ...
    def caraj(self, s):
        precaran = []
        for element in s:
            if element == self.zna[0]:
                precaran.append(self.zna[1])
                self.skupaj += self.racuna
            else:
                precaran.append(element)
        return precaran
