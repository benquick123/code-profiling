...
def najpogostejse_polje(navodila):
    ...
    for i in navodila:
        if i in stevila:
            ...
            a.levo()
        if a.koordinate() in seznam_ukazov:
            seznam_ukazov[a.koordinate()] += 1
        else:
            seznam_ukazov[a.koordinate()] = 1

    if len(seznam_ukazov) >= 2:
        nov = {}
        for kljuc, vrednost in seznam_ukazov.items():
            nov[vrednost] = kljuc
        return nov[max(nov.values())]
