def navodila(zaporedje):
	...
	st = ""
	for e in zaporedje:
		if e not in "1235467890":
			if st != "":
				seznam.append(int(st))
				st = ""
			seznam.append(e)
		else:
			st += e
...
def najpogostejse_polje(navodila):
	...
	for ukaz in navodila:
		if ukaz in "1234567890":
			robocop.naprej(int(ukaz))
			seJePremaknil = True
		else:
			if seJePremaknil:
				seJePremaknil = False
				koord = robocop.koordinate()
				if koord in polja:
					polja[koord] += 1
				else:
					polja[koord] = 1
...
class Carovnik:
	def __init__(self, znanje, cena):
		self.cena = cena
		self.z = 0 #zasluzek
		self.izTega = znanje.split()[0]
		self.vTo = znanje.split()[2]
	
	def caraj(self, s):
		nov = []
		for element in s:
			if element == self.izTega:
				nov.append(self.vTo)
				self.z += self.cena
			else:
				nov.append(element)
