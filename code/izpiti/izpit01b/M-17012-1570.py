...
def najpogostejse_polje(navodila):
    ...
    for n in navodila:
        if n == "D":
            ...
            x = polja[-1][0] + int(n)* smer[0][0]
            y = polja[-1][1] + int(n) * smer[0][1]
            polja.append((x, y))
...
def navodila(zaporedje):
    stevila = [i for i in range(10)]
    zaporedje = list(zaporedje)
    nav = []
    tmp = ""
    for i in range(len(zaporedje)):
        if zaporedje[i] == "L" or zaporedje[i] == "D":
            if not tmp == "":
                nav.append(int(tmp))
            tmp = ""
            nav.append(zaporedje[i])
        elif int(zaporedje[i]) in stevila:
            tmp += zaporedje[i]
    if not tmp == "":
        nav.append(int(tmp))
    return nav
...
def zamenjano(s, menjave):
    tmp = []
    for e in s:
        if e in menjave.keys():
            tmp.append(menjave[e])
        else:
            tmp.append(e)
    return tmp
...
class Carovnik:
    def __init__(self, znanje, cena):
        self.cena = cena
        self.zas = 0
        self.znanje = znanje.split(" -> ")

    def caraj(self, s):
        tmp = []
        for e in s:
            if e == self.znanje[0]:
                tmp.append(self.znanje[1])
                self.zas += self.cena
            else:
                tmp.append(e)
