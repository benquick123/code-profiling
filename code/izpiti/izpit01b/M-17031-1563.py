def najpogostejse_polje(instructions):
    robot = Robot()
    coord_frequency = {(0,0): 1}
    for i in instructions:
        if i == "D":
            ...
            robot.naprej(int(i))
            if robot.koordinate() in coord_frequency:
                coord_frequency[robot.koordinate()] += 1
            else:
                coord_frequency[robot.koordinate()] = 1
    return max(coord_frequency, key = coord_frequency.get)

def navodila(sequence):
    b = []
    a = ""
    for i in sequence:
        if i.isdigit():
...
def zamenjano(s, swaps):
    return [swaps.get(i, i) for i in s]

def zamenjaj(s, swaps):
...
class Carovnik():
    def __init__(self, ability, cost):
    ...
    def caraj(self, s):
        a = []
        for i in s:
            if i in self.ability:
                a.append(self.ability[i])
