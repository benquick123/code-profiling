def najpogostejse_polje(navodila):
    ...
    maks_stevec = 0
    for element in set(polja):
        stevec = 0
        for element2 in polja:
            if element == element2:
                stevec += 1
        if stevec > maks_stevec:
            maks_stevec = stevec
...
class Carovnik:
    ...
    def __init__(self, niz, cena):
        prvotna, nova = str.split(niz, ' -> ')
        self.storitev = [prvotna, nova, cena]
        self.zasluzil = 0
    ...
    def caraj(self, s):
        ...
        zasluzil = 0
        for element in s:
            if self.storitev[0] == element:
                novi.append(self.storitev[1])
                zasluzil += self.storitev[2]
            else:
                novi.append(element)
        self.zasluzil += zasluzil
        return novi
