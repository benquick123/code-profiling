...
def najpogostejse_polje(navodila):
    ...
    for navodilo in a:
        if navodilo == "L":
            ...
            x.naprej(int(navodilo))
            xa, ya = x.koordinate()
            koord.append((xa, ya))
    b = []

    for neki in range(0, len(koord)):
        b.append(0)

    x = 0

    for koordinate in koord:
        for koordinate2 in koord:
            if koordinate == koordinate2:
                b[x] += 1
        x += 1
    naj = 0
    ind = 0
...
def navodila(zaporedje):
    ...
    for neki in a:
        if neki == "D":
            if len(stevila) > 0:
                p_list.append(int(''.join(stevila)))
            p_list.append(neki)
            stevila = []
        elif neki == "L":
            if len(stevila) > 0:
...
def zamenjano(s, menjave):
    ...
    for neki in s:
        if neki in menjave:
            a.append(menjave[neki])
        else:
            a.append(neki)
    return a

def zamenjaj(s, menjave):
    for neki in range(0, len(s)):
        if s[neki] in menjave:
            s[neki] = menjave[s[neki]]
        else:
            s[neki] = s[neki]
...
class Carovnik:
    def __init__(self, znam, cena):
        self.caramiz = znam.split(" -> ")[0]
        self.caramv = znam.split(" -> ")[1]
        self.cena = int(cena)
        self.denar = 0
    ...
    def caraj(self, s):
        nov = []
        for neki in s:
            if (neki == self.caramiz):
                nov.append(self.caramv)
                self.denar = self.denar + self.cena
            else:
                nov.append(neki)
        return nov
