...
def navodila(zaporedje): #7D3221LL
    seznam = []
    seznam2 = []
    for vsak in zaporedje:
        seznam.append(vsak)
    niz = ""
    for crka in zaporedje:
        if crka == "D":
            if niz != "":
                seznam2.append(int(niz))
                niz = ""
            seznam2.append(crka)
        elif crka == "L":
            if niz != "":
    ...
    if niz != "":
        seznam2.append(int(niz))
    return seznam2
print(navodila("D232DLL23233"))


def zamenjano(s, menjave):
    seznam = []
    for i in s:
        if i in menjave.keys():
            seznam.append(menjave[i])
        else:
            seznam.append(i)
    return seznam
print(zamenjano(["Ana","Ana","Berta"], {"Ana":"Peter", "Berta":"Ana"}))
...
def sprazni(s):
    ...
    for i in s:
        if i != None:
            seznam.append(i)
    return seznam
print(sprazni([None, None, []]))


class Carovnik:
    def __init__(self, niz, racuna):
        self.iz = niz.split(" -> ")[0]
        self.v = niz.split(" -> ")[1]
        self.racuna = racuna
        self.ss = []
    ...
    def caraj(self, s):
        ...
        for self.i in s:
            if self.i == self.iz:
                self.seznam.append(self.v)
            else:
                self.seznam.append(self.i)
        return self.seznam

    def zasluzek(self):
        for self.j in self.ss:
            if self.j == self.iz:
                self.zasluzil += self.racuna
