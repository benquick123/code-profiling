...
def najpogostejse_polje(navodila):
    ...
    for i in navodila:
        if i.isdigit():
            bot.naprej(int(i))
            e.append(bot.koordinate())
        elif i == 'D':
            bot.desno()
        else:
            bot.levo()
    temp = Counter(e)
    return max(temp, key=temp.getitem)
...
def sprazni(s):
    ...
    if not s:
        ...
        for i in s:
            if i != None:
                seznam.append(sprazni(i))
    return seznam

class Carovnik:
    def __init__(self, niz, cena):
        self.zna = (niz.split('-> ')[0], cena)
        self.element = niz.split('-> ')[1]
        self._zasluzek = 0
    ...
    def caraj(self, s):
        ...
        for i in s:
            if i in self.zna[0]:
                seznam.append(self.element)
                self._zasluzek += self.zna[1]
            else:
                seznam.append(i)
        return seznam
