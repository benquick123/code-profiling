...
def najpogostejse_polje(navodila):
    ...
    for znak in navodila:
        if znak == "D":
            ...
            for i in range(int(znak)):
                rob.naprej(1)
                koord.append(rob.koordinate())
                if rob.koordinate() not in slov:
                    slov[rob.koordinate()] = 1
                else:
                    slov[rob.koordinate()] += 1
