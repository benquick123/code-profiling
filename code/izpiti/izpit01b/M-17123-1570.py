...
def najpogostejse_polje(navodila):
    stevec = 0
    for crka in navodila:
        if navodila[stevec] == int:
            a.naprej(navodila[stevec])
            stevec+= 1
        print(a.koordinate())
        if navodila[stevec] == "D":
            a.desno()
            stevec+= 1
        if navodila[stevec] == "L":
            a.levo()
            stevec+= 1
    print(stevec)
    return a.koordinate()
...
def navodila(zaporedje):
    ...
    for element in zaporedje:
        if zaporedje[trenutno] == "L" and trenutno < len(zaporedje):
            seznam_zaporedij.append(str(zaporedje[trenutno]))
            trenutno += 1
        if zaporedje[trenutno] == "D" and trenutno < len(zaporedje):
            seznam_zaporedij.append(str(zaporedje[trenutno]))
            trenutno += 1
        if zaporedje[trenutno] == int and trenutno < len(zaporedje):
            stevila.append(zaporedje[trenutno])
            seznam_zaporedij.append(stevila)
            trenutno += 1
    print("dolzina", len(zaporedje))
    print("zaporedje:", zaporedje)
    return seznam_zaporedij
...
def zamenjano(s, menjave):
    ...
    trenutni_elementi = 0
    for elementi in s:
        if elementi in menjave:
            trenutni_elementi = elementi
            nov_seznam.append(menjave[trenutni_elementi])
        if elementi not in menjave:
            nov_seznam.append(elementi)
    print("nov_seznam", nov_seznam)
    return nov_seznam
...
class Carovnik:
    def __init__(self,nekaj,koliko):
        self.zasluzeno = 0
        self.material = nekaj.split(" -> ")
        self.cena = koliko
        self.kolikokrat = 0

    def zasluzek(self):
        return self.kolikokrat

    def caraj(self,s):
        novi_materiali = []
        for nekaj in s:
            if nekaj == self.material[0]:
                novi_materiali.append(self.material[1])
                self.kolikokrat += self.cena
            else:
                novi_materiali.append(nekaj)
        return novi_materiali
