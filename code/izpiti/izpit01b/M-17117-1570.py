...
def navodila(zaporedje):
    ...
    for i in range(len(zaporedje)):
        ...
        if zaporedje[i].isupper():
            navodilo.append(zaporedje[i])

        elif zaporedje[i].isnumeric() and zaporedje[i-1].isnumeric() and len(navodilo) != 0:
            navodilo[-1] = int(str(navodilo[-1]) + str(zaporedje[i]))

        elif zaporedje[i].isnumeric() and not zaporedje[i-1].isnumeric() and len(navodilo) > 0:
            navodilo.append(int(zaporedje[i]))
...
def zamenjano(s, menjave):
    nov_s = []
    for element in s:
        if element in menjave.keys():
            nov_s.append(menjave[element])
        else:
            nov_s.append(element)

    return nov_s

#z spreminjanjem seznama:

def zamenjaj(s, menjave):
    for i in range(len(s)):
        if s[i] in menjave.keys():
            s[i] = menjave[s[i]]
