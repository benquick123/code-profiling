...
def zamenjano(s, menjave):
    seznam = s[:]
    for indx, ime in enumerate(seznam):
        if ime in menjave:
            del seznam[indx]
            seznam.insert(indx, menjave[ime])
    return seznam

def zamenjaj(s, menjave):
    for indx, ime in enumerate(s):
        if ime in menjave:
            del s[indx]
            s.insert(indx, menjave[ime])
...
def sprazni(s):
    ...
    if s[0] == None:
        ...
        return [s[0]] + sprazni(s[1:])


class Carovnik():
    def __init__(self, task, reward):
        self.tasks = {}
        self.tasks[task.split(" -> ")[0]] = (task.split(" -> ")[1], reward)
        self.chest = 0
    ...
    def caraj(self, s):
        for indx, item in enumerate(s):
            if item in self.tasks:
                del s[indx]
                s.insert(indx, self.tasks[item][0])
                self.chest += self.tasks[item][1]
        return s
