...
class Robot:
    ...
    def najpogostejse_polje(self, navodila):
        polja = [(self.x, self.y)]
        max = 0
        for n in navodila:
            if n == "D":
            ...
            if n == "L":
                ...
                self.naprej(int(n))
                polja.append(self.koordinate())
        for n in polja:
            if polja.count(n) > max:
                max = n
        return max
...
def sprazni(s):
    seznam = []
    if s[0] is not None:
        return seznam.append(s[0]) + sprazni(s[1:])

class Carovnik:
    def __init__(self, niz , cena):
        self.a = niz.split("->")
        self.zna = {self.a[0].strip(): self.a[1].strip()}
        self.cena = cena
        self.opravil = 0
