import unittest
"""
smeri = {"naprej(e)": (0, 0), "levo()": (1,0), "desno()": (-1,0)}
pomeni = {"e": naprej(int(e)), "L": levo(), "D" : desno()}

def najpogostejse_polje(niz):
    s = []
    x, y = 0, 0
    dx, dy = smeri[smer]
    e = "D"
    for e in niz:
        if e == str(e):
            ...
            o = pomeni["D"]
            smeri[pomeni["D"]] = smeri.values()
            x = x + smeri.values()[0]
            y = y + smeri.values()[1]
        elif e == "L":
            o = pomeni["L"]
            smeri[pomeni["L"]] = smeri.values()
            x = x + smeri.values()[0]
            y = y + smeri.values()[1]
