...
def navodila(zaporedje):
    ...
    a = []
    for z in zaporedje:
        if z[i].isalnum() and z[i+1].isalnum():
            a.append[z[i:i+1]]
        else:
            a.append(z)
        i += 1
...
def navodila(zaporedje):
    ...
    for element in zaporedje:
        seznam1.append(element)
        for x, y in enumerate(seznam1):
            if y[x].isalnum() and y[x+1].isalnum():
                seznam2 += y[x:x+1]
            else:
                seznam2.append(y)
    return list(seznam2)
...
def zamenjano(s, menjave):
    ...
    if not menjave:
        return s
    for ime in s:
        for kljuc, vrednost in menjave.items():
            if ime == kljuc:
                seznam.append(vrednost)
            if ime not in menjave and ime not in seznam:
                seznam.append(ime)
    return seznam
