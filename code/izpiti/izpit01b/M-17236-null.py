...
def najpogostejse_polje(s):
    ...
    for crka in s:
        if(crka == "D"):
            ...
            for i in range(int(crka)):
                janez.naprej(1)
                terka_koordinat.append(janez.koordinate())


    for a in terka_koordinat:
        if(terka_koordinat.count(a))>st_koordinat:
            st_koordinat = terka_koordinat.count(a)
            max_koordinata = a

    return max_koordinata
