class Minobot:
    ...
    def najpogosteje_polje(self,navodila):
        najvecje = 0
        rezultat = {}
        seznam = [self.koordinate()]
        for s in navodila:
            if s is "D":
                self.desno()
            elif s is "L":
...
def zamenjano(s, menjave):
    ...
    for kljuc in s:
        if kljuc in menjave:
            novi_seznam.append(menjave[kljuc])
        else:
            novi_seznam.append(kljuc)
    return novi_seznam
...
def zamenjaj(s, menjave):
    ...
    for kljuc in s:
        if kljuc in menjave:
            ...
            novi_seznam.append(kljuc)
    s.clear()
    for zameni in novi_seznam:
        s.append(zameni)


class Carovnik():
    def __init__(self, niz, vrednost):
        novi_niz = niz.split(" -> ")
        self.slovarij = {novi_niz[0]: novi_niz[1]}
    ...
    def caraj(self, s):
        novi_seznam = []
        for kljuc in s:
            if kljuc in self.slovarij:
                self.zasluzio = self.zasluzio + self.vrednost
                novi_seznam.append(self.slovarij[kljuc])
            else:
                novi_seznam.append(kljuc)
        return novi_seznam

    def zasluzek(self):
        return self.zasluzio
