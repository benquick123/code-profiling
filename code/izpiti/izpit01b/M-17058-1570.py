def najpogostejse_polje(navodila):
    ...
    najpogostejse = 0
    for x in ponovitve:
        if ponovitve[x] > najpogostejse:
            najpogostejse = ponovitve[x]
            najpogostejsa = x
...
def navodila(zaporedje):
    ...
    while index <= len(zaporedje):
        ...
        if x == "D":
            ...
            i = 1
            ukaz = x
            while zaporedje[zaporedje.index(x)+i] not in  "LD":
                ukaz += zaporedje[zaporedje.index(x)+i]
                i += 1
            index = zaporedje.index(zaporedje[zaporedje.index(x)+i])
            ukazi.append(int(ukaz))
        if index > len(zaporedje)-1:
            break
    return ukazi
...
def zamenjano(s, menjave):
    ...
    for ime in s:
        if ime in menjave:
            zamenjani.append(menjave[ime])
        else:
            zamenjani.append(ime)
    return zamenjani
...
class Carovnik:
    def __init__(self, carovnija, zaracuna):
        self.kaj = {}
        delo = carovnija.split( )
        self.kaj[delo[0]] = delo[2]
        self.koliko = zaracuna
        self.racun = 0
    ...
    def caraj(self, s):
        ...
        for ime in s:
            if ime in self.kaj:
                zamenjani.append(self.kaj[ime])
                self.racun += self.koliko
            else:
                zamenjani.append(ime)
        return zamenjani
