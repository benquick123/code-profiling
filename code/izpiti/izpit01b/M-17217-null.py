...
def najpogostejse_polje(navodila):
    ...
    for x in navodila:
        if x == "D":
            ...
            a.naprej(st)
        koor = a.koordinate()
        if koor not in slovar:
            slovar[koor] = 1
        else:
            slovar[koor] = slovar[koor] + 1
...
class Carovnik:
    ...
    def caraj(self,s):
        vrni = []
        for x in s:
            prej = self.moc.split()[0]
            pol = self.moc.split()[2]

            if x == prej:
                vrni.append(pol)
                self.skupno = self.skupno + self.cena
            else:
                vrni.append(x)
