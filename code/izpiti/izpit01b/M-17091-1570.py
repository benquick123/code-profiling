def najpogostejse_polje( navodila):
    ...
    for e in navodila:
        if e == "L":
            ...
            r.naprej(int(e))
            pot.append(r.koordinate())
    import collections
    kolikokrat = dict(collections.Counter(pot))
    return max(kolikokrat) # zmanjkalo časa

def zamenjano(s, menjave):
    t = []
    for i in range(len(s)):
        t.append(menjave.get(s[i], s[i]))
    return t

def zamenjaj(s, menjave):
    for i in range(len(s)):
        s[i] = menjave.get(s[i], s[i])

def navodila(zaporedje):

    samo_stevikle = {zaporedje.split("D")} | {zaporedje.split("L")}
    return samo_stevikle
...
class Carovnik:
    def __init__(self, storitev, cena):
        self.storitev = [storitev.split()[0], storitev.split()[-1], cena]
        self.denar = 0

    def caraj(self, s):
        for i in range(len(s)):
            if s[i] == self.storitev[0]:
                s[i] = self.storitev[1]
                self.denar += self.storitev[2]
        return s
