...
def navodila(zaporedje):
    ...
    for i in zaporedje:
        if i == "D":
            ...
            konec.append(i)
        elif konec[-1] == "D" or konec[-1] == "L":
            konec.append(i)
        else:
            konec[-1] += i
    for i in range(len(konec)):
        if konec[i] != "D" and konec[i] != "L":
            konec[i] = int(konec[i])
    return konec[1:]
...
class Carovnik:
    def __init__(self,abil,price):
        self.getting = abil.split(" -> ")[0]
        self.giving = abil.split(" -> ")[1]
        self.price = price
        self.money = 0
