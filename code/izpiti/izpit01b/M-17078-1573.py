...
def navodila(zaporedje):
    ...
    for i in range(len(zaporedje)):
        if zaporedje[i] == "L" or zaporedje[i] == "D":
            if prev != "L" and prev != "D" and prev != "":
                tab.append(int(niz))
                niz = ""
            tab.append(zaporedje[i])
            prev = zaporedje[i]
        else:
            if niz == "":
                niz = str(zaporedje[i])
...
def sprazni(s):
    tab = []
    status = True
    while (None in s or status) and len(s) > 0:
        for x in s:
            if x is not None:
                tab.append(sprazni(x))
            else:
                s.pop(s.index(x))
                status = False
    return tab #popravi
...
class Carovnik:
    ...
    def __init__(self, niz, racuna):
        self.prvo = niz.split("->")[0][:-1]
        self.drugo = niz.split("->")[1][1:]
        self.racuna = racuna
        self.denar = 0
    ...
    def caraj(self, s):
        ...
        for x in s:
            if x == self.prvo:
                tab.append(self.drugo)
                self.denar += self.racuna
            else:
                tab.append(x)
        return tab
