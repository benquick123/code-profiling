...
def zamenjaj(s,menjave):
    ...
    for x in s:
        if not x in menjave:
            ...
            nov.append(menjave[x])
    s[:] = nov
#print(zamenjaj([5, 3, 1, 1, 3, 2, 6],{3: 1, 1: 7}))
...
def najpogostejse_polje(navodila):
    ...
    slovar = {}
    for polje in polja:
        if not polje in slovar:
            slovar[polje] = 0
        slovar[polje] += 1
    #print(slovar)
    prvo_polje = najvec(slovar)
    del slovar[prvo_polje]
    drugo_polje = najvec(slovar)
    return prvo_polje

#print(najpogostejse_polje("D2DD2"))
...
def navodila(zaporedje):
    ...
    while i < len(zaporedje):
        ...
        if zaporedje[i] == "D":
            if stevilka:
                nov.append(int(stevilka))
            nov.append(zaporedje[i])
            stevilka = ""
        elif zaporedje[i] == "L":
            print(zaporedje[i])
            if stevilka:
            ...
            stevilka += str(zaporedje[i])
        i += 1
    if stevilka:
        nov.append(int(stevilka))
    return nov
print(navodila("123D41L1"))

class Carovnik:
    def __init__(self,stvar,cena):
        stvar = stvar.split(" ")
        self.stvar1 = stvar[0]
        self.stvar2 = stvar[2]
        self.cena = cena
    ...
    def caraj(self,s):
        ...
        for x in s:
            if x == self.stvar1:
                seznam.append(self.stvar2)
                self.zasluzek1 += self.cena
            else:
                seznam.append(x)
        return seznam
