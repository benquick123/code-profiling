def najpogostejse_polje(navodila):
    ...
    for x in seznam_polj:
        stevilo = seznam_polj.count(x)
        if stevilo > naj_polje_s:
            naj_polje_s = stevilo
            naj_polje = x
    return naj_polje
...
def zamenjano(s,menjave):
    output = []
    for x in s:
        output.append(menjave.get(x,x))
    return output


def zamenjaj(s,menjave):
    a = zamenjano(s, menjave)
    s.clear()
    for x in a:
        s.append(x)
...
def sprazni(s):
    ...
    for x in s:
        if x or x == []:
            seznam.append(sprazni(x))
    return seznam



class Carovnik:
    def __init__(self,snov,cena):
        razbitje = snov.split(" -> ")
        self.vhod = razbitje[0]
        self.izhod = razbitje[1]
        self.cena = cena
        self.denar = 0
