...
def najpogostejse_polje(navodila):
    ...
    for korak in navodila:
        if korak == "D":
            ...
            r.naprej(int(korak))
            polja.append(r.koordinate())
    maxPolj = 1
    celica = (0,0)
    print(polja)
    for polje in polja:
        if polja.count(polje) > maxPolj:
            maxPolj = polja.count(polje)
            celica = polje
    return celica

def navodila(zaporedje):
    koncnaNavodila = []
    for navodilo in range(0, len(zaporedje)):
        if zaporedje[navodilo] == "D" or zaporedje[navodilo] == "L":
            koncnaNavodila.append(zaporedje[navodilo])
        elif zaporedje(navodilo-1) == "D" or zaporedje(navodilo-1) == "L":
            for i in range(10):
                if zaporedje(navodilo+i) == "D" or zaporedje(navodilo+i) == "L":
                    koncnaNavodila.append(zaporedje[navodilo:navodilo+i])
                    break
    return koncnaNavodila

def zamenjano(s, menjave):
    seznam = s
    for element in range(len(seznam)):
        if seznam[element] in menjave:
            seznam[element] = menjave[seznam[element]]
    return seznam
...
def sprazni(s):
    ...
    for element in s:
        if element != None:
            koncniSeznam.append(sprazni(element))
    return koncniSeznam

class Carovnik:
    def __init__(self, pogoj, placa):
        self.kateriElement, self.spremeniElement = pogoj.split(" -> ")
        self.placa = placa
        self.denar = 0
    def caraj(self, s):
        for element in range(len(s)):
            if s[element] == self.kateriElement:
                s[element] = self.spremeniElement
                self.denar += self.placa
        return s
