...
def migracije(ime_datoteke):
    ...
    for i in f:
        ...
        # print(a)
        # print(a[0][0])
    return (sorted(seznam)[-1][1], sorted(seznam)[0][-1])
...
def roboti(navodila, n):
    ...
    rez=[]
    for i in range(n):
        robotiseznam.append(Robot())
    c = 0
    for navodilo in navodila:
        if navodilo == "J":
            while robotiseznam[c].smer != 2:
                robotiseznam[c].desno()

            robotiseznam[c].naprej(1)
            c += 1
        elif navodilo == "S":
            while robotiseznam[c].smer != 0:
                robotiseznam[c].desno()
            robotiseznam[c].naprej(1)
            c += 1
        elif navodilo == "V":
            while robotiseznam[c].smer != 1:
                robotiseznam[c].desno()
            robotiseznam[c].naprej(1)
            c += 1
        elif navodilo == "Z":
            while robotiseznam[c].smer != 3:
                robotiseznam[c].desno()
            robotiseznam[c].naprej(1)
            c += 1
        if c>len(robotiseznam)-1:
            c=0
    for r in robotiseznam:
        rez.append(r.koordinate())
    return rez
