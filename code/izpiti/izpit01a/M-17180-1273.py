...
def migracije(ime_datoteke):
    iz = collections.defaultdict(int)
    v = collections.defaultdict(int)
    izs = []
    vs = []
    for vrstica in open(ime_datoteke):
        vrstica = vrstica.strip()
        stevilo = vrstica.split(":")[0]
        kraja = vrstica.split(":")[1]
        izk = kraja.split("->")[0]
        izk = izk.strip()
        vk = kraja.split("->")[1]
        vk = vk.strip()
        iz[izk.strip()] += int(stevilo)
        v[vk.strip()] += int(stevilo)
    for kraj, st in iz.items():
        izs.append((st,kraj))
    for kraj, st in v.items():
        vs.append((st,kraj))


    return (sorted(izs)[-1][1], sorted(vs)[-1][1])
...
def zakladi(navodila):
    ...
    for nav in navodila:
        ...
        if nav == "L":
            ...
            c.naprej(int(nav))
            x, y = c.koordinate()[0],c.koordinate()[1]
            if (abs(x) + abs(y)) % 7 == 0:
                z += 1
    return z
