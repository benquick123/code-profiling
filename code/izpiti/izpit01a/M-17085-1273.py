...
class Delitelj:
    ...
    def akcija(self, s):
        nov_seznam=[]
        for element in s:
            if element%self.delitelj==0:
                kolicnik=element/self.delitelj
                nov_seznam.append(kolicnik)
                self.cena+=kolicnik
            else:
                nov_seznam.append(element)
#nisem bil prepričan zakaj mi ni dalo enakih vrednosti, in za popravo mi je zmanjkalo časa
...
def zakladi(navodila):
    ...
    vsota_zakladov=0
    niz=navodila.split()
    vsota_koordinat=abs(Robot.x)+abs(Robot.y)
    if vsota_koordinat%7==0:
        vsota_zakladov=vsota_koordinat+1
    navodilo=navodila.split()
...
def roboti(navodila, n):
    ...
    for stevec in range(1, n+1):
        seznam_robotov[stevec].append(x)
        seznam_robotov[stevec].append(y)
    stevec=0
    for element in niz:
        if stevec <=n:
            robot = seznam_robotov[stevec]
            if element =="J":
                robot[1]= robot[1]-1
            if element == "S":
                robot[1] = robot[1] + 1
...
def brez_ponavljanja(s):
    nov_seznam=[]
    dolzina=len(s)
    for stevec in range(0, dolzina+1):
        if s[stevec] != s[stevec+1]:
            nov_seznam.append(s[stevec])
        else:
            return brez_ponavljanja(nov_seznam)
    return nov_seznam
# če je vrednost naslednjega elementa različna od vrednosti trenutnega elementa potem jo dodamo v nov seznam, če
# pa je enaka se rekurzivno vrtimo dokler ni različna
