def migracije(ime_datoteke):
    ...
    for vrstica in datoteka:
        ...
        array.append((vrstica[0][0],vrstica[0][3:-1], vrstica[1][1:]))
    datoteka.close()
    preselilo={}
    odslo={}
    for stevilo, kje, kam in array:
        odslo.update({kje:0})
        preselilo.update({kam:0})
    for stevilo, kje, kam in array:
        odslo[kje]+=+int(stevilo)
        preselilo[kam]+=int(stevilo)
    najv=0
    for kljuc, vrednost in preselilo.items():
        if vrednost > najv:
            mesto1=kljuc
            najv=vrednost
    najv=0
    for kljuc, vrednost in odslo.items():
        if vrednost > najv:
            mesto2 = kljuc
            najv = vrednost
    return (mesto2, mesto1)
...
def zakladi(navodila):
    ...
    for x in navodila:
        if(str(x)=="L"):
            ...
            c.naprej(int(x))
        x,y=c.koordinate() #zakaj (6, -6)

        if(abs(x+y))%7==0:
            if (x,y) not in koordinate:
                koordinate.append((x,y))
    stevilo=0
    if(0,0) not in koordinate:
        koordinate.append((0,0))
...
def roboti(navodila, n):
    ...
    for navodilo in navodila:
        if(navodilo == "V"):
            robot[j][0] = robot[j][0] + 1
        if(navodilo == "Z"):
            robot[j][0] = robot[j][0] - 1
        if(navodilo == "S"):
            robot[j][1] = robot[j][1] + 1
        if(navodilo == "J"):
            robot[j][1]=robot[j][1]-1
        j=j+1
        if(j>=n):
            j=0
    return [(x,y) for x,y in robot.values()]

def brez_ponavljanja(s):
    return s if len(s) < 2 else s[:s[0] != s[1]] + brez_ponavljanja(s[1:])
...
class Delitelj:
    ...
    def akcija(self, s):
        ...
        for x in s:
            if(int(x)%self.k==0):
                self.deljiva.append(int(x/self.k))
                self.kolicnik.append(int(x/self.k))
            else:
                self.deljiva.append(x)
        return self.deljiva
    def cena(self):
        return sum(self.kolicnik)
