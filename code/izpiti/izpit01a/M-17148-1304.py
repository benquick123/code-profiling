def migracije(ime_datoteke):
    ...
    vse = datoteka.read().split("\n")
    selitve = {}
    for i in vse:
        stevilo = int(i.split(":")[0])
        ostanek = i.split(":")[1]
        mestoiz, mestov = ostanek.split(" -> ")[0].strip(), ostanek.split(" -> ")[1].strip()
        if not selitve[mestoiz]:
            selitve[mestoiz] = stevilo
        if not selitve[mestoiv]:
            selitve[mestov] = stevilo
        selitve[mestoiz] -= stevilo
        selitve[mestov] += stevilo
...
def zakladi(navodila):
    ...
    bil_tukaj = set()
    zakladi = 0
    if (not robot.koordinate() in bil_tukaj) and not robot.razdalja() % 7:
        zakladi += 1
        bil_tukaj.add(robot.koordinate())
...
def brez_ponavljanja(s):
    ...
    if len(s) < 2:
        return s[0]
    if not s[0] == s[1]:
        neki.append(brez_ponavljanja(s[1:]))
    else:
        neki.append(brez_ponavljanja(s[2:]))
    return neki


def roboti(navodila,n):
    robotki = {i: (0,0) for i in range(n)}
    for j, napotek in enumerate(navodila):
        x,y = 0,0
        if napotek == "J":
            y = -1
        elif napotek == "S":
            ...
            x = -1
        starax,staray = robotki[(j)%n]
        novax = starax + x
        novay = staray + y
        robotki[(j)%n] = (novax, novay)
    return [robotki[i] for i in robotki]
