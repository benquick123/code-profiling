...
def zakladi(navodila):
    t = 0
    for i in navodila:
        if (fabs(r.position[0] + r.position[1])) == 7: # Probably i didnt understand the task
            t += 1 #amout of treasures
        if navodila[i] == "":
            break
        if navodila[i] == "D":
            r.desno()
...
class Delitelj():
    ...
    def akcija(self, s):
        for i in s:
            smthwht = 0
            if s[i] % s == 0:
                self.price[i] = self.price[i] // s
                self.cena += self.price
