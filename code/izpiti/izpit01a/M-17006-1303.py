...
def zakladi(navodila):
    for el in navodila:
        if el == "L":
            ...
            bot.naprej(int(el))
        bot.polja.append((bot.x, bot.y))

    st = 0
    for el in set(bot.polja):
        if (abs(el[0]) + abs(el[1])) % 7 == 0:
            st += 1
...
def brez_ponavljanja(s):
    if not s:
        ...
        return s[:1:].append(brez_ponavljanja(s[1::]))
