    ...
    with open(ime_datoteke) as d:
        for vrstica in d:
            ...
            if len(v) > 4:
                v[3] = ' '.join(v[3:])
                del v[4]
            if v[1] in mesta.keys():
                mesta[v[1]] += int(v[0][0])
            else:
                mesta[v[1]] = int(v[0][0])
            if v[3] in mesta.keys():
                mesta[v[3]] -= int(v[0][0])
            else:
                mesta[v[3]] = -int(v[0][0])
    print(mesta) # minusi so koliko jih je prišlo v ta kraj, plusi pa koliko s ejih je odselilo
...
def zakladi(navodila):
    robot = Robot()
    seznam = list(navodila)
    zaklad = 0
    for navodilo in seznam:
        if robot.razdalja()%7 == 0:
            zaklad += 1
        if navodilo == "D":
            robot.desno()
        elif navodilo == "L":
    ...
    return bool(zaklad) # Če večkrat obišče polje z zakladom dobi zaklad le enkrat, nikjer ne piše isto da velja za isto polje
...
def roboti(navodila,n):
    ...
    for navodilo in navodila:
        premakni_tega = seznam[navodila.index(navodilo) % n]
        ukazi = "VJZS"
        premakni_tega.smer = ukazi.index(navodilo)+1
        premakni_tega.naprej(1)
    return [robot.koordinate() for robot in seznam]
