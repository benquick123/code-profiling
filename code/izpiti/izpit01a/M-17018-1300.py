...
def migracije(ime_datoteke):
    ...
    for line in file:
        ...
        num = int(line[:i])
        j = line.index("->")
        name_from = line[i+2:j-1]
        name_to = line[j+3:]

        if name_from in places_from.keys():
            places_from[name_from] += num
        else:
            places_from[name_from] = num

        if name_to in places_to.keys():
            places_to[name_to] += num
        else:
            places_to[name_to] = num
    ...
    for name, value in places_from.items():
        if value > max_from_value:
            max_from_value = value
            max_from_name = name

    max_to_value = 0
    max_to_name = ""

    for name, value in places_to.items():
        if value > max_to_value:
            max_to_value = value
            max_to_name = name

    return max_from_name, max_to_name
...
def zakladi(navodila):
    ...
    for n in navodila:
        if n.isnumeric():
            r.naprej(int(n))
            if r.razdalja() % 7 == 0:
                if r.koordinate() not in t_list:
                    t_list.append(r.koordinate())
        elif n == "D":
            r.desno()
        elif n == "L":
            r.levo()
