...
def migracije(ime_datoteke):
    ...
    for vrstica in t:
     ...
     g = int(d[0])
     f = d[1]
     rojstni = f.split("->")[0].strip()
     novi = f.split("->")[1].strip()
     seznam.append((g, rojstni))
     seznam2.append((g, novi))
    slovarrojstni = {}
    for stevilo, kraj in seznam:
        if kraj not in slovarrojstni:
         slovarrojstni[kraj] = [stevilo]
        else:
            slovarrojstni[kraj].append(stevilo)
    slovarnovi = {}
    for stevilo, kraj in seznam2:
        if kraj not in slovarnovi:
         slovarnovi[kraj] = [stevilo]
        else:
            slovarnovi[kraj].append(stevilo)
    for e in slovarrojstni:
        slovarrojstni[e] = sum(slovarrojstni[e])
    for e in slovarnovi:
        slovarnovi[e] = sum(slovarnovi[e])
    slovarR = {slovarrojstni[x]: x for x in slovarrojstni}
    slovarN = {slovarnovi[x]: x for x in slovarnovi}
    n = max(slovarR)
    m = max(slovarN)
    return(slovarR[n], slovarN[m])
    ...
    for e in navodila:
        if e == "D":
            ...
            if smer == "desno":
                ...
                y -= int(e)
            if (abs(x) + abs(y)) % 7 == 0 and (x,y) not in s:
                s.append((x,y))
                c += 1
