...
def migracije(ime_datoteke):
    ...
    for vrstica in datoteka:
        split1 = vrstica.split(':')
        st_ljudi = int(split1[0])
        split2 = split1[1].split('->')
        mesto_odhod = split2[0].strip()
        mesto_vhod = split2[1].strip()

        if mesto_vhod not in mesta_prislo:
            mesta_prislo[mesto_vhod] = 0

        if mesto_odhod not in mesta_odslo:
            mesta_odslo[mesto_odhod] = 0

        mesta_odslo[mesto_odhod] += st_ljudi

        mesta_prislo[mesto_vhod] += st_ljudi

    datoteka.close()

    max_odslo = None
    for i in mesta_odslo:
        if max_odslo == None or mesta_odslo[i] > mesta_odslo[max_odslo]:
            max_odslo = i

    max_prislo = None
    for i in mesta_prislo:
        if max_prislo == None or mesta_prislo[i] > mesta_prislo[max_prislo]:
            max_prislo = i
...
def zakladi(niz):
    ...
    if (abs(robot.x) + abs(robot.y)) % 7 == 0 and robot.koordinate() not in ze_obiskana_polja:
        st_zakladov += 1
        ze_obiskana_polja.append(robot.koordinate())
...
def roboti(navodila,n):
    robots = [Robot() for i in range(n)]

    for i,char in enumerate(navodila):
        robot = robots[i % n]
        if char == 'Z':
            robot.x -= 1
        elif char == 'J':
    ...
    return [ robot.koordinate() for robot in robots]
