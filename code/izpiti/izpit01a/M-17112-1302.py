    ...
    for znak in navodila:
        if znak == "D":
            ...
            try:
                znak = int(znak)
                pinko.naprej(znak)
                if (abs(pinko.x) + abs(pinko.y)) % 7 == 0 and pinko.koordinate() not in cekini:
                    cekini.append(pinko.koordinate())
            except ValueError:
                 pass
