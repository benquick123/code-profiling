def migracije(ime_datoteke):
    ...
    najvecje=999999
    for vrstice in file:
        vrstica= vrstice.split("->")

        vrstica2=vrstica[0].split(":")
        print(vrstica2)
        if int(vrstica2[0]) < najvecje:
            najvecje=int(vrstica2[0])
            par=(vrstica2[1],vrstica[1])
    return par
...
def zakladi(navodila):
    ...
    for neki in navodila:
        ...
        if neki == "D":
            ...
            robi.naprej(int(neki))
        x,y = robi.koordinate()
        if abs(x)+abs(y) % 7 == 0 and (x,y) not in sez_kor:
            st_zaklad+=1
            sez_kor.append((x,y))
    return st_zaklad
...
def roboti(navodila, n):
    ...
    i2=0
    while i < n:
        while i2 < len(navodila):
            if i2 <= len(navodila)-1:
                if  navodila[i2] == "S":
                    x,y=seznam[i]
                    if i2 <= len(navodila) - 1:
                        seznam[i]=(x,y+1)
...
class Delitelj:
    ...
    def akcija(self, s):
        ...
        for neki in s:
            if neki % self.k == 0:
                seznam2.append((neki/self.k))
                self.cena+=(neki/self.k)
            else:
                seznam2.append(neki)
        s[:]=seznam2
