...
def migracije(ime_datoteke):
   mig = defaultdict(int)
   ##!!! datoteka = open("Seznam.txt")
   datoteka = open(ime_datoteke)
   for vrstica in datoteka.read().splitlines():
       #print(vrstica)
       a = int(vrstica.split()[0][:-1])
       #print(a)
       b= vrstica.split("->")
      # print(b)
       krjaminus = b[0][len(str(a))+2:-1]
       krajplus = b[1][1:]
       #print(krjaminus)
       #print(krajplus)
       mig[krajplus] += a
   ...
   krajveliki= 0
   krajmali =0
   for krja, vrednost in mig.items():
       if vrednost < minn:
           minn = vrednost
           krajmali = krja
           print("kraj mali je {} in minn je {}".format(krajmali, minn))
       if vrednost > maxx:
           maxx = vrednost
           krajveliki = krja
           print("kraj veliki je {} in maxx je {}".format(krajveliki,maxx))



   return (krajmali, krajveliki)
    ...
    def zakladi(self, k):
        for znak in k:
            ...
            if znak == "L":
                ...
                self.naprej(float(znak))

        a,b =self.koordinate()
        if (abs(a)+ abs(b)) %7 ==0:
            self.zak.append(self.koordinate())
