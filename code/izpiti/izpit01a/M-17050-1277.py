...
def brez_ponavljanja(s):
    if len(s)==0:
        return s
    for st in s:
        if st in brez_ponavljanja(s[1:]):
            return [st]
        else:
            return [st] + brez_ponavljanja(s[1:])
...
def zakladi(navodila):
    ...
    for znak in navodila:
        ...
        if znak == "D":
            ...
            d.levo()
        elif znak!="D" and znak != "L":

            d.naprej(int(znak))
            vsota = abs(d.koordinate()[0]) + abs(d.koordinate()[1])

            preveri  = True
        if vsota %7==0 and preveri and d.koordinate()!=(0,0) and d.koordinate() not in seznam_ze_dobljenih:

            seznam_ze_dobljenih.append(d.koordinate())
...
class Delitelj:
    ...
    def akcija(self,s):
        ...
        for stevilo in s:
            if stevilo%self.k==0:
                self.cena+=stevilo/self.k
                nov.append(stevilo/self.k)
            else:
                nov.append(stevilo)
...
def migracije(ime_datoteke):
    ...
    for vrstica in datoteka:
        ...
        stevec=2
        od_kje_kam = vrstica.split("->")[0].split(":")[1][1:] #od kot so ljudi
        print(od_kje_kam,"odkjekam")

        kam_so_prisli = vrstica.split("->")[1][1:-1]  #kam so prisli
        print(kam_so_prisli,"kamsoprilsli")
        kot_kam = (od_kje_kam,kam_so_prisli)
        if st_ljudi>najvec:
            print(najvec,kraj_najvec,"najvec")
            najvec=st_ljudi
            kraj_najvec=kam_so_prisli
        if st_ljudi <= najmanj:
            print(najvec, kraj_najvec, "najmanj")
            najmanj = st_ljudi
            kraj_najmanj = od_kje_kam
        if kot_kam not in slovar.values():
            slovar[kot_kam]=st_ljudi
        else:
            slovar[kot_kam]+=st_ljudi
    ...
    return kraj_najvec,kraj_najmanj
