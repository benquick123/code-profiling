...
def roboti(navodila,n):
    a=robotisez(navodila,n)
    b=[]
    for x,y in a:
        b.append((x,y))         #če sem to delal v zgornji funkciji se mi je 2x sesul računalnik in mi je metalo ven da je return izven funkc.
    return b
...
def migracije(ime_datoteke):
    ...
    for vrstica in datoteka:
        ...
        if not razclenjena_vrstica[1] in ven:
            ven[razclenjena_vrstica[1]]= int(razclenjena_vrstica[0][:-1])
        else:
            ven[razclenjena_vrstica[1]] += int(razclenjena_vrstica[0][:-1])

        if not razclenjena_vrstica[3] in noter:
            noter[razclenjena_vrstica[3]]= int(razclenjena_vrstica[0][:-1])
        else:
            noter[razclenjena_vrstica[3]] += int(razclenjena_vrstica[0][:-1])

    #Funkcija dela to kar se zahteva od nje vendar pri krajih ki so sestavljeni iz 2 besed da samo prvo
    return (max(ven),max(noter))
