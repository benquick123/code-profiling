...
def migracije(ime_datoteke):
    ...
    slovar2 = collections.defaultdict(int)
    for vrstice in open(ime_datoteke).read().splitlines():
        cifra = vrstice.split(":")
        iz = cifra[1].split("-")
        v = iz[1].split(">")

        slovar[iz[0].strip()] += int(cifra[0])
        slovar2[v[1].strip()] += int(cifra[0])
    for x in slovar:
        if slovar[x] > naj1:
            naj1 = slovar[x]
            naj1Mesto = x
    ...
    return (naj1Mesto,naj2Mesto)

def roboti(navodila, n):
    x, y = 0, 0
    seznam = []

    for s in range(len(navodila)):
        if navodila[s] == "S":
            y +=1
        elif navodila[s] == "J":
            y -=1
...
def brez_ponavljanja(s):
    seznam = []
    for x in s:
        if x not in brez_ponavljanja(seznam):
            seznam.append(x)
    return seznam
...
class Delitelj:
    ...
    def akcija(self,seznam):
        for x in range(len(seznam)):
            if seznam[x]%self.k == 0:
                kolicnik = seznam[x]/self.k
                seznam[x] = kolicnik
                self.cena += kolicnik
