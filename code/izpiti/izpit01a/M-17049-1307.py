...
def migracije(ime_datoteke):
    ...
    for vrstica in open(ime_datoteke):
        s = vrstica.split(" ")
        if s[1] not in dictIz:
            dictIz[s[1]] = 0
        elif s[3] not in dictV:
            dictV[s[3]] = 0
        else:
            cifra, y = s[0]
            dictIz[s[1]] += cifra
    ...
    stev1 = 0
    mesto1 = ""
    for mesto, cifra in dictIz.items():
        if cifra > stev1:
            stev1 = cifra
            mesto1 = mesto
    ...
    stev2 = 0
    mesto2 = ""
    for mesto, cifra in dictV.items():
        if cifra > stev2:
            stev2 = cifra
            mesto2 = mesto
...
def zakladi(navodila):
    ...
    for i in navodila:
        ...
        if vsota % 7 == 0 or vsota == 0 and r.koordinate() not in s:
            zaklad += 1
            s.append(r.koordinate())
    return zaklad
