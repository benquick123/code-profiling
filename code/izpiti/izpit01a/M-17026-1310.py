...
# test javlja napako pri os.remove(fn) -- Timotej


#vem da je grozno napisana prva naloga :)
def migracije(ime_datoteke):
    datoteka=open(ime_datoteke)
    a_iz=defaultdict(int)
    a_kam=defaultdict(int)
    for vrstica in datoteka:
        s=vrstica.split(":")
        koliko=int(s[0])
        s=vrstica.split("->")
        iz = s[0].split()
        iz="".join(i for i in iz[1:])
        kam=s[1].strip()
        a_iz[iz]+=koliko
        a_kam[kam]+=koliko
    ma1=None
...
def zakladi(navodila):
    ...
    for el in navodila:
        if  el =="D":
            ...
            if (abs(koordinate[0])+abs(koordinate[1]))%7==0 and koordinate not in ze:
                ze.append(koordinate)
                vsota+=1
    return vsota
