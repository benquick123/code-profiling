def migracije(ime_datoteke):
    ...
    s1 = {}
    for vrstica in dat:
        for kraji in vrstica.split():
            s1[kraji[3]] = 0
        for st in vrstica.split(":"):
            s1[kraji] += int(st)

    for i in s1:
        return (max(s1.values()),max(s1.values()))
    ...
    if navodila == "":
        ...
        for n in navodila:
            if n == "D" or n == "L":
                ...
                if vsota % 7 == 0 and (r1_x, r1_y) not in obiskana_polja:
                    obiskana_polja.append((r1_x, r1_y))
                    st_zakladov += 1
