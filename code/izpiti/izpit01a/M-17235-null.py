...
def migracije(ime_datoteke):
    ...
    for vrstica in open(ime_datoteke):
        ...
        slovarIz[vrstica[1]]= 0
        slovarV[vrstica[3]] = 0


    for vrstica in open(ime_datoteke):
        vrstica = vrstica.split()
        cifra = int(vrstica[0][:-1])
        slovarIz[vrstica[1]]+= cifra
        slovarV[vrstica[3]] += cifra

    return((max(slovarIz),max(slovarV)))
