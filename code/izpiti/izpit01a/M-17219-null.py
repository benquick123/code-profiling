...
def migracije(ime_datoteke):
    ...
    for vrstica in dat:
        porezan = vrstica.split("->")
        preseljeni1 = porezan[0]
        preseljeni12 = vrstica.split(":")
        preseljeni = int(preseljeni1.split(":")[0])
        izKje = porezan[0]
        izKje1 = izKje.split(":")[1]
        izKje2 = izKje1[1:-1]
        doKam = porezan[1]
        doKam1 = doKam[1:-1]
        slov[izKje2] -= preseljeni
        slov[doKam1] += preseljeni
    maxK = ""
    maxL = 0

    minK =""
    minL = 0
    for kraj, ljudje in slov.items():
            if ljudje > maxL:
                maxL = ljudje
                maxK = kraj
            if ljudje < minL:
...
def zakladi(navodila):
    ...
    for ukaz in navodila:
        ...
        if ukaz == "D":
            ...
            rob.naprej(int(ukaz))
        vsota = abs(rob.koordinate()[1]) + abs(rob.koordinate()[0])
...
class Delitelj:
    ...
    def akcija(self, s):
        ...
        for x in s:
            if x % self.stevilo == 0:
                sez.append(x//self.stevilo)
                self.cena += x//self.stevilo
            else:
                sez.append(x)
        s[:] = sez
