...
def migracije(ime_datoteke):
    ...
    for vrstica in open(ime_datoteke, "r"):
        var = vrstica.split() #ne deluje saj splita po presledkih kraj ima lahko tudi presledek
        try:
            od_kod[var[1]] += int(var[0][:-1])
            kam[var[3]] += int(var[0][:-1])
        except:
            od_kod[var[1]] = int(var[0][:-1])
            kam[var[3]] = int(var[0][:-1])
    od_kod_max = ("", 0)
    od_kje_max = ("", 0) #kam
    for kraj, koliko in od_kod.items():
        if koliko > od_kod_max[1]:
            od_kod_max = kraj, koliko
    for kraj, koliko in kam.items():
        if koliko > od_kje_max[1]:
            od_kje_max = kraj, koliko
    return od_kod_max[0], od_kje_max[0]
...
def zakladi(navodila):
    ...
    for ukaz in navodila:
        ...
        x, y = robot.koordinate()
        if (abs(x)+abs(y)) % 7 == 0 and robot.koordinate() not in obiskana:
            zaklad += 1
            obiskana.append((x, y))
    return zaklad
...
class Delitelj:
    ...
    def akcija(self, s):
        ...
        for element in s:
            if element % self.k == 0:
                nov.append(element/self.k)
                self.cena += element / self.k
            else:
                nov.append(element)
        s[:] = nov
