def migracije(ime_datoteke):
    for i, vrstic in enumerate(open(ime_datoteke)):
        for a in vrstic.split(":")[i]:
            for b in vrstic.split(":")[i + 1]:
                if a.split("->")[1] == b.split("->")[0]:
                    if b.split("->")[1] == a.split("->")[0]:
                        return a.split("->")[1]
                else:
                    return b.split("->")[1]
...
def roboti(navodila, n):
    ...
    for i in range(n):
        a.append(Robot())
    for j in a[counter]:
        j.x, j.y = j.koordinate()[0], j.koordinate()[1]
        for k in navodila:
            if k == "J":
                j.y -= 1
            if k == "Z":
            ...
            if k == "V":
                j.x += 1
        counter += 1
        if counter == range(len(n)):
            counter = 0
        roboti.append(j.koordinate())
    return roboti
