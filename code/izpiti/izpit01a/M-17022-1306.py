...
def migracije(ime_datoteke):
    ...
    for vrstica in open(ime_datoteke):
        b = [vrstica.strip().split(":")[0]]
        b.extend(vrstica.strip().split(" -> "))
        b[1] = b[1].split(": ")[1]
        odseljeni[b[1]] = 0
        priseljeni[b[2]] = 0
        print(odseljeni)
    ...
    for vrstica in open(ime_datoteke):
        ...
        print(b)
        if b[1] in odseljeni:
            odseljeni[b[1]] += int(b[0])
        if b[2] in priseljeni:
            priseljeni[b[2]] += int(b[0])
    ...
    for kljuc, vrednost in odseljeni.items():
        if int(vrednost) > naj_odseljenost:
            naj_odseljenost = kljuc
            naj_odseljeni = vrednost

    for kljuc, vrednost in priseljeni.items():
        if vrednost > naj_priseljenost:
            naj_priseljenost = vrednost
            naj_priseljeni = kljuc

    return(naj_odseljeni, naj_priseljeni)

def zakladi(navodila):
    robot = Robot()
    koordinate_robota = [(0, 0)]
    for navodilo in navodila:
        try:
            nav = int(navodilo)
            robot.naprej(nav)
            koordinate_robota.append(robot.koordinate())
        except:
            if navodilo == "D":
                robot.desno()
            elif navodilo == "L":
    ...
    koordinate_zakladov = [i for i in koordinate_robota if (abs(i[0]) + abs(i[1])) % 7 == 0 or i == (0, 0)]
    unikati_zakladov = [x for i, x in enumerate(koordinate_zakladov) if x not in koordinate_zakladov[:i] and x not in koordinate_zakladov[i+1:]]
    print(unikati_zakladov)
    return len(unikati_zakladov)


def roboti(navodila, n):
    koordinate = [() for i in range(n)]
    for i, navodilo in enumerate(navodila):
        pass

def brez_ponavljanja(s):
    for i,j in enumerate(s):
        if j not in s[i + 1:i +3]:
            del(s[i])
            brez_ponavljanja(s)
    return s
