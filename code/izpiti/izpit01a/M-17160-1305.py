...
def migracije(ime_datoteke):
    ...
    mesta = defaultdict(int)
    tab = []
    for vrstica in datoteka:
        tab = vrstica.split()
        if len(tab) > 4:
            tab[3] = tab[3] + tab[4]
        mesta[tab[1]] -= int(tab[0][:1])
        mesta[tab[3]] += int(tab[0][:1])
...
def zakladi(navodila):
    ...
    for ukaz in navodila:
        if ukaz == 'D':
            ...
            r.naprej(ukaz)
            a = r.koordinate()
        if (abs(a[1])+abs(a[0])) % 7 == 0:
            kordinate1.add(r.koordinate())
            if not r.koordinate() in kordinate1:
                zakladov +=1
    print(r.koordinate())
    return zakladov
...
def roboti(navodila,n):
    ...
    for b in navodila:
        if stevec == n:
            stevec = 0
        if b == 'V':
            a[stevec] = (a[stevec][0] + 1, a[stevec][1])
        if b == 'Z':
            a[stevec] = (a[stevec][0] - 1, a[stevec][1])
        if b == 'S':
            a[stevec] = (a[stevec][0], a[stevec][1] + 1)
        if b == 'J':
            a[stevec] = (a[stevec][0], a[stevec][1] - 1)
        stevec +=1
    return a
