...
def migracije(ime_datoteke):
    mesta_iz=collections.defaultdict(int)
    mesta_v=collections.defaultdict(int)
    datoteka=open(ime_datoteke)
    for vrstica in datoteka:
        delna_vrstica=vrstica.split(':')
        delna_vrstica[1].split('->')
        mesta_iz[delna_vrstica[1][0]]+=int(delna_vrstica[0])
        mesta_v[delna_vrstica[1][1]] += int(delna_vrstica[0])
    iz_mig=0
    iz_mig_m=""
    v_mig=0
    v_mig_m=""
    for ime,mig in mesta_iz.items():
        if iz_mig<mig:
            iz_mig=mig
            iz_mig_m=ime
    for ime, mig in mesta_v.items():
        if v_mig < mig:
            v_mig = mig
            v_mig_m = ime
    return  iz_mig_m,v_mig_m
...
def roboti(navodila,n):
    ...
    for ukaz in navodila:
        if trenutni_rob>=len(slovar_botov):
            trenutni_rob=0
            slovar_botov[trenutni_rob].append(premik(ukaz))#ja z clllasm bi mogu nardit
...
class Robot:
    ...
    def zakladi(self,navodila):
        ...
        for ukaz in navodila:
           ...
           if self.razdalja() % 7 == 0:
               zakladi.add((self.koordinate()))
