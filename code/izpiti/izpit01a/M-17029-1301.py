...
def migracije(dat):
    ...
    with open(dat) as f:
        for line in f.readlines():
            ...
            sl[l[0].split()[1]] -= int(l[0].split()[0][:-1])
            sl[l[1]] += int(l[0].split()[0][:-1])
    ...
    for k, v in sl.items():
        if v < najmal:
            najmal = v
            najmalime = k
    #vem kaj e narobe v isto spremenlivko shranujem kok je odslo in kok je prislo nazalost ni casa popravit
    return najmalime, najime
        ...
        for n in navodila:
            if isinstance(n, int):
                ...
                if n == "D":
                    ...
                    self.levo()
            if (self.koordinate()[0] ** 2 + self.koordinate()[1] ** 2) % 7 == 0:
                self.z.append(self.koordinate())
        return len(self.z)


def roboti(navodila, n):
    rob = [[0, 0] for i in range(n)]
    ind = 0
    i = 0
    while ind < len(navodila):
        if navodila[ind] == "J":
            rob[i][1] -= 1
        elif navodila[ind] == "S":
            rob[i][1] += 1
