...
def migracije(ime_datoteke):
    ...
    priselitev = {}
    for element in a:
        seznam.append(element.strip())
    for element in seznam:
        a = element.split(": ")
        for ele in a:
            if "->" in ele:
                b = ele.split(" -> ")
                if b[0] not in odselitev:
                    odselitev[b[0]] = int(a[0])
                else:
                    odselitev[b[0]] += int(a[0])
                if b[1] not in priselitev:
                    priselitev[b[1]] = int(a[0])
                else:
                    priselitev[b[1]] += int(a[0])
    count1 = 0
    count2 = 0
    max_odselitev = None
    max_priselitev = None
    for element in odselitev:
        if odselitev[element] > count1:
            max_odselitev = element
            count1 = odselitev[element]
    for element in priselitev:
        if priselitev[element] > count2:
            max_priselitev = element
            count2 = priselitev[element]
    return (max_odselitev, max_priselitev)
...
def zakladi(navodila):
    robot = Robot()
    obiskani_zakladi = []
    trenutni_x, trenutni_y = robot.koordinate()
    if (abs(trenutni_x) + abs(trenutni_y)) % 7 == 0:
        obiskani_zakladi.append(robot.koordinate())
    for navodilo in navodila:
        if navodilo == "D":
            ...
            if robot.koordinate() not in obiskani_zakladi:
                trenutni_x, trenutni_y = robot.koordinate()
                if (abs(trenutni_x) + abs(trenutni_y)) % 7 == 0:
                    obiskani_zakladi.append(robot.koordinate())
    return len(obiskani_zakladi)


def roboti(navodila, n):
    roboti = []
    for i in range(1, n + 1):
        roboti.append("robot{}".format(i))
    print(roboti)

roboti("SJVZSJVZSJVZ", 4)


def brez_ponavljanja(s):
    if s[0] not in s[1:]:
        s = brez_ponavljanja(s[1:])
    else:
        s[1:].remove(s[0])
        s = brez_ponavljanja(s[1:])
    return s
