...
def razdalja(ime1, ime2, kraji):
    ...
    tocnarazdalja = razdalja_koordinat(x1, y1, x2, y2)
    return tocnarazdalja

def v_dometu(ime, domet, kraji):
    seznam = []
    for kraj, x1, y1 in kraji:
        if ime != kraj and razdalja(ime, kraj, kraji) <= domet:
            seznam.append(kraj)
    return seznam
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    for imekraja in imena:
        razdaljax = razdalja(ime, imekraja, kraji)
        if razdaljax > maxrazdalja:
            maxrazdalja = razdaljax
            kraj = imekraja
    return kraj

def zalijemo(ime, domet, kraji):
    for kraj, x1, y1 in kraji:
       najkraj = najbolj_oddaljeni(ime, v_dometu(ime, domet, kraji), kraji)
    return najkraj
