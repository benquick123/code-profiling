...
def v_dometu(ime, domet, kraji):
    ...
    for kraj, x2, y2 in kraji:
        if kraj == ime:
            pass
        elif razdalja_koordinat(x1, y1, x2, y2) <= domet:
            sez_krajev.append(kraj)
    return sez_krajev
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    for ime1, x2, y2 in kraji:
        if ime1 in imena:
            if razdalja(ime, ime1, kraji) > maxi_razdalja:
                maxi = ime1
    return maxi
...
def zalijemo(ime, domet, kraji):
    ...
    maxi = 0
    for kraj in kraji:
        func = razdalja(ime, kraj[0], kraji)
        if func < domet:
            if func > maxi:
                maxi = func
                maxi_kraj = kraj[0]
