...
def v_dometu(ime, domet, kraji):
    seznam = []
    for kraj in kraji:
        if ime is not kraj[0]:
            if razdalja(ime, kraj[0], kraji) <= domet:
...
def zalijemo(ime, domet, kraji):
    sez = v_dometu(ime, domet, kraji)
    return najbolj_oddaljeni(ime, sez, kraji)
...
for prva, druga, tretja in kraji:
    dist = sqrt((druga - sirina1) ** 2 + (tretja - dolzina1) ** 2)
    top = domet - dist
    if top > 0:
        kraji1.append(prva)

for prva, druga, tretja in kraji:
    dist = sqrt((druga - sirina2) ** 2 + (tretja - dolzina2) ** 2)
    top = domet - dist
    if top > 0:
        kraji2.append(prva)
