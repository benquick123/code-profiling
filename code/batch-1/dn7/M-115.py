...
"""Daniil Popov 23.11"""
"""Minolovec"""
"""pylint main.py: Your code has been rated at -6.99/10"""
...
def sosedov(x, y, mine):
	return len([(x + x_temp, y + y_temp) for x_temp in range(-1, 2)
										 for y_temp in range(-1, 2)
										 if ((x + x_temp, y + y_temp) in mine) and (x_temp != 0 or y_temp != 0)])

def najvec_sosedov(mine, s, v):
	return collections.OrderedDict(sorted(({sosedov(x_temp, y_temp, mine): (x_temp, y_temp)
									for x_temp in range(0, s)
									for y_temp in range(0, v)}).items())).popitem()[1]


def brez_sosedov(mine, s, v):
	return set((x_temp, y_temp) for x_temp in range(0, s)
								for y_temp in range(0, v)
								if sosedov(x_temp, y_temp, mine) == 0)

def po_sosedih(mine, s, v):
	return {i: {(x_temp, y_temp)for x_temp in range(0, s)
								for y_temp in range(0, v)
								if sosedov(x_temp, y_temp, mine) == i}
								for i in range(0, 9)}
...
def varen_premik(x0, y0, x1, y1, mine):
	return all((x_temp, y_temp) not in mine for x_temp in range(min(x0, x1), max(x0, x1) + 1) for y_temp in range(min(y0, y1), max(y0, y1) + 1))

def varna_pot(pot, mine):
	return (pot[0][0], pot[0][1]) not in mine if len(pot) == 1 else all(varen_premik(x0, y0, x1, y1, mine)
			for (x0, y0), (x1, y1) in zip(pot, pot[1:]))
	...
	for str in polje.split():
		for c in range(len(str)):
			if str[c] == 'X':
				d.add((c, i))
		i += 1
	return (d, len(str), len(polje.split()))
