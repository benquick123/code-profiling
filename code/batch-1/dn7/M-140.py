...
def sosedov(x, y, mine):
    ...
    for coords in mine:
        x1 = (x - coords[0])
        y1 = (y - coords[1])
        if x1 >= -1 and x1 <= 1 and y1 >= -1 and y1 <= 1 and coords != (x, y):
            count += 1
    return count
...
def najvec_sosedov(mine, s, v):
    ...
    for polje in vsa_polja(s, v):
        sosedi = sosedov(polje[0], polje[1], mine)
...
def po_sosedih(mine, s, v):
    ...
    for key in result:
        for polje in vsa_polja(s, v):
            if sosedov(polje[0], polje[1], mine) == key:
                result[key].add(polje)
    return result
...
def dolzina_poti(pot):
    ...
    count = 0
    for coords in pot:
        if coords[1] != previous_y:
            count += abs(coords[1] - previous_y)
        previous_y = coords[1]
    for coords in pot:
        if coords[0] != previous_x:
            count += abs(coords[0] - previous_x)
        previous_x = coords[0]
    return abs(count)
