...
def koordinate(ime, kraji):                      #koordinate kraja
    for kraj,x,y in kraji:
        if ime==kraj:
            return x,y

def razdalja_koordinat(x1,y1,x2,y2):             #razdalja med tockama
    return sqrt((x1-x2)**2 + (y1-y2)**2)

def razdalja(ime1,ime2,kraji):                   #razdalja med krajema
    x1,y1 = koordinate(ime1,kraji)
    x2,y2 = koordinate(ime2,kraji)
    return razdalja_koordinat(x1,y1,x2,y2)

def v_dometu(ime, domet, kraji):                 #seznam krajev v dometu
    zaliti_kraji=[]
    x1, y1 = koordinate(ime, kraji)
    for kraj,x,y in kraji:
        if domet >= sqrt((x1-x)**2 + (y1-y)**2)>0:
            zaliti_kraji.append(kraj)
    return zaliti_kraji

print(v_dometu("Domžale",42,kraji))

def najbolj_oddaljeni(ime,imena,kraji):          #najbolj oddaljen kraj v seznamu
    naj_razdalja=0
    naj_kraj=""
    for kraj1 in imena:
        for kraj2,x,y in kraji:
            if kraj1 == kraj2:
                x= razdalja(kraj1,ime,kraji)
                if x > naj_razdalja:
                    naj_razdalja=x
                    naj_kraj= kraj1
    return naj_kraj
...
def zalijemo(ime,domet,kraji):                   #najbolj oddaljen kraj v dometu
    kraj = najbolj_oddaljeni(ime,v_dometu(ime,domet,kraji),kraji)
    return kraj
print(zalijemo("Ljubljana",30,kraji))

#dodatni del, zakaj pa ne

def presek(s1,s2):                               #presek 2 množic
    s3=[]
    for x in s1:
        for y in s2:
            if x ==y:
...
def skupno_zalivanje(ime1,ime2,domet,kraji):     #presek dometov 2 krajev
    kraji1 = v_dometu(ime1,domet,kraji)
    kraji2 = v_dometu(ime2,domet,kraji)
    skupno = presek(kraji1,kraji2)
    return skupno
print(skupno_zalivanje("Ljubljana","Domžale",30,kraji))
