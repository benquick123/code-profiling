...
#dobis razdaljo med mesti
def razdalja_koordinat (x1,y1,x2,y2):
    d = abs(sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2))
    return d

#dobis koordinate enega mesta
...
#razdaljo med mesti ki si jih vneses
...
#izračuna vse kraje znotraj dometa
...
def zalijemo(ime,domet,kraji):
    ...
    for kraj, x, y in kraji:
        d = sqrt((x - izbrani_x) ** 2 + (y - izbrani_y) ** 2)
        if d <= domet and d != 0 and d > max_d:
            max_d = d
            naj_ime = kraj
    return naj_ime
