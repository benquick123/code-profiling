...
def find(sez_krajov):
    for ime, x, y in sez_krajov:
        if ime == kraj:
            print(f'{ime} je na koordinatah {x} {y}')
            x2, y2 = x, y
            break
    maxi = 0
    maxi_ime = ""
    for ime, x, y in sez_krajov:
        the_dist = distance(x, y, x2, y2)
        if the_dist > maxi and (the_dist < domet):
            maxi = the_dist
            maxi_ime = ime
    print(f'Iz kraja {kraj} lahko zalijemo kraj {maxi_ime} na razdalji {maxi}')
    return maxi, maxi_ime
...
def find_inrange(place1, place2, places, dometnik):
    d = {}
    place1_list = []
    place2_list = []
    for ime, x, y in places:
        if ime == place1:
            d[place1] = [x, y]
        if ime == place2:
            d[place2] = [x, y]
    for ime, x, y in places:
        if distance(x, y, d[place1][0], d[place1][1]) < dometnik:
            place1_list.append(ime)
        if distance(x, y, d[place2][0], d[place2][1]) < dometnik:
            place2_list.append(ime)
    for i in place1_list:
        if i in place2_list:
            print(i)


find_inrange(kraj1, kraj2, kraji, domet)
