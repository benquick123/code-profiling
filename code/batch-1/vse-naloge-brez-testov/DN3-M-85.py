...
izb = int(input("Obvezna naloga (1)\nDodatna naloga (2)\nVnesi: "))

if izb == 1:
    kraj = "Ljubljana"
    domet = 30
    for ime_kraj, x, y in kraji:
        if ime_kraj == kraj:
            xk = x
            yk = y
            break
    ...
    max_razdalja = 0
    for ime_kraj, x, y in kraji:
        razdalja = sqrt(pow(x - xk, 2) + pow(y - yk, 2))
        if (razdalja - domet <= 0) & (razdalja > max_razdalja):
            max_razdalja = razdalja
            kraj2 = ime_kraj
    print("Iz kraja ", kraj, " lahko zalijemo kraj ", kraj2, " na razdalji ", max_razdalja)
elif izb == 2:
    kraj1 = "Ljubljana"
    kraj2 = "Bled"
    domet = 30
    ...
    for ime_kraja, x, y in kraji:
        ...
        if ime_kraja == kraj2:
            ...
            yk2 = y
    for ime_kraja, x, y in kraji:
        razdalja1 = sqrt(pow(x - xk1, 2) + pow(y - yk1, 2))
        razdalja2 = sqrt(pow(x - xk2, 2) + pow(y - yk2, 2))
        if (razdalja1 - domet <= 0) and (razdalja2 - domet <= 0):
            tarce.append(ime_kraja)
    for ime_kraja in tarce:
        print(ime_kraja)
