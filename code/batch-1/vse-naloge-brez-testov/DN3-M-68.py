...
kraj = "Ljubljana"                        #določimo kraj
domet = 30                                #določimo max domet vodnega topa

#obvezna naloga
'''
krajX = krajY = razdalja = naj_razdalja= 0                                          #potrebne spremenljivke za primerjavo, izračun
naj_kraj=""
for mesto,X,Y in kraji:                                                             #v tej zanki dobimo koordinate našega kraja
    if mesto == kraj:
        krajX=X
        krajY=Y

for mesto,X,Y in kraji:
    razdalja = sqrt((X-krajX)**2+(Y-krajY)**2)                                      #izračun razdalje med krajema
    if razdalja <= domet:                                                           #primerjava če je ta kraj v dometu vodnega topa
        if razdalja > naj_razdalja:                                                 #primerjava če je na daljši razdalji kot prejšnji kraj,
            naj_razdalja=razdalja                                                   #če je potem shrani razdaljo do njega
            naj_kraj=mesto                                                          #in ime tega kraja



print(kraj,"je na koordinatah",krajX,krajY)
print("Iz kraja",kraj,"lahko zalijemo kraj",naj_kraj,"na razdalji",naj_razdalja)
'''
...
kraj1X = kraj1Y = kraj2X = kraj2Y = razdalja1 = razdalja2 = naj_razdalja= 0                             #potrebne spremenljivke za primerjavo, izračun
naj_kraj=""

for mesto, X, Y in kraji:                                                                               #v tem foru dobimo koordinate podanih krajev
    if mesto == kraj1:
        kraj1X = X
        kraj1Y = Y
...
for mesto,X,Y in kraji:
    razdalja1 = sqrt((X-kraj1X)**2+(Y-kraj1Y)**2)                                                       #izračun razdalje od prvega kraja
    razdalja2 = sqrt((X - kraj2X) ** 2 + (Y - kraj2Y) ** 2)                                             #izračun razdalje od drugega kraja
    if razdalja1 <= domet and razdalja2 <= domet:
        print(mesto)                                                                                    #če je v dometu obeh ga izpišemo