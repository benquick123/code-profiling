...
for ime, x, y in kraji:
    razdalja = sqrt((x_kraj - x) ** 2 + (y_kraj - y) ** 2)
    if razdalja > največja_razdalja and razdalja < domet:
        največja_razdalja = razdalja
        naj_kraj = ime
print("Iz kraja", kraj, "lahko zalijemo kraj", naj_kraj, "na razdalji", največja_razdalja)
...
največja_razdalja = 0
for ime, x, y in kraji:
    razdalja1 = sqrt((x_kraj1 - x) ** 2 + (y_kraj1 - y) ** 2)
    razdalja2 = sqrt((x_kraj2 - x) ** 2 + (y_kraj2 - y) ** 2)
    if razdalja1 < domet2 and razdalja2 < domet2:
        seznam.append(ime)

if not seznam:
    print(kraj1, "in", kraj2, "imata premajhnega")
else:
    print(kraj1, "in", kraj2, "lahko z dometom", domet2, "km skupaj zalijeta naslednje kraje:", ', '.join(seznam))
