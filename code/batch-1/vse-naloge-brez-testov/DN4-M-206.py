...
def koordinate(ime, kraji):
    #ta funkcija je za lažjo preglednost kasnejših funkcij
    for kraj in kraji:
        ime_kraja, sirina, visina = kraj
        if ime == ime_kraja:
...
def razdalja(ime1, ime2, kraji):
    #koordinate prvega kraja
    koord_1 = koordinate(ime1, kraji)
    x1, y1 = koord_1

    #koordinate drugega kraja
    koord_2 = koordinate(ime2, kraji)
    x2, y2 = koord_2

    #izračun po pitagorovem izreku
    return razdalja_koordinat(x1, y1, x2, y2)

#testni klic
print(razdalja("Mislinja", "Muta", kraji))


def v_dometu(ime, domet, kraji):

    #array v katerega se bojo appendali podatki
    seznam_krajev = []

    for ime_kraja, visina, sirina in kraji:
        #če je razdalja med krajema manjša kot domet topa
        if (razdalja(ime, ime_kraja, kraji) <= domet):
            #da ne zaliva samega sebe
            if (ime_kraja != ime):
                seznam_krajev.append(ime_kraja)
    return seznam_krajev

#testni klic
print(v_dometu("Mislinja", 16, kraji))
...
def najbolj_oddaljeni(ime, imena, kraji):
    #podatki o najbolj oddaljenem kraju
    max = 0
    najbolj_oddaljen_kraj = ""

    #poišče najbolj oddaljen kraj
    for kraj_v_seznamu in imena:

        razdalja_med_krajema = razdalja(ime, kraj_v_seznamu, kraji)

        if razdalja_med_krajema > max:
            max = razdalja_med_krajema
            najbolj_oddaljen_kraj = kraj_v_seznamu

    return najbolj_oddaljen_kraj

#testni klic
print(najbolj_oddaljeni("Ljubljana", imena, kraji))

#testni klic
print(v_dometu("Mislinja", 16, kraji))


#PROBLEMATIČNA FUNKCIJA
def zalijemo(ime, domet, kraji):
    seznam_krajev = v_dometu(ime, domet, kraji)
    #return seznam_krajev
    return najbolj_oddaljeni(ime, seznam_krajev, kraji)

#testni klic
print(zalijemo("Mislinja", 20, kraji))

def presek(seznam1, seznam2):
    skupni_elementi = []
    for element1 in seznam1:
        for element2 in seznam2:
            if element2 == element1:
                skupni_elementi.append(element2)
    return skupni_elementi

print(presek(["Domžale", "Kranj", "Maribor", "Vrhnika"], ["Domžale", "Maribor", "Vrhnika"]))
