...
skupno = [] #prazna lista za skupno zalivanje


kraj1 = input("Izberite prvi kraj:")
kraj2 = input("Izberite drugi kraj:")
domet = int(input("Izberite domet:")) #pridobimo kraje in domet
najvecji_domet = 0
flag = 0

for x,y,z in kraji:                          #nisem znal združiti obeh for stavkov tako, da bi prvo izpisalo kraj, zato sem uporabil dva
    if x == kraj1:
        y1 = y
        z1 = z
    if x == kraj2:
...
for x,y,z in kraji:
    if (sqrt((y1-y)**2 + (z1-z)**2) <= domet) and (x != kraj1):
        flag = 1
        if sqrt((y1-y)**2 + (z1-z)**2) > najvecji_domet:
           najvecji_domet = sqrt((y1-y)**2 + (z1-z)**2)
           najdljesi_kraj = x
    if (sqrt((y2-y)**2 + (z2-z)**2) <= domet) and (x != kraj2) and flag == 1:
        skupno.append(x)
    flag = 0

print(kraj1, "leži na koordinatah", y1, z2)
print("Iz", kraj1, "lahko zalijemo", najdljesi_kraj, "na razdalji", najvecji_domet)
print("Kraja lahko skupaj zalivata", skupno)
