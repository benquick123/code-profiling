...
for val, x, y in kraji:
    if val == kraj:
        x1 = x
        y1 = y
        print(val, "je na koordinatah", x, y)

for val, x, y in kraji:
    if val != kraj:
        AB = sqrt((x1 - x) ** 2 + (y1 - y) ** 2)
        if AB <= domet and AB > topAB:
            topAB = AB
            topOddaljenKraj = val

print("Iz kraja", kraj, "lahko zalijemo kraj", topOddaljenKraj, "na razdalji", topAB)
