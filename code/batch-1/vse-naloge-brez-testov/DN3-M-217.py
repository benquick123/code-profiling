...
a = int(input("Zaženem obvezni ali dodaten del naloge?(0-obvezni, 1-dodatni): "))

if (a == 0):#obvezna naloga
    domet = float(input("Vnesi domet: "))
    iz_kraj = input("Vnesi kraj: ")

    for k in kraji:
        if (k[0] == iz_kraj):
            print("Kraj",k[0],"leži na koordinatah",k[1],"in",k[2])

    for k in kraji:
        for kr in kraji:
            razdalja = sqrt((kr[1] - k[1])**2 + (kr[2] - k[2])**2)
            if (razdalja <= domet):
                if (iz_kraj == k[0] and kr[0] != iz_kraj):
                    print("Iz kraja",k[0],"lahko zalivamo kraj",kr[0],"na razdalji",razdalja)
elif (a == 1):#dodatna naloga
    domet = float(input("Vnesi domet: "))
    iz_kraj = input("Vnesi prvi kraj: ")
    ...
    for k in kraji:
        razdalja = sqrt((k[1] - prvi[1]) ** 2 + (k[2] - prvi[2]) ** 2)
        razdalja_drugi = sqrt((k[1] - drugi[1]) ** 2 + (k[2] - drugi[2]) ** 2)
        if (razdalja <= domet):
            if (razdalja_drugi <= domet):
                print ("Skupaj lahko zalivata kraj",k[0])
                st += 1
    if(st == 0):
        print("Skupaj ne morata zalivati nobenega kraja.")
else:
    print("Napačen vnos.")