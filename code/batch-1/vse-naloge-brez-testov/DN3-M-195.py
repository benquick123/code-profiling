...
coords = [(i[1], i[2]) for i in kraji if i[0] == kraj][0]
longestDistance = 0
farthestPossibleTarget = None
for i in kraji:
    distance = sqrt((i[1] - coords[0])**2 + (i[2] - coords[1])**2)
    if longestDistance < distance <= domet:
        longestDistance = distance
        farthestPossibleTarget = i[0]
print(kraj, "je na koordinatah", coords[0], coords[1])
print("Iz kraja", kraj, "lahko zalijemo kraj", farthestPossibleTarget, "na razdalji", longestDistance)
print("----------------")
...
coords = [(i[1], i[2]) for i in kraji if i[0] in (kraj1,kraj2)]
for i in kraji:
    distance1 = sqrt((i[1] - coords[0][0])**2 + (i[2] - coords[0][1])**2)
    distance2 = sqrt((i[1] - coords[1][0])**2 + (i[2] - coords[1][1])**2)
    if distance1 <= domet and distance2 <= domet:
        print(i[0])
print("----------------")
...
locationTargets = {}
locationStepCount = {}
start, target = [i for i in kraji if i[0] in (kraj1,kraj2)]
...
for i in routes:
    ...
    if i[-1] not in locationTargets:
        locationTargets[i[-1]] = []
        for j in kraji:
            distance = sqrt((i[-1][1] - j[1])**2 + (i[-1][2] - j[2])**2)
            if distance <= domet:
                locationTargets[i[-1]].append(j)

    if target in locationTargets[i[-1]]:
        i.append(target)
        if shortestRoute == None or len(i) <= len(shortestRoute):
            shortestRoute = i
        continue

    for j in locationTargets[i[-1]]: 
        if j not in i and (j not in locationStepCount or locationStepCount[j] > len(i) + 1):
            route = i.copy() 
            route.append(j) 
            routes.append(route) 
            locationStepCount[j] = len(route)
