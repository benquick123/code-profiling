...
def v_dometu(ime, domet, kraji):
	koord = koordinate(ime, kraji)
	seznam = []
	for ime1, x, y in kraji:
		if razdalja_koordinat(x,y,koord[0],koord[1]) <= domet and ime1 != ime:
			seznam.append(ime1)
	return seznam
...
def najbolj_oddaljeni(ime, imena, kraji):
	...
	koord = koordinate(ime, kraji)
	for ime1, x, y in kraji:
		if ime1 in imena and max_razdalja < razdalja(ime,ime1,kraji):
			max_razdalja = razdalja(ime,ime1,kraji)
			max_kraj = ime1
	return max_kraj

def zalijemo(ime, domet, kraji):
	krajeki = v_dometu(ime, domet, kraji)
	return najbolj_oddaljeni(ime, krajeki, kraji)
