...
for x in kraji:
    if x[0]==kraj:
        print(x[0], "je na koordinatah", x[1], x[2])
        x0=x[1]; y0=x[2]
najvecja_razdalja=0
list=[]
dictionary={}
...
for ime, x, y in kraji:
    ...
    if razdalja <= domet:
        ...
        dictionary={ime:najvecja_razdalja}
        for k, v in dictionary.items():
            if v > max_val:
                max_key=k
                max_val=v
print("Iz kraja",kraj,"lahko zalijemo kraj",max_key, "na razdalji", max_val)
...
for ime, x, y in kraji:
    ...
    razdalja2 = sqrt((x00 - x)**2 + (y00 - y)**2)
    if razdalja1 <= domet and razdalja2 <= domet:
        najvecja_razdalja=razdalja1
        dictionary={ime:najvecja_razdalja}
        for k, v in dictionary.items():
            print(k)
