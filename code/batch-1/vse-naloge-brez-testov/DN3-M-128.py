...
kraj_x = 0
kraj_y = 0
while i < len(kraji):       #Poišče koordinate kraja
    if kraj == kraji[i][0]:
        kraj_x = kraji[i][1]
        kraj_y = kraji[i][2]
    i += 1
i = 0
while i < len(kraji):       #Poišče najbolj oddaljen kraj v dometu
    raz = sqrt((kraji[i][1] - kraj_x)** 2 + (kraji[i][2] - kraj_y)** 2) #Izračuna razdaljo od kraja do kraja
    if maxraz < raz and raz < domet:    #Če je kraj v dometu in še nimamo kraja, ki je bolj oddaljen nastavi kraj kot najbolj oddaljen
        maxraz = raz
        maxkraj = kraji[i][0]
    i += 1
print(kraj,"je na koordinatah",kraj_x,kraj_y)
print("Iz kraja",kraj,"lahko zalijemo kraj",maxkraj,"na razdalji",maxraz)
...
domet = 30
i = 0
while i < len(kraji):       #Poišče koordinate krajev
    if kraj1 == kraji[i][0]:
        kraj1_x = kraji[i][1]
        kraj1_y = kraji[i][2]
    if kraj2 == kraji[i][0]:
    ...
    i += 1
i = 0
while i < len(kraji): #Preveri vsak kraj
    raz1 = sqrt((kraji[i][1] - kraj1_x) ** 2 + (kraji[i][2] - kraj1_y) ** 2) #Izračuna razdaljo od obeh krajev do izbranega kraja
    raz2 = sqrt((kraji[i][1] - kraj2_x) ** 2 + (kraji[i][2] - kraj2_y) ** 2)
    if raz1 < domet and raz2 < domet: #Če je izbrani kraj v dometu obeh krajev, ga izpišemo
        print("Skupaj lahko zalijemo mesto:",kraji[i][0])
    i += 1
