...
kraj = input("Vnesite kraj:")
domet = float(input("Vnesite domet:"))
max_razdalja = 0
kraj_max = ""
...
for ime2, x, y in kraji:
   ...
   # print("Drugi kraj", ime, x2, y2)
    razdalja = math.sqrt((x2 - x1)**2 +(y2 - y1)**2)

    if(razdalja <= domet):

        if(max_razdalja <= razdalja):
...
print("Iz kraja", kraj, "lahko zalijemo kraj", kraj_max, "na razdalji", max_razdalja )


kraj1 = input("Prvi kraj:")
kraj2 = input("Drugi kraj:")
domet2 = float(input("Domet:"))

for kraj, x, y in kraji:
    if(kraj1 == kraj):
        x_kraj1 = x
        y_kraj1 = y
        #print(kraj1, "leži na", x_kraj1, y_kraj1)

    elif(kraj2 == kraj):
        x_kraj2 = x
        y_kraj2 = y
        #print(kraj2, "leži na", x_kraj2, y_kraj2)
...
for kraj, x, y in kraji:
    if(True):
        ...
        #print(iskani_kraj, "leži na", x_iskani, y_iskani)
        razdalja_13 = math.sqrt((x_iskani - x_kraj1)**2 + (y_iskani - y_kraj1)**2)
        razdalja_23 = math.sqrt((x_iskani - x_kraj2)**2 + (y_iskani - y_kraj2)**2)
        #print(razdalja_23)
        #print(razdalja_13)

        if(razdalja_13 <= domet2 and razdalja_23 <= domet2):
            print(iskani_kraj)
