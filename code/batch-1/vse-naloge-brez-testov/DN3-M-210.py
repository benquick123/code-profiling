...
#    print("Ime kraja: ",kraj,", koordinate: ",x,",",y,", razdalja od izhodisca: ",abs(sqrt(x*x + y*y)))
#    if(abs(sqrt(x*x + y*y))>maxr):
#        maxr=abs(sqrt(x*x + y*y))
#        najbolj_oddaljen_kraj=kraj
#print("Najbolj oddaljen kraj je: ", najbolj_oddaljen_kraj, ", na razdalji: ",maxr)
...
for ime_kraja, x, y in kraji:
    if(ime_kraja==kraj):
        x1=x
        y1=y
        print(ime_kraja, "je na koordinatih:", x1, y1)

for ime_kraja, x, y in kraji:
    if(sqrt((x-x1)**2 + (y-y1)**2))<=domet:
        if(sqrt((x-x1)**2 + (y-y1)**2))>maxr:
            maxr=(sqrt((x-x1)**2 + (y-y1)**2))
            najbolj_oddaljen_kraj=ime_kraja
print("Iz kraja", kraj, "lahko zalijemo kraj", najbolj_oddaljen_kraj, "na razdalji", maxr)

kraj = str(input("Vneseš lahko tudi drugi kraj: "))
domet = float(input("In domet: "))
for ime_kraja, x, y in kraji:
    if(ime_kraja==kraj):
        x1=x
        y1=y
        print(kraj, "je na koordinatih:", x1, y1)
