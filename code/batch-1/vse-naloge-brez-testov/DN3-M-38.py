...
for nek_kraj, x, y in kraji:
    if nek_kraj == kraj:
        x1, y1 = (x, y)
        break
print(kraj , "je na kordinatah" , x1, y1 )
najvecja_razdalja = 0
for doseg, x2, y2 in kraji:
    razdalja = sqrt((x2-x1)**2 + (y2-y1)**2)
    if razdalja > najvecja_razdalja and razdalja < domet:
        najvecja_razdalja = razdalja
        dosegljiv_kraj = doseg

print("Iz kraja" , kraj , "lahko zalijemo kraj" , dosegljiv_kraj , "na razdalji" , najvecja_razdalja)
