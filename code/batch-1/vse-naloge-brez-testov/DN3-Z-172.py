...
for kraj2, koord_a, koord_b in kraji:
    if kraj2 == kraj:
        koord_x1 = koord_a
        koord_y1 = koord_b
...
naj_kraj = ""
naj_razdalja = 0
razdalja_med_krajema = 0
for kraj2, koord_x2, koord_y2 in kraji:
    razdalja_med_krajema = sqrt((koord_x1 - koord_x2) ** 2 + (koord_y1 - koord_y2) ** 2)
    if razdalja_med_krajema < domet and razdalja_med_krajema > naj_razdalja:
        naj_kraj = kraj2
        naj_razdalja = razdalja_med_krajema

print(kraj, "je na koordinatah:", koord_x1,",",koord_y1)
...
for kraj3, koord_a, koord_b in kraji:
    if kraj3 == kraj1:
        koord_x1 = koord_a
        koord_y1 = koord_b
    elif kraj3 == kraj2:
...
for kraj3, koord_a, koord_b in kraji:
    razdalja_med_krajema = sqrt((koord_x1 - koord_a) ** 2 + (koord_y1 - koord_b) ** 2)
    if razdalja_med_krajema < domet:
        razdalja_med_krajema = sqrt((koord_x2 - koord_a) ** 2 + (koord_y2 - koord_b) ** 2)
        if razdalja_med_krajema < domet:
            print(kraj3)
