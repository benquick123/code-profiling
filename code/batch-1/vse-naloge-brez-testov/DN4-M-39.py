...
def v_dometu(ime, domet, kraji):
    ...
    for ime_temp, x_temp, y_temp in kraji:
        if razdalja(ime, ime_temp, kraji) <= domet and ime != ime_temp:
            lokek.append(ime_temp)
    return (lokek)
def najbolj_oddaljeni(ime, imena, kraji):
    maxd   = 0
    maxIme = ""
    for ime_temp in imena:
        if razdalja(ime, ime_temp, kraji) > maxd:
            maxd = razdalja(ime, ime_temp, kraji)
            maxIme = ime_temp
    return(maxIme)
