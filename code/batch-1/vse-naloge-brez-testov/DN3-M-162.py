...
razdalja = 0
kraj = input("Vnesi ime kraja ")
domet = int(input("Vnesi razdaljo "))
...
for mesto, x, y in kraji:
    ...
    if r < domet and r > razdalja:
        dolocenkraj = mesto
        razdalja = r

print(kraj + " je na kordinatah " + str(x1) + ", " + str(y1))
print("Iz " + kraj + " lahko zalijemo kraj " + dolocenkraj + " na razdalji " + str(razdalja))

# ---------------------------------------------------------------------------------------------------------

print("Dodatna naloga")

kraj1 = input("Vnesi ime prvega kraja ")
kraj2 = input("Vnesi ime drugega kraja ")
domet1 = int(input("Vnesi razdaljo "))

for mesto1, x, y in kraji:
    if mesto1 == kraj1:
       x2 = x
       y2 = y

for mesto2, x, y in kraji:
    if mesto2 == kraj2:
       x3 = x
       y3 = y

for mesto, x, y in kraji:
    r1 = sqrt(pow((x-x2),2) + pow((y-y2),2))
    r2 = sqrt(pow((x-x3),2) + pow((y-y3),2))
    if r1 < domet1 and r2 < domet1:
        print(mesto)