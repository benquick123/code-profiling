...
# Na tem mestu dopiši
for i in kraji:
    #razdalja = math.sqrt(math.pow(kraj[1], 2)+math.pow(kraj[2], 2))
    if i[0]==kraj:
        kraj=i

print kraj[0], " je na koordinatih ", kraj[1]," ", kraj[2]

prenos=0

for i in kraji:
    razdalja = math.sqrt(math.pow(kraj[1]-i[1], 2)+math.pow(kraj[2]-i[2], 2))
    if (razdalja<=domet):
        if razdalja>prenos:
            prenos=razdalja
            kraj_dve=i
...
if prenos==0:
    print kraj[0]," lahko pomaga samo sebi"
else:
    print "Iz kraja", kraj[0], "lahko zalijemo kraj", kraj_dve[0],"na razdalji", prenos
