...
def v_dometu(ime, domet, kraji):
    ...
    for naziv,x,y in kraji:
        if naziv != ime:
            a = razdalja_koordinat(ime_x_y[0],ime_x_y[1],x,y)
            if a <= domet:
                seznam.append(naziv)
    return seznam
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    ime_naj = None
    for e in seznam:
        if razdalja_koordinat(e[1],e[2],ime_x_y[0],ime_x_y[1]) > najvec:
            najvec = razdalja_koordinat(e[1],e[2],ime_x_y[0],ime_x_y[1])
            ime_naj = e[0]
    return ime_naj

def zalijemo(ime,domet,kraji):
    koordine = koordinate(ime,kraji)
    x = koordine[0]
    y = koordine[1]
    naj = 0
    kdo = None
    for x,y,z in kraji:
        if razdalja(ime,x,kraji) > naj and razdalja(ime,x,kraji) <= domet:
            naj = razdalja(ime,x,kraji)
            kdo = x
    return kdo

def presek(s1,s2):
    return list(set(list(s1)).intersection(set(list(s2))))

def skupno_zalivanje(ime1,ime2,domet,kraji):
    imena = []
    for x,y,z in kraji:
        if razdalja(x,ime1,kraji) <= domet and razdalja(x,ime2,kraji) <= domet:
            imena.append(x)
    return imena
