...
def v_dometu(ime,domet,kraji):
    ...
    for x,y,z in kraji:
       if x == ime:
           ...
           z1 = z
    for x,y,z in kraji:
        if (sqrt((y1-y)**2 + (z1-z)**2) <= domet) and (sqrt((y1-y)**2 + (z1-z)**2) != 0):
            vdometu.append(x)
    return vdometu
...
def najbolj_oddaljeni(ime,imena,kraji):
    ...
    for i in imena:
        for x,y,z in kraji:
            if (sqrt((y1-y)**2 + (z1-z)**2) >= razdalja) and x == i:
                razdalja = (sqrt((y1-y)**2 + (z1-z)**2))
                najod = i
    return najod
