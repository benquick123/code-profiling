...
c=[x for x in kraji if x[0] == kraj]

najdlje=0
kraj=0

for ime,x,y in kraji:
    razdalja=sqrt((abs(c[0][1]-x)**2)+(abs(c[0][2]-y)**2))
    if razdalja<domet and razdalja>najdlje:
        kraj=ime
        najdlje=razdalja

print(c[0][0],"je na koordinatah",c[0][1],c[0][2])
print("Iz kraja ",c[0][0]," lahko zalijemo kraj ",kraj," na razdalji ",najdlje,"\n")
...
c=[x for x in kraji if x[0] == kraj1]
c2=[x for x in kraji if x[0] == kraj2]
...
for ime,x,y in kraji:
    razdalja1=sqrt((abs(c[0][1]-x)**2)+(abs(c[0][2]-y)**2))
    razdalja2=sqrt((abs(c2[0][1]-x)**2)+(abs(c2[0][2]-y)**2))
    if (razdalja1<domet and razdalja2<domet) and (razdalja1>najdlje1 and razdalja2>najdlje2):
        kraj=kraj + (ime,)

print(c[0][0],"in",c2[0][0],"lahko zalivata kraje:")
for x in kraj:
    print(x)
