...
def RacunRazdalja(i,j):
    raz = sqrt((kraji[i][1] - kraji[j][1])**2 + (kraji[i][2] - kraji[j][2])**2)
    return raz

kraj = input("Vpiši kraj: ")
domet = float(input("Vpiši domet: "))
krajIndex = -1
...
while i < len(kraji):
    if kraj == kraji[i][0]:
        krajIndex = i
        print(kraj," je na koordinatah ",kraji[i][1]," ",kraji[i][2])
    i += 1

if krajIndex == -1:
    print("Vpisanega kraja ni v tabeli.")
else:
    maxIndex = krajIndex
    maxRazdalja = RacunRazdalja(krajIndex, maxIndex)

    i = 0
    while i < len(kraji):
        if  RacunRazdalja(krajIndex,i) <= domet:
            if RacunRazdalja(krajIndex,i) >= RacunRazdalja(krajIndex,maxIndex):
                maxIndex = i
                maxRazdalja = RacunRazdalja(krajIndex, maxIndex)
        i += 1

    print("Iz kraja ",kraji[krajIndex][0]," lahko zalijemo kraj ",kraji[maxIndex][0]," na razdlji ",maxRazdalja,".")
...
kraj1 = input("Vpiši kraj 1: ")
kraj2 = input("Vpiši kraj 2: ")
domet = float(input("Vpiši domet: "))
kraj1Index = -1
kraj2Index = -1
...
while i < len(kraji):
    if kraj1 == kraji[i][0]:
        kraj1Index = i
        print(kraj1," je na koordinatah ",kraji[i][1]," ",kraji[i][2])

    if kraj2 == kraji[i][0]:
        kraj2Index = i
        print(kraj2," je na koordinatah ",kraji[i][1]," ",kraji[i][2])

    i += 1

if kraj1Index == -1 or kraj2Index == -1:
    print("Vpisanih krajev ni v tabeli.")

else:
    i = 0
    while i < len(kraji):
        if  RacunRazdalja(kraj1Index,i) <= domet and RacunRazdalja(kraj2Index,i) <= domet:
            if i != kraj1Index and i != kraj2Index:
                mozneTarce.append(i)
        i += 1

    i = 0
    while i < len(mozneTarce):
        print(kraji[mozneTarce[i]][0])
        i += 1
