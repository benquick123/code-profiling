...
x = 0
while x < len(kraji):
    razdaljaOdLj = sqrt((kraji[x][1] - kraji[krajID][1]) ** 2 + ((kraji[x][2] - kraji[krajID][2])) ** 2)
    if razdaljaOdLj <= domet and razdaljaOdLj > najdlje:
        najdlje = razdaljaOdLj
        trenutniKraj = kraji[x][0]
    x += 1
print(kraji[krajID][0], "je na koordinatah", kraji[krajID][1], kraji[krajID][2])
print("Iz kraja", kraji[krajID][0],  "lahko zalijemo kraj", trenutniKraj, "na razdalji", najdlje)
