...
for ime, x, y in kraji:
    if(ime == kraj):
        print(ime, " se nahaja na koordinatah", x, y)
        x_kraj = x
        y_kraj = y

z_razdalja=0
for ime, x, y in kraji:
    razdalja_med = sqrt((x_kraj - x) ** 2 + (y_kraj - y) ** 2)
    if(razdalja_med<=domet):
        if(razdalja_med>z_razdalja):
            z_razdalja=razdalja_med
            z_kraj=ime

print("Iz kraja ", kraj, "lahko zalijemo kraj ", z_kraj, "na razdalji", z_razdalja)
#-------------------------------------------------------------------------------------------------------------------
print("\n--------------------------Dodatna naloga--------------------------")
kraj1 = "Ljubljana"
kraj2 = "Bled"
...
for ime, x, y in kraji:
    razdalja_med1 = sqrt((x_kraj1 - x) ** 2 + (y_kraj1 - y) ** 2)
    razdalja_med2 = sqrt((x_kraj2 - x) ** 2 + (y_kraj2 - y) ** 2)
