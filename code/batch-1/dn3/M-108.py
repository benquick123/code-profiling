...
domet=30
min=0
for sublist in kraji:
    if kraj0==sublist[0]:
       x1=sublist[1]
       y1=sublist[2]
print(kraj0, "je na koordinatah", x1, "in", y1)

for sublist in kraji:
    if kraj1==sublist[0]:
        x2 = sublist[1]
        y2 = sublist[2]
print(kraj1, "je na koordinatah", x2, "in", y2)

for sublist in kraji:
    r = sqrt((x1-sublist[1]) ** 2 + (y1-sublist[2]) ** 2)
    if r > min and r<domet:
        min = r
        kraj2=sublist[0]
print("Iz kraja", kraj0, "lahko zalijemo kraj", kraj2, "na razdalji", min)

for sublist in kraji:
    r1 = sqrt((x1 - sublist[1]) ** 2 + (y1 - sublist[2]) ** 2)
    r2 = sqrt((x2 - sublist[1]) ** 2 + (y2 - sublist[2]) ** 2)
    if r1<domet and r2<domet:
        kraj3=sublist[0]
        kraj4=sublist[0]
        if kraj3==kraj4:
            print("Kraja", kraj0, "in", kraj1, "lahko skupaj zalivata", kraj3)
