...
def vodniTopMedKraji(kraji, izbran_kraj, domet):
    naj_kraj = ""
    naj_razdalja = -1
    x1 = "0"
    for kraj, x, y in kraji:
        if kraj == izbran_kraj:
            ...
            break
    if x1 == "0":
        print("Kraj", izbran_kraj, "v tabeli ne obstaja.")
    else:
        for kraj, x2, y2 in kraji:
            razdalja = math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2)
            if naj_razdalja < razdalja < domet and izbran_kraj != kraj:
                naj_razdalja = razdalja
                naj_kraj = kraj
        if naj_razdalja != -1:
            print("Iz kraja", izbran_kraj, "lahko zalijemo kraj", naj_kraj, "na razralji", naj_razdalja)
        else:
            print("Iz kraja", izbran_kraj, "nemoremo zaliti nobenega drugega kraja.")

def vodniTopMedKrajiDodatno(kraji, kraj1, kraj2, domet):
    x1 = "0"
    x2 = "0"
    for kraj, x, y in kraji:
        if kraj == kraj1:
    ...
    kraj = True
    if x1 == "0":
        print("Kraj", kraj1, "v tabeli ne obstaja.")
        kraj = False
    if x2 == "0":
        print("Kraj", kraj2, "v tabeli ne obstaja.")
        kraj = False
    if kraj:
        st = False
        for kraj, x, y in kraji:
            razdalja1 = math.sqrt((x1 - x) ** 2 + (y1 - y) ** 2)
            razdalja2 = math.sqrt((x2 - x) ** 2 + (y2 - y) ** 2)
            if razdalja1 < domet and razdalja2 < domet and kraj1 != kraj and kraj2 != kraj:
                print(kraj)
                st = True
        if not st:
            print("Družno ne moreta zalivati nobenega kraja.")


kraj = "Litija"
domet = 60
vodniTopMedKraji(kraji, kraj, domet)

kraj1 = "Ljubljana"
kraj2 = "Bled"
domet = 30
vodniTopMedKrajiDodatno(kraji, kraj1, kraj2, domet)
