...
def v_dometu(ime, domet, kraji):
    ...
    for ime3, x3, y3 in kraji:
        if razdalja(ime, ime3, kraji) <= domet and ime != ime3:
            zalijene.append(ime3)
    return (zalijene)
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    maxIme = ""
    for ime3 in imena:
        if razdalja(ime, ime3, kraji) > maxd:
            maxd = razdalja(ime, ime3, kraji)
            maxIme = ime3
    return(maxIme)
...
def zalijemo(ime, domet, kraji):
    maxd   = 0
    maxIme = ""
    for ime3, x_temp, y_temp in kraji:
        m = razdalja(ime, ime3, kraji)
        if m > maxd and m <= domet:
            maxd = m
            maxIme = ime3
    return(maxIme)
