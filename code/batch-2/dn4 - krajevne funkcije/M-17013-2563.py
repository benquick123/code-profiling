...
razdalja_koordinat(-80.44, -34.96, -73.34, -33.18)
...
razdalja("Ljubljana", "Kranj", kraji)
...
def v_dometu(ime, domet, kraji):
    k = []
    for mesto, x, y in kraji:
        r = razdalja(ime, mesto, kraji)
        if r <= domet and r > 0:
            k.append(mesto)
    return k

v_dometu("Ljubljana", 20, kraji)
...
#                 obeh.append(element1)
#     return obeh

def presek(s1, s2):
    obeh = []
    for element1 in s1:
        if s2.__contains__(element1):
            obeh.append(element1)
    return obeh

presek(["Ana", "Berta", "Cilka"], ["Cilka", "Dani", "Ema"])


def skupno_zalivanje(ime1, ime2, domet, kraji):
    return presek(v_dometu(ime1, domet, kraji),v_dometu(ime2, domet, kraji))

skupno_zalivanje("Bled", "Ljubljana", 30, kraji)
