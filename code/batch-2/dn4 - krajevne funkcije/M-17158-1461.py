...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    maxime = ""
    for imet in imena: 
        if razdalja(ime, imet, kraji) > maxd:
            maxd = razdalja(ime, imet, kraji)
            maxime = imet
    return(maxime)
...
def zalijemo(ime, domet, kraji):
    ...
    maxime = ""
    for i in kraji: 
        a = razdalja(ime, i[0], kraji)
        if a > maxd and a <= domet:
            maxd = a
            maxime = i[0]
    return(maxime)
...
def skupno_zalivanje(ime1, ime2, domet, kraji):
    arr = []
    for i in kraji:
        if razdalja(ime1, i[0], kraji) <= domet and razdalja(ime2, i[0], kraji) <= domet:
            arr.append(i[0])
    return (arr)
