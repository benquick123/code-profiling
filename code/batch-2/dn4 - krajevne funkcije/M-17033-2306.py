...
def koordinate(ime, kraji):      #---------- PRVI KRAJ --------------
    for a in kraji:
        prvo_ime, x_k, y_k = a
        if(prvo_ime == ime):
            return x_k, y_k
...
def razdalja_koordinat(x1, y1, x2, y2):          #---------- RACUNA RAZDALJO MED KOORDINATI --------------
    razdalja_k=sqrt(pow(x2 - x1,2)+pow(y2 - y1,2))
    return razdalja_k

def razdalja(ime1, ime2, kraji):          #---------- VRNE RAZDALJO MED KRAJEMA --------------
    x1,y1 = koordinate(ime1, kraji)
    x2,y2 = koordinate(ime2, kraji)
    rezultat = razdalja_koordinat(x1, y1, x2, y2)
    return rezultat

def v_dometu(ime, domet, kraji):          #---------- ALI JE NEK KRAJ V DOMETU NEKEGA DRUGEGA --------------
    seznam = []
    x_static = 0
    y_static = 0
    for a in kraji:
        imekraja, x_drugi, y_drugi = a
        if (imekraja == ime):
            x_static = x_drugi
            y_static = y_drugi

    for b in kraji:
       imekraja, x_drugi, y_drugi = b
       r = sqrt(pow(x_drugi - x_static, 2) + pow(y_drugi - y_static, 2))
       if (r <= domet):
            if (r == 0):
                None
            else:
...
def najbolj_oddaljeni(ime, imena, kraji):          #---------- VRNE NAJBOLJ ODDALJEN KRAJ (IME) --------------
    n_0 = 0
...
def zalijemo(ime, domet, kraji):          #---------- KATERI KRAJ LAHKO ZALIJEMO --------------
    n_o = 0
    x1,y1 = koordinate(ime, kraji)
    for b in kraji:
        ime_x, x, y = b
        r = razdalja_koordinat(x1, y1, x, y)
        if r>n_o and r<=domet:
            n_o = r
            najbolj_oddaljen_ime = ime_x
    return najbolj_oddaljen_ime
