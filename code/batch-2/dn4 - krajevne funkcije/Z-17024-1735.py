...
def razdalja(ime1, ime2, kraji):
    koordinate_ime1=koordinate(ime1, kraji)
    koordinate_ime2=koordinate(ime2, kraji)
    if koordinate_ime1 is not None and koordinate_ime2 is not None:
        x1,y1=koordinate_ime1
        x2,y2=koordinate_ime2
        return razdalja_koordinat(x1, y1, x2, y2)

def v_dometu(ime, domet, kraji):
    kraji_v_dometu=[]
    for kraj,x,y in kraji:
        razdalja_do_kraja=razdalja(ime, kraj, kraji)
        if razdalja_do_kraja is not None and ime!=kraj and razdalja_do_kraja<=domet:
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    naj_kraj= None
    for kraj in imena:
        trenutna_razdalja=razdalja(ime, kraj, kraji)
        if trenutna_razdalja is not None and trenutna_razdalja>naj_razdalja:
            naj_razdalja=trenutna_razdalja
            naj_kraj=kraj
    return naj_kraj
...
def zalijemo(ime, domet, kraji):
    ...
    for kraj,x,y in kraji:
        trenutna_razdalja=razdalja(ime, kraj, kraji)
        if trenutna_razdalja is not None and trenutna_razdalja <= domet and trenutna_razdalja>=naj_razdalja:
            naj_kraj=kraj
            naj_razdalja=trenutna_razdalja
    return naj_kraj
