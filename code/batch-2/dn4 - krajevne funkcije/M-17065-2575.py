...
def v_dometu(ime, domet, kraji):
    zacetne_koordinate = koordinate(ime, kraji)

    if zacetne_koordinate is None:
        return []


    return [kraj[0] for kraj in kraji if kraj[0] != ime and razdalja_koordinat(zacetne_koordinate[0], zacetne_koordinate[1], kraj[1], kraj[2]) <= domet]
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    druge_koordinate = koordinate(max_kraj, kraji)
    max_oddaljenost = razdalja_koordinat(zacetne_koordinate[0], zacetne_koordinate[1], druge_koordinate[0], druge_koordinate[1])

    for ime_kraja in imena[1:]:
        druge_koordinate = koordinate(ime_kraja, kraji)
        oddaljenost = razdalja_koordinat(zacetne_koordinate[0], zacetne_koordinate[1], druge_koordinate[0], druge_koordinate[1])

        if oddaljenost > max_oddaljenost:
            max_kraj = ime_kraja
            max_oddaljenost = oddaljenost
