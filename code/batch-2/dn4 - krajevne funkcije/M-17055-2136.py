...
def v_dometu(ime,domet,kraji):
    ...
    for i, x1, y1 in kraji:
        lenght = razdalja_koordinat(x,y,x1,y1)
        if lenght <= domet and ime != i:
            s.append(i)
    return s
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    x, y = koordinate(ime, kraji)
    for iskan in imena:
        x1, y1 = koordinate(iskan, kraji)
        razdalja_krajev = razdalja_koordinat(x,y,x1,y1)
        if razdalja_krajev > i:
            i = razdalja_krajev
            naj_kraj = iskan
    return naj_kraj
...
def zalijemo(ime, domet, kraji):
    a = 0
    x,y = koordinate(ime, kraji)
    for r, x1, y1 in kraji:
        razdalj_kraji_a = razdalja_koordinat(x,y,x1,y1)
        if razdalj_kraji_a <= domet:
            if razdalj_kraji_a > a:
                a = razdalj_kraji_a
                n_kraj = r
    return n_kraj
...
def skupno_zalivanje(ime1, ime2, domet, kraji):
    r = v_dometu(ime1, domet, kraji)
    c = v_dometu(ime2, domet, kraji)
    a = presek(r, c)
    return a
