#Domača naloga Vodni top s funkcijami Kenan Mušić
kraji = [
...
def koordinate(ime, kraji):                              #funkcija koordinate

    for začasni_grad, začasni_x, začasni_y in kraji:
        if začasni_grad == ime:
            x = začasni_x
            y = začasni_y
            return ((x, y))
...
def razdalja_koordinat(x1, y1, x2, y2):                  #funkcija razdalje test1
...
def v_dometu(ime, domet, kraji):                         #funkcija dometa test 4
    lokek = []

    for ime_začas, x_začas, y_začas in kraji:

        if razdalja(ime, ime_začas, kraji) <= domet and ime != ime_začas:
            lokek.append(ime_začas)
    return (lokek)


def najbolj_oddaljeni(ime, imena, kraji):                 #funkcija najbolje oddaljeni test 5

    največja = 0
    največIme = ""

    for ime_začas in imena:

        if razdalja(ime, ime_začas, kraji) > največja:

            največja = razdalja(ime, ime_začas, kraji)
            največIme = ime_začas

    return (največIme)

def zalijemo(ime, domet, kraji):                          #funkcija zalijemo test 6
    ...
    for ime_začas, x_začas, y_začas in kraji:
        ...
        z = razdalja(ime, ime_začas, kraji)
        if z > največja and z <= domet:
            največja = z
            največIme = ime_začas
