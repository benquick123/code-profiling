...
print(koordinate("Brežice", kraji))
...
print(razdalja_koordinat(0, 0, 1, 0))
...
def v_dometu(ime, domet, kraji):
    ...
    for mesto, x, y in kraji:
        raz = sqrt(pow((x - x1), 2) + pow((y - y1), 2))
        if raz <= domet:
            if ime != mesto:
                tabela.append(mesto)
    return tabela

print(v_dometu("Ljubljana", 20, kraji))
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    x1, y1 = koordinate(ime, kraji)
    for mesto, x, y in kraji:
        razdalja = sqrt(pow((x - x1), 2) + pow((y - y1), 2))
        if naj_razdalja < razdalja and mesto in imena:
...
print(presek(["Banana", 2, 5, 3.15, True], ["Banana", 5, True, 7, 3.15]))
...
def skupno_zalivanje(ime1, ime2, domet, kraji):
    ...
    for ime, x, y in kraji:
        if sqrt(pow((x - x1), 2) + pow((y - y1), 2)) < domet and sqrt(pow((x - x2), 2) + pow((y - y2), 2)) < domet:
            t.append(ime)
    return t

print(skupno_zalivanje("Bled", "Ljubljana", 30, kraji))
