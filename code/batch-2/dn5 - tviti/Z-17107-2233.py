...
def vsi_avtorji(tvit):
    names = []
    for tvitek in tvit:
        names.append(avtor(tvitek))
    return unikati(names)
...
def se_zacne_z(tvit, c):
    ...
    for letter in tvit:
        ...
        if found:
            results += letter
    results = results.split(c)[1:]
    for i in range(len(results)):
        results[i] = izloci_besedo(results[i])
    return results

def zberi_se_zacne_z(tviti, c):
    results = []
    for tvit in tviti:
        result = se_zacne_z(tvit, c)
        for rezultat in result:
            if rezultat not in results:
                results.append(rezultat)
    return results

def vse_afne(tviti):
    results = []
    for tvit in tviti:
        result = se_zacne_z(tvit, "@")
        for rezultat in result:
            if rezultat not in results:
                results.append(rezultat)
    return results
