...
def vse_osebe(tviti):
    #osebe = []
    #osebe.extend(vse_afne(tviti) + vsi_avtorji(tviti))
...
def custva(tviti, hashtagi):
    custveni = []
    for tvit in tviti:
        for hash1 in hashtagi:
            for hash2 in se_zacne_z(tvit, "#"):
                if hash1 == hash2:
                    custveni.append(avtor(tvit))
    custveni.sort()
    return unikati(custveni)

def se_poznata(tviti, oseba1, oseba2):
    for tvit in tviti:
        if avtor(tvit) == oseba1 or avtor(tvit) == oseba2:
            for oseba in se_zacne_z(tvit, "@"):
                if oseba == oseba1 or oseba == oseba2:
                    return True
    return False
