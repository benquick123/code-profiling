...
def dveurezenedela(tvit):
    seznam=tvit.split(" ")
    seznam2=[]
    for beseda in seznam:
        if beseda[0] == "@":
            seznam2.append(izloci_besedo(beseda))
    return seznam2
...
def omembe(tviti):
    ...
    for tvit in tviti:
        if avtor(tvit) not in slovar:
            slovar[avtor(tvit)] = []
        slovar[avtor(tvit)]+=unikati(dveurezenedela(tvit))
