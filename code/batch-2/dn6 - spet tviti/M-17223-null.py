...
def se_poznata(ime1, ime2, omembe):
    for k, v in omembe.items():
        if (k == ime1 and ime2 in omembe[k]) or (k == ime2 and ime1 in omembe[k]):
            return True
    return False

def hashtagi(tviti):
    vsi_hashi = vsi_hashtagi(tviti)
    slovar = dict.fromkeys(vsi_hashi, None)
    for line in tviti:
        hashs = se_zacne_z(line, '#')
        for element in hashs:
            if slovar[element] is not None:
                slovar[element] += [avtor(line)]
            else:
                slovar[element] = [avtor(line)]
    for i in slovar:
        slovar[i].sort()
