...
def omembe(tviti):
    ...
    for tvit in tviti:
        user = avtor(tvit)
        mentioned = vse_afne(tvit)
        if user not in omenjeni:
            omenjeni[user] = mentioned
        else:
            omenjeni[user].extend(mentioned)
...
def neomembe(ime, omembe):
    avtorji = {}
    avtorji.update(omembe)
    neomenjeni = []
    for user in omembe[ime]:
        if user in avtorji:
            del avtorji[user]
    del avtorji[ime]
    for x in avtorji:
...
def hashtagi(tviti):
    tag = vsi_hashtagi(tviti)
    hash = {}
    for x in tag:
        hash[x] = custva(tviti, x)
    return hash
