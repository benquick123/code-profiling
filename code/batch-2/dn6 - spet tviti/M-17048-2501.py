...
def se_poznata(ime1, ime2, omembe):
    for author in omembe.keys():
        if author == ime1:
            if ime2 in omembe[author]:
                return True

        if author == ime2:
            if ime1 in omembe[author]:
                return True
...
def get_all_hashtags(tweets):
    ...
    for tweet in tweets:
        content = besedilo(tweet)
        hashtags.extend(extract_word_starting_with(tweet, '#'))

    return hashtags

def hashtagi(tviti):
    hashtags_used_by = defaultdict(list)

    for tweet in tviti:
        author = get_author(tweet)
        content = besedilo(tweet)
        hashtags_used = extract_word_starting_with(content, '#')
        for hashtag in hashtags_used:
            if author not in hashtags_used_by[hashtag]:
                hashtags_used_by[hashtag].append(author)

    hashtags_used_by = dict(hashtags_used_by)
    # Sort the people by name
    for hashtag in hashtags_used_by:
        hashtags_used_by[hashtag].sort()

    return hashtags_used_by
