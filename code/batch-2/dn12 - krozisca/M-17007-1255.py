    ...
    def vrstni_red(tab):
        ...
        for i in range(len(tab)):
            if tab[i] < a:
                a = tab[i]
                indeks = i
        return tab[indeks:] + tab[:indeks]

    for vrstica in file:
        tabela = re.split(r' +', vrstica)
        for i in range(len(tabela)):
            tabela[i]= int(tabela[i])
        tabela = vrstni_red(tabela)
        slovar[ind] = tabela
        ind += 1
    return slovar
...
def mozna_pot(pot, slovar):
    #slovar = preberi(zemljevid)
    if len(slovar[pot[0]]) != 1 or len(slovar[pot[-1]]) != 1:
        return False
    for i in range(len(pot) - 1):
        if pot[i + 1] not in slovar[pot[i]]:
            return False
        if len(slovar[pot[i]]) == 1 and i != 0:
            return False
    return True
...
def hamiltonova(pot, zemljevid):
    ...
    koncni = []
    for x in zemljevid:
        if len(zemljevid[x]) == 1 and x not in pot:
            koncni.append(x)
    pot.sort()
...
def navodila(pot, zemljevid):
    return [(zemljevid[pot[i]].index(pot[i+1]) - zemljevid[pot[i]].index(pot[i-1]) + len(zemljevid[pot[i]])) % len(zemljevid[pot[i]]) for i in range(1, len(pot) - 1)]

def prevozi(zacetek, navodila, zemljevid):
    pot = [zacetek, zemljevid[zacetek][0]]
    for i in range(len(navodila)):
        naslednji = zemljevid[pot[-1]][(zemljevid[pot[-1]].index(pot[-2]) + navodila[i]) % len(zemljevid[pot[-1]])]
