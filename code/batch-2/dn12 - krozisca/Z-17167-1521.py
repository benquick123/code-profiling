...
def preberi(ime_datoteke):
    ...
    def ostevilci(vrsta):
        i= 17
        for kljuc in range(len(vrsta)):
            if vrsta[kljuc] < i:
                i = vrsta[kljuc]
                indeks = kljuc
        return vrsta[indeks:] + vrsta[:indeks]
    for vrstica in datoteka:
        vrstice = re.split(r' +', vrstica)
        for i in range(len(vrstice)):
            vrstice[i]= int(vrstice[i])
        vrstice = ostevilci(vrstice)
        slovar[n] = vrstice
        n += 1
    return slovar

def mozna_pot(pot, slovar):
    #Štartna pozicija + izvozi
    for pozicija in range(len(pot)-1):
        if len(slovar[pot[0]]) is not 1 or len(slovar[pot[-1]]) is not 1 or pot[pozicija + 1] not in slovar[pot[pozicija]] or len(slovar[pot[pozicija]]) == 1 and pozicija is not 0:
            return False
    return True

def hamiltonova(pot, zemljevid):
    hamilton = []
    #Če je možna pot = True je možen tudi hamilton. Unikati?
    if mozna_pot(pot, zemljevid) == True:
        for zavoj in zemljevid:
            if len(zemljevid[zavoj]) == 1 and zavoj not in pot:
                hamilton.append(zavoj)
        pot.sort()
        direkt = [izvoz for izvoz in range(1, max(pot)+1)]
        for zavoj in hamilton:
            direkt.remove(zavoj)
        if pot == direkt:
            return True
        else:
