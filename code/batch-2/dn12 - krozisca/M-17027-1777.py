...
def preberi(ime_datoteke):
    ...
    i=0
    stevc=1
    text= open(ime_datoteke).read().splitlines()
    for a in text:
         i+=1
         b[i] = list(map(int, a.split(" ")))

    for stevc in b:
        if b[stevc][0] == min(b[stevc]):
            #print(b[stevc])
            continue
        else:
            while True:
...
def mozna_pot(pot, zemljevid):
    ...
    k.pop(-1)
    for a in pot:
       if len(zemljevid[a]) > 1 and (a==pot[0] or a==pot[-1]):
           return False
       if min not in zemljevid[a] and min != 0:
           return False
       if a == min:
           return False
       elif i<len(pot)-1:
           ...
           min=0
    for j in k:
        if len(zemljevid[j]) == 1:
            return False
    return True

def hamiltonova(pot, zemljevid):
   if mozna_pot(pot,zemljevid) == False or len(pot)<len(zemljevid)-2 or len(pot) != len(set(pot)):
       return False
   return True
