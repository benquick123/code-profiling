...
def razvrsti(vrsticka):
    #iz vrsticke naredit seznam...
    seznam = []
    for x in vrsticka.split(" "):
        seznam.append(int(x))


    #razvrščanje...
    if min(seznam) != seznam[0]:
        lowin = seznam.index(min(seznam))
        for x in range(lowin):
            seznam.append(seznam[x])
        del seznam[0:lowin]
    return seznam
...
def preberi(ime_datoteke):
    ...
    xs = {i: set() for i in range(1,stevilo_vrstic)}
    s = 1
    datoteka.close()
    datoteka = open(ime_datoteke)
    for vrstica in datoteka:
        xs[s] = razvrsti(vrstica)
        s = s + 1
    datoteka.close()
    return xs
...
def mozna_pot(pot,zemljevid):
    for x in range(len(pot) - 1):
        if pot[x] == pot[x + 1]:
            print("ponovita se stevilki")
            return False
        for krizisce, poti in zemljevid.items():
            if pot[x] == krizisce:
                if pot[x] == pot[0]:
                    if len(poti) != 1:
                        print("pru krizise ma vec k eno pot")
                        return False
                elif pot[x] == pot[-1]:
                    if len(poti) != 1:
                        print("zadn krizise ma vec k eno pot")
                        return False
                else:
                    if len(poti) <= 1:
                        print("vmesna krizisca majo vec men k dve poti")
                        return False
                if pot[x + 1] not in poti:
                    print("poti se ne povezujejo")
                    return False
            if pot[-1] == krizisce:
                if len(poti) != 1:
                    print("zadn krizise ma vec k eno pot")
