...
def preberi(ime_datoteke):
    ...
    sez = []
    for line in f:
        sez.append(line.strip())
    slovar = {}
    for i in range(len(sez)):
        slovar[i + 1] = sort_order(sez[i])
    return slovar

def je_povezava(num1, num2, zemljevid):
    if num1 in zemljevid[num2]:
        return True
...
def mozna_pot(pot, zemljevid):
    ...
    for i in range(len(pot) - 1):
        if not je_povezava(pot[i], pot[i + 1], zemljevid):
            je_mozna = False
...
def hamiltonova(pot, zemljevid):
    ...
    if len(pot) != len(a):
        ham = False
    if len(a) != len(zemljevid.keys()) - (st_izhodov(zemljevid) - 2):
        ham = False
    return ham
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    s += zemljevid[zacetek]
    for a in navodila:
        if zemljevid[s[-1]].index(s[-2]) + a < len(zemljevid[s[-1]]):
            s.append(zemljevid[s[-1]][zemljevid[s[-1]].index(s[-2]) + a])
        else:
            s.append(zemljevid[s[-1]][zemljevid[s[-1]].index(s[-2]) + a - len(zemljevid[s[-1]])])
    return s
...
def razdalja(x, y, zemljevid):
    return len(poti(x, y, zemljevid)) - 1

def najkrajsa_navodila(x, y, zemljevid):
    return navodila(poti(x, y, zemljevid), zemljevid)
