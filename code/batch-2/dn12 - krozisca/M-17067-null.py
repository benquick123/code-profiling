...
def preberi(ime_datoteke):
    ...
    for vrstica in datoteka.read().splitlines():
        if len(vrstica) <= 1:
            slovar[i] = [int(vrstica)]
            i += 1
        else:
            sez = vrstica.split()
            mesto_najm = sez.index(min(sez, key=int))
            for j in range(mesto_najm):
...
def mozna_pot(pot, zemljevid):
    ...
    for i in zemljevid:
       if len(zemljevid[i]) == 1:
            seznam_vhod_izhod.append(i)
    pot = list(map(int, pot))
    if pot[0] in seznam_vhod_izhod:
        if pot[-1] in seznam_vhod_izhod:
            for z in seznam_vhod_izhod:
                if z in pot[1:-1]:
                    return False
            for i in pot:
                if i == enak:
            ...
            for k in pot:
                if k == pot[-1]:
                    return True
                if k not in zemljevid[pot[stev]]:
                    return False
                if stev < len(pot):
                    stev += 1
            return True
...
def hamiltonova(pot, zemljevid):
    ...
    for i in zemljevid:
       if len(zemljevid[i]) == 1:
            seznam_vhod_izhod.append(i)
    if len(pot) != len(zemljevid)-(len(seznam_vhod_izhod)-2):
        return False
