...
def preberi(ime_datoteke):
    ...
    for vrstica in datoteka:
        ...
        kopija = sez.copy()
        slov.update({count: kopija})
        count += 1
        sez.clear()

    kopija = slov.copy()
    for kljuc, el in kopija.items():
        slov[kljuc] = sort(el)

    return slov

def koncna(kriz,zemljevid):
    if len(zemljevid[kriz]) == 1:
        return True
    else:
        return False

def povezana(kriz1, kriz2, zemljevid):
    if kriz2 in zemljevid[kriz1]:
        return True
    else:
        return False

def mozna_pot(pot, zemljevid):
    if koncna(pot[0], zemljevid) and koncna(pot[-1], zemljevid) is not True:
        return False
    elif koncna(pot[0], zemljevid) and koncna(pot[-1], zemljevid): #začne in konča z končno
        if len(pot) <= 2:
            return False
        potka = pot[1:-1]
        potika = pot[1:]
        for stev, krizisce in enumerate(potka):
            x = potika[stev + 1]
            y = pot[stev - 1]
            if koncna(krizisce, zemljevid): # če je kiro od krožišč umes končno je false
                return False
            elif krizisce == potika[stev + 1 ]: #če je isti ko nasledn
                return False

            elif povezana(krizisce, potika[stev + 1], zemljevid) is not True: # če so povezani
                return False
            elif stev >= 1:
                if povezana(krizisce, pot[stev], zemljevid) is not True:
                    return False
            elif povezana(pot[0], pot[1], zemljevid) is not True:
                return False
        return True
...
def hamiltonova(pot, zemljevid):
    ...
    if mozna_pot(pot, zemljevid):
        ...
        for key, element in zemljevid.items():
            if koncna(key, zemljevid):
                koncni += 1
        ...
        # koncni = 4
        # sez = [1, 3, 7, 8, 16, 9, 14, 11, 6, 4, 5, 10, 13, 12]
        zem = (len(zemljevid.keys()) - koncni + 2)
        sezn = len(sez)
        if zem == sezn:
            return True
        else:
