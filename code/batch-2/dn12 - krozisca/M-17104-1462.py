...
def preberi(ime_datoteke):
    ...
    for vsak in datoteka:
        seznam = []
        seznam.append(vsak.split())
        sez = [int(i) for i in seznam[0]]
        sez2 = []
        m = min(sez)
        ali = False
        for i in sez:
...
def mozna_pot(pot, zemljevid):
    ...
    if len(zemljevid[pot[0]]) != 1:
        ...
        return False
    for vsak in pot[1:-1]:
        if vsak not in zemljevid:
            return False
        elif len(zemljevid[vsak]) == 1:
            return False
    for i in range(len(pot)):
        if i+1 >= len(pot):
            break
        if pot[i+1] in zemljevid[pot[i]]:
            izpis = True
        ...
        if pot[i+1] == pot[i]:
            return False
    return izpis
print(mozna_pot([1, 3, 8, 8, 16, 15], preberi("zemljevid.txt")))
...
def hamiltonova(pot, zemljevid):
    ...
    if mozna_pot(pot, zemljevid) == True:
        izp = True
    ss = []
    for vsak in zemljevid:
        if len(zemljevid[vsak]) > 1:
            ss.append(vsak)
    for i in pot:
        if i in ss:
            ss.remove(i)
    ...
    for x in pot:
        if pot.count(x) != 1:
            izp = False
    return izp
print(hamiltonova([1, 3, 8, 16, 9, 14, 11, 6, 4, 5, 10, 11, 14, 13, 12], preberi("zemljevid.txt")))

"""================================================================================"""

#{1: [3], 2: [4], 3: [1, 8, 7, 6, 4], 4: [2, 3, 6, 5], 5: [4, 11, 10], 6: [3, 11, 4]}
def navodila(pot, zemljevid):
    sez = []
    for i in range(len(pot)):
        if len(zemljevid[pot[i]]) != 1:
            levi = zemljevid[pot[i]].index(pot[i-1]) +1
            desni = zemljevid[pot[i]].index(pot[i+1]) +1
            sez.append((desni-levi)%len(zemljevid[pot[i]]))
    return sez
print(navodila([1, 3, 6, 4, 2], preberi("zemljevid.txt"))) #[3, 2, 2]
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    st = 0
    for vsak in navodila:
        a = (zemljevid[seznam[st+1]].index(seznam[st]) + vsak)
        if a >= len(zemljevid[seznam[st+1]]):
            a = a - len(zemljevid[seznam[st+1]])
        ss = zemljevid[seznam[st+1]]
        seznam.append(ss[a])
        st += 1
    return seznam
print(prevozi(1, [3, 2, 2], preberi("zemljevid.txt"))) #[1, 3, 6, 4, 2]

"""================================================================================"""

#{1: [3], 2: [4], 3: [1, 8, 7, 6, 4], 4: [2, 3, 6, 5], 5: [4, 11, 10], 6: [3, 11, 4], 7: [3, 8, 11]}
def sosedi(doslej, zemljevid):
    izpis = set()
    for vsak in doslej:
        for vsi in zemljevid[vsak]:
            if vsi not in izpis and vsi not in doslej:
                izpis.add(vsi)
    return izpis
print(sosedi({1, 3, 7}, preberi("zemljevid.txt"))) #{4, 6, 11, 8}

def razdalja(x, y, zemljevid):
    seznam = [x]
    koliko = 0
    while y not in seznam:
        koliko += 1
        ss = []
        for vsak in seznam:
            for vsi in zemljevid[vsak]:
                if vsi not in seznam:
                    ss.append(vsi)
        seznam += ss
    return koliko
print(razdalja(15, 14, preberi("zemljevid.txt")))
