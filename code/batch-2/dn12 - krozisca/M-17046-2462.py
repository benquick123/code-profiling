...
def preberi(ime_datoteke):
    krozisca = { index+1: [int(izvoz) for izvoz in krozisca.strip().split()] for index, krozisca in enumerate(open(ime_datoteke)) }

    for kljuc in krozisca:
        krozisca[kljuc] = shift(krozisca[kljuc],krozisca[kljuc].index(min(krozisca[kljuc])))
...
def mozna_pot(pot,zemljevid):
    ...
    for index, smer in enumerate(pot[0:-1]):
        naslednja_smer = pot[index+1]
        if index > 0:
            if len(zemljevid[pot[index]]) == 1:
                return False

        if naslednja_smer not in zemljevid[smer]:
            return False
...
def hamiltonova(pot, zemljevid):
    ...
    krozisca_stevec = { indeks: 0 for indeks in zemljevid if len(zemljevid[indeks]) > 1 }

    for krozisce in pot:
        if krozisce in krozisca_stevec:
            krozisca_stevec[krozisce] += 1

    for stevilo_prevozov in krozisca_stevec:
        if krozisca_stevec[stevilo_prevozov] != 1:
            return False
...
def navodila(pot,zemljevid):
    smeri = []
    for vhod, krizisce, izhod in zip(pot,pot[1:],pot[2:]):
        indeks_vhod = zemljevid[krizisce].index(vhod)
        indeks_izhod = zemljevid[krizisce].index(izhod)
        smer = (indeks_izhod - indeks_vhod) % len(zemljevid[krizisce])
        smeri.append(smer)
    return smeri

def prevozi(zacetek, navodila, zemljevid):
    prejsnje_krizisce = zacetek
    trenutno_krizisce = zemljevid[zacetek][0]
    indeks_vhod = zemljevid[trenutno_krizisce].index(prejsnje_krizisce)
    pot = [zacetek, trenutno_krizisce]

    for navodilo in navodila:
        prejsnje_krizisce = trenutno_krizisce
        trenutno_krizisce = zemljevid[trenutno_krizisce][(indeks_vhod+navodilo)%len(zemljevid[trenutno_krizisce])]    
        indeks_vhod = zemljevid[trenutno_krizisce].index(prejsnje_krizisce)
        pot.append(trenutno_krizisce)
    return pot

def sosedi(doslej,zemljevid):
    krozisca = set()
    prvotna_krozisca = []
    for st_krozisce in doslej:
        prvotna_krozisca.append(st_krozisce)
        sosednja_krozisca = zemljevid[st_krozisce]
        for sosednje_krozisce in sosednja_krozisca:
            krozisca.add(sosednje_krozisce)

    for prvotno_krizisce in prvotna_krozisca:
        krozisca.discard(prvotno_krizisce)
    return krozisca
