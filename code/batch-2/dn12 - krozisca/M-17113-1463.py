...
def preberi(ime_datoteke):
    stev = 1
    slovar={}
    for numbers in open(ime_datoteke):
        list = []
        ...
        slovar[stev] = list[mesto:]+list[:mesto]
        stev += 1
    return slovar
...
def mozna_pot(pot, zemljevid):
    ...
    for a,b in zip(pot,pot[1::]):
        if b not in zemljevid[a]:
            return False
        if len(zemljevid[b]) == 1:
            InOut = False
        if len(zemljevid[b]) != len(zemljevid[a]) == 1 and InOut == False:
            return False
    return True
    ...
    for a in pot:
        if a in Želist:
            ...
            if(len(zemljevid[a]) > 1):
                Želist.append(a)
    for a in zemljevid:
        if len(zemljevid[a]) > 1 and a not in Želist:
            return False
    return True
...
def prevozi(zacetek, navodila, zemljevid):
    kazalec = zacetek
    list = [kazalec]
    kazalecPrej = kazalec #vzame prejšnjo križišče
    kazalec = zemljevid[kazalec][0] #pokaže na nasledno ki je edina v vhodu
    list.append(kazalec)

    for korak in navodila:
        korak = korak + zemljevid[kazalec].index(kazalecPrej)
        if korak > len(zemljevid[kazalec])-1:
            korak -= len(zemljevid[kazalec])

        kazalecPrej = kazalec
        kazalec = zemljevid[kazalec][korak]
        list.append(kazalec)
...
def sosedi(doslej, zemljevid):
    ...
    for a in doslej:
        for b in zemljevid[a]:
            if b not in list and b not in doslej:
                list.add(b)
    return list
...
def razdalja(x, y, zemljevid):
    ...
    value = 0
    while y not in list:
        for a in sosedi(list,zemljevid):
            list.add(a)
        value += 1
    return value
...
def najkrajsa_navodila(x, y, zemljevid):
    ...
    def pot(zacetek,konec,zemljevid): #naredi nov dictionary ki ima samo en occurance vozlišč v seznamih
        def zafili(list, dic): #polne dictionary (vem... ne optimizirano)
            seznam = set()
            sez = list[:]
            for a in sez:
                if a not in dic:
                    dic[a] = []
                for b in zemljevid[a]:
                    if b not in seznam and b not in list:
                        list.append(b)
                        seznam.add(b)
                        dic[a].append(b)
            return dic,list
        #To bi moralo zapolniti nov dictionary z vrednostmi ki se ne ponavljajo
        #gre po znanem listu in po seznamu teh stevilk in se upraša če že obstaja v zeznamu poznanih in ze obstaja v seznamu krožišč
        def reverselookup(zacetek,konec,dic):
            pot = [konec]
            key = konec
            while key != zacetek:
                for a in dic:
        ...
        dic = {zacetek:zemljevid[zacetek]}
        list = []
        list.append(zacetek)
        list.append(dic[zacetek][0])

        while konec not in list:
            dic,list = zafili(list, dic)

        return reverselookup(zacetek, konec,dic)

    return navodila(pot(x,y,zemljevid)[::-1],zemljevid)
