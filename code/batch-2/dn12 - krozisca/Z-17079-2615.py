...
def preberi(ime_datoteke):
    pot = {}
    vrstica = 1
    for value in open(ime_datoteke):
        max = []
        ostali = []
        a = False
        tocke = [int(x) for x in value.split()]
        najmanjsi = min(tocke)
        for element in tocke:
            if element == najmanjsi:
                a = True
            if a:
                ostali.append(element)
            else:
                max.append(element)
        ostali.extend(max)
        pot[vrstica] = ostali
...
def hamiltonova(pot, zemljevid):
    ...
    for x in pot:
        if x not in unikati:
            unikati.append(x)
    if len(pot) > len(unikati):
        return False
    dolzina = [len(pot) for pot in zemljevid.values()]
    for el in dolzina:
        if el <= 1:
            c.append(el)
    if len(pot) == len(zemljevid) - len(c) + 2:
        return True
    else:
        return False
