...
def preberi(ime_datoteke):
    ...
    for vrstica in open(ime_datoteke, "r"):
        vrednost = list(map(int, vrstica.split()))
        while vrednost[0] != min(vrednost):
            vrednost.append(vrednost.pop(0))
        poti[st] = vrednost
        st += 1
    return poti
...
def mozna_pot(pot, zemljevid):
    ...
    for i in range(len(pot)-1):
        ...
        if vred not in zemljevid[kljuc]:
            return False

        if i != 0 and len(zemljevid[kljuc]) == 1:
            return False
...
def hamiltonova(pot, zemljevid):
    ...
    for x in range(len(pot)-1):
        ...
        if len(zemljevid[v1]) == 1:
            zac_konc.append(v1)
        if len((zemljevid[v2])) == 1:
            zac_konc.append(v2)
        if v2 in zemljevid[v1]:
            veljajo[v2] -= 1
...
def navodila(pot, zemljevid):
    ...
    for i in range(1, len(pot)-1):
        ...
        dol_list = len(zemljevid[kl])
        prej_index = zemljevid[kl].index(pot[i-1])
        val_index = zemljevid[kl].index(val)

        if prej_index <= val_index:
            zavoji.append(val_index - prej_index)
        # elif prej_index == val_index:
        #     zavoji.append(0)
        else:
            zavoji.append(dol_list - prej_index + val_index)

    return zavoji

def prevozi(zacetek, navodila, zemljevid):
    pot = [zacetek]
    print(zacetek, navodila, zemljevid)
    print(pot)
