...
def preberi(ime_datoteke):
    with open(ime_datoteke, "r") as vrstice:
        zemljevid = []
        for vrstica in vrstice:
            zemljevid.append(vrstica)

    stevec = 1
    slovar = {}

    for krozisce in zemljevid:
        izvozi = [int(x) for x in krozisce.replace('\n', '').split(' ')]
        if len(izvozi) < 2:
            slovar[stevec] = izvozi
        else:
            uvoz           = min(izvozi)
            i              = izvozi.index(uvoz) + 1
            urejeni_izvozi = [uvoz]
            ...
            while i < len(izvozi):
                if izvozi[i] != uvoz and (i + 1) == len(izvozi) and izvozi[0] != uvoz:
                    urejeni_izvozi.append(izvozi[i])
                    i = 0
                if izvozi[i] != uvoz and (i + 1) == len(izvozi) and izvozi[0] == uvoz:
                    urejeni_izvozi.append(izvozi[i])
                    break
    ...
    for i in range(len(pot)):
        if (i + 1) == len(pot):
            ...
            if i == 0 and len(zemljevid[pot[i]]) > 1:
                return False
            if i != 0 and len(zemljevid[pot[i]]) < 2:
                return False
    ...
    for i in range(len(pot)):
        if i == 0:
            ...
            if (i + 1) == len(pot):
                ...
                n = 0
                c = zemljevid[pot[i]].index(pot[i - 1])

                while 1:
                    if c == len(zemljevid[pot[i]]):
                        c = 0
                    if c == zemljevid[pot[i]].index(pot[i + 1]):
                        break
                    n += 1
                    c += 1
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    while i <= len(navodila):
        if i == 0:
            krozisca.append(zacetek)
            krozisca.append(zemljevid[zacetek][0])
        else:
            n = 0
            c = zemljevid[krozisca[len(krozisca) - 1]].index(krozisca[len(krozisca) - 2])

            while n < navodila[i - 1]:
                c += 1
                n += 1
                if c == len(zemljevid[krozisca[len(krozisca) - 1]]):
                    c = 0

            krozisca.append(zemljevid[krozisca[len(krozisca) - 1]][c])
        i += 1
...
def sosedi(doslej, zemljevid):
    ...
    for krozisce in doslej:
        for izvoz in zemljevid[krozisce]:
            if izvoz not in sosedi and izvoz not in doslej:
                sosedi.add(izvoz)
...
def razdalja(x, y, zemljevid):
    ...
    i += 1
    while 1:
        mreza[i] = sosedi(mreza[i - 1], zemljevid)
        if y in mreza[i]:
            break
        i += 1
