...
def preberi(ime_datoteke):
    with open(ime_datoteke, 'r') as inline:
        list_dict = {}
        number = 0
        for line in inline:
            number += 1
            transformed_ = list(map(int, line.split()))
            mini = min(transformed_)
            while transformed_[0] != mini:
                transformed_.insert(0, transformed_.pop())
            list_dict[number] = transformed_
        return list_dict


def mozna_pot(pot, zemljevid):
    if len(zemljevid[pot[0]]) != len(zemljevid[pot[-1]]):
        return False
    for pot1, pot2 in zip(pot, pot[1:]):
        if (pot2 not in zemljevid[pot1]) or pot1 == pot2:
            return False
    for i in pot[1:-1]:
...
def hamiltonova(pot, zemljevid):
    if not mozna_pot(pot, zemljevid):
        return False
    primerjava = [x for x in zemljevid if len(zemljevid[x]) != 1]
    sez = []
    for potek in pot:
        if potek not in sez:
            sez.append(potek)
            ...
            return False
    if set(primerjava).issubset(sez):
        return True


def uredi_seznam(inc_node, exit_node, zemljevid):
    seznam = zemljevid[inc_node][:]
    if len(seznam) > 1:
        while seznam[0] != exit_node:
            seznam.insert(0, seznam.pop())
    return seznam

def navodila(pot, zemljevid):
    instructions = []
    previous_entry = pot[0]
    for pot1, pot2 in zip(pot[1:], pot[2:]):
        urejen_seznam = uredi_seznam(pot1, previous_entry, zemljevid)
        previous_entry = pot1
        instructions.append(urejen_seznam.index(pot2))
    return instructions


def prevozi(zacetek, navodila, zemljevid):
    zaporedja = [zacetek, zemljevid[zacetek][0]]
    for ukaz in navodila:
        helper = zemljevid[zaporedja[-1]]
        while helper[0] != zaporedja[-2]:
            helper.insert(0, helper.pop())
        if len(helper) < 2:
            zaporedja.extend(helper)
        else:
            zaporedja.append(helper[ukaz])
    return zaporedja


def sosedi(doslej, zemljevid):
    dozdaj = set()
    for member in doslej:
        for adder in zemljevid[member]:
            dozdaj.add(adder)
    return dozdaj.difference(doslej)
