...
def preberi(ime_datoteke):
    ...
    for vrstica in dat:
        i += 1
        povezave = str.split(vrstica)
        povezave = [int(x) for x in povezave]
        while povezave[0] != min(povezave):
            povezave.append(povezave.pop(0))
        zemljevid[i] = povezave
    dat.close()
    return zemljevid
...
def mozna_pot(pot, zemljevid):
    ...
    for i in range(1, len(pot)):
        if pot[i] not in zemljevid[pot[i-1]]:
            return False
        if i != len(pot) - 1 and len(zemljevid[pot[i]]) == 1:
            return False
    return True

def hamiltonova(pot, zemljevid):
    krozisca = [x for x in zemljevid.keys() if len(zemljevid[x]) > 1]
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    for smer in navodila:
        pot.append(zemljevid[pot[-1]][(zemljevid[pot[-1]].index(pot[-2]) + smer) % len(zemljevid[pot[-1]])])
    return pot

def sosedi(doslej, zemljevid):
    vsi = set()
    for lokacija in doslej:
        for povezava in zemljevid[lokacija]:
            if povezava not in doslej:
                vsi.add(povezava)
    return vsi
...
def razdalja(x, y, zemljevid):
    ...
    while y not in vsi:
        i+=1
        vsi = vsi.union(sosedi(vsi, zemljevid))
    return i
