    ...
    for i in range(len(lines)):
        ...
        for j in range(len(lines[i])):
            if lines[i][j] != " ":
                ...
                tabelca.append(int(niz))
                niz = ""
        tabelca.append(int(niz))
        minimum = tabelca[0]
        indeks = 0
        for g in range(len(tabelca)):
            if minimum > tabelca[g]:
                minimum = tabelca[g]
                indeks = g
        for o in range(indeks):
            x = tabelca[0]
            tabelca.pop(0)
            tabelca.append(x)
        dvodim.append(tabelca)
    slovar = {}
    for i in range(len(dvodim)):
        slovar[i + 1] = dvodim[i]
    return slovar
...
def mozna_pot(pot, zemljevid):
   ...
   while i in zemljevid:
    ...
    #zacne/konca
   if pot[-1] not in start:
       return False
   if pot[0] not in start:
       return False
...
def hamiltonova(pot, zemljevid):
   ...
   for i in range(len(zemljevid)):
       tabel.append(i + 1)
   tabeldve = set(pot)
   if len(tabeldve) != len(pot):
       return False
   if len(tabeldve) == (len(tabel) - len(tab) + 2):
       return True
   return False
