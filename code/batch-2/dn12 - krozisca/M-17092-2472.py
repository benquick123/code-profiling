...
def mozna_pot(pot, zemljevid):
    ...
    while(i < len(pot)):
        if i != 0 and i != len(pot)-1 and len(zemljevid[pot[i]]) == 1:
            return False
        if i != len(pot)-1 and pot[i] == pot[i+1]:
            return False
        if i != len(pot)-1 and pot[i+1] not in zemljevid[pot[i]]:
            return False
        i+=1
...
def hamiltonova(pot, zemljevid):
    ...
    if len(pot) != len(set(pot)):
        return False
    for kr in zemljevid:
        if len(zemljevid[kr]) != 1 and kr not in pot:
            return False
    return True
