...
def preberi(ime_datoteke):
    ...
    i=1
    for vrstica in dat:
        krizisca[i] = preobrni([int(x) for x in vrstica.split("\n")[0].split()],"m")
        i+=1
    return krizisca
...
def mozna_pot(pot,zemljevid):
    for i in range(1, len(pot)):
        if i == len(pot)-1:
            if len(zemljevid[pot[0]]) != 1 or len(zemljevid[pot[i]]) != 1:
                return False
        else:
...
def hamiltonova(pot, zemljevid):
    if(not mozna_pot(pot,zemljevid) or len(pot) != len(set(pot)) or  len(pot[1: -1])!= len([zemljevid for i in zemljevid if(len(zemljevid[i]) > 1)])):
        return False
    return True
...
def navodila(pot, zemljevid):
    ...
    for i in range(1, len(pot)-1):
        tmp.append((zemljevid[pot[i]].index(pot[i+1])-zemljevid[pot[i]].index(pot[i-1]))%len(zemljevid[pot[i]]))
    return tmp
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    tmp.append(zacetek)
    for k in zemljevid:
        if zacetek in zemljevid[k] and len(zemljevid[k]) > 1:
            break
    tmp.append(k)
    for e in navodila:
        kprej = tmp[i-1]
        k = shift(preobrni(zemljevid[k],kprej), e)[0]
        tmp.append(k)
        i+=1
    return tmp
...
def sosedi(doslej, zemljevid):
    tmp = set()
    for e in doslej:
        for k in zemljevid[e]:
            if k not in tmp and k not in doslej:
                tmp.add(k)
    return tmp
...
def razdalja(x, y, zemljevid):
    ...
    while y not in tmp:
        i+=1
        tmp.update(sosedi(tmp, zemljevid))
    return i
...
def najkrajsa_navodila(x, y, zemljevid):
    ...
    pot = [x]
    while y not in pot:
        for sosed in sosedi(pot, zemljevid):
