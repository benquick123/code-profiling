...
def obrni(izvozi):
    razporejeni = []
    najmanjsi = 10000
    for element in izvozi:
        if element < najmanjsi:
            najmanjsi = element
    for element in izvozi:
        if element == najmanjsi:
            razporejeni.append(element)
        elif razporejeni:
            razporejeni.append(element)
    if len(razporejeni) != len(izvozi):
        for element in izvozi:
            if element != razporejeni[0]:
                razporejeni.append(element)
            else:
                break
    return razporejeni
...
def preberi(ime_datoteke):
    krozisca = {}
    opis = open(ime_datoteke)
    for i, krozisce in enumerate(opis.readlines()):
        krozisce = str(krozisce.rstrip())
        krozisca[i+1] = []

        if len(krozisce) > 1:
            krozisce = krozisce.split(" ")
            spominski_stevec = list(range(len(krozisce)))
            for s, k in enumerate(krozisce):
                k = int(k)

                krozisca[i+1].append(k)
            if len(krozisca[i + 1]) == len(krozisce):
                krozisca[i + 1] = obrni(krozisca[i + 1])
        else:
            krozisca[i+1].append(int(krozisce))
    return krozisca
...
def mozna_pot(pot, zemljevid):
    for i, korak in enumerate(pot):
        k = len(zemljevid[pot[0]])
        k1 = len(zemljevid[pot[-1]])
        if k != 1 or k1 != 1:
            return False
        if i != 0 and i != len(pot)-1:
            ...
            if korak == pot[i-1]:
                return False
        if i != 0 and korak not in zemljevid[pot[i-1]]:
            return False
    return True
def hamiltonova(pot, zemljevid):
    ...
    for krozisce in zemljevid:
        if len(zemljevid[krozisce]) == 1:
            izhod += 1
    if len(kolikokrat) != len(zemljevid)-(izhod - 2):
        k = len(kolikokrat)
        k1 = len(zemljevid)
        return False
    else:
...
def navodila(pot, zemljevid):
    to_do = []
    for i, korak in enumerate(zip(pot, pot[1:])):
        koliko_izvozov = len(zemljevid[korak[0]])
        if koliko_izvozov != 1:
            razdalja = (zemljevid[korak[0]].index(korak[1])-zemljevid[korak[0]].index(pot[i-1]))%koliko_izvozov
            to_do.append(razdalja)
    return to_do
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    for navodilo in navodila:
        ...
        if len(zemljevid[prejsnji]) == 1:
            naslednji = zemljevid[prejsnji][0]
            pot.append(naslednji)
        for i, krozisce in enumerate(zemljevid[naslednji]):
            if len(zemljevid[naslednji])==1:
                break
            if krozisce == prejsnji:
                korak = zemljevid[naslednji].index(krozisce)+navodilo
                if korak > len(zemljevid[naslednji])-1:
                    korak = korak - len(zemljevid[naslednji])
                prejsnji = naslednji
                naslednji = zemljevid[naslednji][korak]
                pot.append(naslednji)
    return pot
def sosedi(doslej, zemljevid):
    zraven = set()
    for vozlisce in doslej:
        for krozisce in zemljevid[vozlisce]:
            zraven.add(krozisce)
    return zraven-doslej
def razdalja(x, y, zemljevid):
    stevec = 1
    if not y in zemljevid[x]:
        for krozisce in zemljevid[x]:
            stevec += razdalja(krozisce, y, zemljevid)
    else:
        return stevec
...
def razdalja(x, y, zemljevid):
    zraven = {x}
    stevec = 1
    while y not in zraven:
        for krozisca in zraven:
            vsi_sosedi = sosedi(zraven, zemljevid)
        for sosed in vsi_sosedi:
            zraven.add(sosed)
        if y not in zraven:
            stevec += 1
    return stevec
...
class Test06(unittest.TestCase):
    ...
    def test_03_hamiltonova(self):
        ...
        self.assertTrue(hamiltonova([1, 3, 7, 8, 16, 9, 14, 11, 6, 4, 5, 10, 13, 12][
                                    ::-1], zemljevid))
