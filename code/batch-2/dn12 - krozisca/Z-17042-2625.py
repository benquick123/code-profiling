...
def preberi(ime_datoteke):
    ...
    for i in range(len(vrstice)):
        seznam = []
        for j in vrstice[i].split():
            seznam.append(int(j))
        minind = seznam.index(min(seznam))
        seznam = seznam[minind:] + seznam[:minind]
        slovar[i+1]=seznam
    return slovar

def mozna_pot(pot, zemljevid):
    if len(pot) <= 2: #For zanka ne gleda koncev. Če pot vsebuje le konce tako ali tako ne more biti povezana
        return False
    ...
    if len(pot) == len(set(pot)):
        ...
        for i in zemljevid.keys():
            if len(zemljevid[i]) > 1:
                krozisca.add(i)
        presek = krozisca & set(pot)
        if len(presek) == len(krozisca):
...
def navodila(pot, zemljevid):
    seznam=[]
    for i in range(len(pot)-2):
        razlika = zemljevid[pot[i+1]].index(pot[i+2]) - zemljevid[pot[i+1]].index(pot[i])
        seznam.append(razlika % len(zemljevid[pot[i+1]]))
    return seznam
