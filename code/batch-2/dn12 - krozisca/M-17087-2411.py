...
def preberi(ime_datoteke):
    ...
    for line in txt:
        ...
        if dolzina > 1:
            oc = line.split()
            for c in oc:
                slovar[stevc].append(int(c))
            stevc += 1
        else:
            o = int(line)
            slovar[stevc].append(o)
            stevc += 1

    nov_slovar = {}
    slovar_stevc = 1
    for vred in slovar.values():
        nov_s = []
        im = vred.index(min(vred))
        for h in range(2):
            while im < len(vred):
                if vred[im] not in nov_s:
                    nov_s.append(vred[im])
                im += 1
            im = 0
        nov_slovar[slovar_stevc] = nov_s
        slovar_stevc += 1
...
def mozna_pot(pot,zemljevid):
    ...
    for a in pot:
        if a == b:
            return False
        b = a
    if len(zemljevid[zacetek]) != 1:
        return False
    if len(zemljevid[konec]) != 1:
        return False
    for c,d in enumerate(pot):
        if c+1 < len(pot):
            if pot[c+1] not in zemljevid[d]:
                return False
    for i in pot:
        if i != pot[0] and i != pot[-1]:
