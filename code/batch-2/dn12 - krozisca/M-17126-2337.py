...
def preberi(ime_datoteke):
    ...
    for line in file:
        ...
        for element in splitted_line:
            if count not in dictionary_map:
                dictionary_map[count] = list()
                dictionary_map[count].append(int(element))
            else:
                dictionary_map[count].append(int(element))
        count += 1

    for i in range(1, len(dictionary_map) + 1):
        dictionary_map[i] = start_with_small(dictionary_map[i])
    file.close()
...
def mozna_pot(pot, zemljevid):
    for i in range(1, len(pot)):
        if pot[i] not in zemljevid[pot[i - 1]]:
            return False
        elif len(zemljevid[pot[0]]) > 1 or len(zemljevid[pot[len(pot) - 1]]) > 1:
            return False
        elif pot[i - 1] == pot[i]:
            return False
...
def hamiltonova(pot, zemljevid):
    if not mozna_pot(pot, zemljevid):
        ...
        for element_map in zemljevid:
            if len(zemljevid[element_map]) > 1:
                if element_map not in pot:
                    return False
        check_list = []
        for element_route in pot:
            if element_route not in check_list:
                check_list.append(element_route)
            else:
                return False
    return True
...
def navodila(pot, zemljevid):
    if mozna_pot(pot, zemljevid):
        instructions = []
        for i in range(1, len(pot) - 1):
            c = zemljevid[pot[i]].index(pot[i + 1]) - zemljevid[pot[i]].index(pot[i - 1])
            instructions.append(c % (len(zemljevid[pot[i]])))
        return instructions
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    path.append(zacetek)
    coming_from = zacetek
    starting_roundabound = zemljevid[zacetek][0]
    path.append(starting_roundabound)
    for element in navodila:
        actual_roundabound = zemljevid[starting_roundabound][(zemljevid[starting_roundabound].index(coming_from) + element) % (len(zemljevid[starting_roundabound]))]
        coming_from = starting_roundabound
        starting_roundabound = actual_roundabound
        path.append(starting_roundabound)
    return path
...
def sosedi(doslej, zemljevid):
    connections = set()
    for connecting_point in doslej:
        for connection in zemljevid[connecting_point]:
            if connection not in doslej and connection not in connections:
...
def razdalja(x, y, zemljevid):
    ...
    set_point = set()
    set_point.add(x)
    network = sosedi(set_point, zemljevid)
    while y not in network:
        network = sosedi(network, zemljevid)
        count += 1
    return count
