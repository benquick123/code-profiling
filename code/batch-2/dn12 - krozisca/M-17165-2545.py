...
def preberi(ime_datoteke):
    ...
    i = 1
    for vrstica in f:
        bula = [int(x) for x in vrstica.strip().split()]
        while min(bula) != bula[0]:
            x= bula.pop(0)
            bula.append(x)
...
def mozna_pot(pot,zemljevid):
    ...
    i = 0
    for x0,x1 in zip(pot,pot[1:]):

        if x1 not in zemljevid[x0] or x1 == x0 or (len(zemljevid[x1]) == 1 and i != len(pot)-2):

            return False
        i += 1

    return True and prvazadna
...
def hamiltonova(pot,zemljevid):
    ...
    if len(pot) == len(zemljevid)- izhodi:
        vse = True

    if len(kopija) == len(pot):
        ponavljanje = True

    return mozna_pot(pot,zemljevid) and ponavljanje and vse
