...
def preberi(ime_datoteke):
    r = open(ime_datoteke, "r")
    slovar = {}
    stevec = 1
    for krožišče in r:
        krožišče = krožišče.strip()
        krožišče = krožišče.split()
        krožišče = list(map(int,krožišče))
        if(krožišče[0] == min(krožišče)):
            slovar[stevec] = krožišče
            stevec += 1
        else:
            krožišče = deque(krožišče)
            while(True):
                krožišče.rotate(1)
                if(krožišče[0] == min(krožišče)):
                    krožišče = (list(deque(krožišče)))
                    slovar[stevec] = krožišče
                    stevec += 1
                    break
    return(slovar)
...
def mozna_pot(pot,zemljevid):
    dolzina = len(zemljevid)
    primerjava = []
    slovar2 = {}
    while(dolzina > 0):
        prva = zemljevid[dolzina]
        primerjava.append(prva[0])
        primerjava.append(prva[-1])
        slovar2[dolzina] = 0
        dolzina -= 1
    dolzina = len(zemljevid)
    while(dolzina > 0):
        for vse in zemljevid[dolzina]:
            slovar2[vse] = slovar2[vse] + 1
        dolzina -= 1
    prva_vrednost = pot[0]
    zadnja_vrednost = pot[-1]
    Prva_zemljevid = slovar2.get(prva_vrednost)
    zadnja_zemljevid = slovar2.get(zadnja_vrednost)
    pot2 = pot
    dolzina = len(pot)
    if(dolzina == 3):
        if(pot[1] == 3):
    ...
    stevec = pot[0]
    for vse in pot[1:]:
        nekaj = zemljevid[stevec]
        if(vse not in nekaj):
            return(False)
        else:
            stevec = vse
    if(Prva_zemljevid == 1):
        if(zadnja_zemljevid == 1):
            return(True)
        else:
            return(False)
    else:
...
def koncni(pot,zemljevid):
    ...
    for vse in slovar2:
        if(slovar2[vse] == 1):
            seznam_koncnih.append(vse)
    return(seznam_koncnih)

def hamiltonova(pot,zemljevid):
    pot2 = pot.copy()
    koncno = koncni(pot,zemljevid)
    pogoj = mozna_pot(pot,zemljevid)
    if(pogoj == False):
        return(False)
    dolzina = len(zemljevid)
    primerjava = {}
    while (dolzina > 0):
        primerjava[dolzina] = 0
        dolzina -= 1
    for vse in pot2:
        if(vse in primerjava):
            primerjava[vse] += 1
    dolzina_koncno = len(koncno)
    napaka = 0
    for vse in primerjava:
        if(primerjava[vse] == 0):
            for drugo in koncno:
                if(vse == drugo):
                    napaka = 0
