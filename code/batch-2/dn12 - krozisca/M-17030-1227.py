    ...
    for x in vsebina:
        ...
        for k in sosednja:
            if k==' ':
                ...
                k = int(k)
                sosede.append(k)
        slovar_krizisc[krizisce]=sosede
        krizisce=krizisce+1
    for kluc in slovar_krizisc:
        vsebina_kljuca=slovar_krizisc[kluc]
        for stevilo in vsebina_kljuca:
            list_vseh_stevil=vsebina_kljuca
            stevilo=list_vseh_stevil[0]
            minimum=min(vsebina_kljuca)
            if stevilo!=minimum and not list_vseh_stevil[0]==minimum:
                list_vseh_stevil.remove(stevilo)
                list_vseh_stevil.append(stevilo)
            else:
                slovar_krizisc[kluc]=list_vseh_stevil
                aja=True
                break
    return slovar_krizisc
def mozna_pot(pot, zemljevid):
    i=1
    seznam_koncev_in_zacetkov=[]
    stevilo_elementov={}
    for x,vsebina in zemljevid.items():
        stevilo_elementov[x]=len(vsebina)
        if stevilo_elementov[x]==1:
            seznam_koncev_in_zacetkov.append(x)
    for krozisce in pot:
        naslednje_krizisce = pot[i]
        i = i + 1
        koncnu_krizisce = pot[-1]
        sosedi = zemljevid[krozisce]
        if pot[0] not in seznam_koncev_in_zacetkov:
            return False
        if naslednje_krizisce in sosedi:
            if naslednje_krizisce == koncnu_krizisce:
                if naslednje_krizisce in seznam_koncev_in_zacetkov:
                    return True
                else:
                    return False
            if naslednje_krizisce in seznam_koncev_in_zacetkov:
...
def hamiltonova(pot, zemljevid):
    ...
    for x, vsebina in zemljevid.items():
        ...
        if stevilo_elementov[x] == 1:
            seznam_koncev_in_zacetkov.append(x)
    dolzina_zacetka_konca=len(seznam_koncev_in_zacetkov)-2
    dolzina_poti=len(pot)
    vsa_krozisca=len(zemljevid)-dolzina_zacetka_konca
    if dolzina_poti==vsa_krozisca:
        al_je_al_nej=mozna_pot(pot,zemljevid)
        return al_je_al_nej
    else:
        return False
def navodila(pot, zemljevid):
    ...
    for p in pot:
        q=q+1
        if gremo_naprej==True:
            if pot[i] == krozisce[0]:
                stevec_za_wn = 0
                i = i + 1
                naslednje_krizisce = pot[i + 1]
                for izhod in krozisce:
                    stevec_za_wn = stevec_za_wn + 1
                    if izhod == naslednje_krizisce:
                        wn = stevec_za_wn
                        wn = wn - 1
                        if wn != 0:
                            seznam_navodil.append(wn)
        if q==dolzina_poti:
            if seznam_navodil==[]:
                seznam_navodil=[0]
            ...
            if p==pot[0]:
                ...
                gremo_naprej=False
                if gremo_naprej==False:
                    krozisce=zemljevid[p]
                if pot[i]==krozisce[0] :
                    stevec_za_wn = 0
                    i = i + 1
                    naslednje_krizisce = pot[i+1]
                    ...
                    for l in krozisce:
                        l = krozisce[0]
                        if minimum!=krozisce[0] and l!=minimum:
                            krozisce.remove(l)
                            krozisce.append(l)
                            gremo_naprej=True
