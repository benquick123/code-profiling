...
def preberi(ime_datoteke):
    ...
    for i in datoteka.readlines():
        i = i.strip()
        novline = i.split()
        novlist = list(map(int, novline))
        while st < len(novlist):
            if novlist[st] == min(novlist):
                break
            else:
                novlist.append(novlist[st])
                novlist.remove(novlist[st])
        slovar[x] = novlist
        x += 1
    return slovar
...
def mozna_pot(pot, zemljevid):
    ...
    st2 = 0
    stanje = True
    if len(zemljevid[pot[0]]) == 1 and len(zemljevid[pot[::-1][0]]) == 1:
        stanje = True
    else:
        stanje = False
    while st < len(pot)-1:
        if len(zemljevid[pot[st]]) == 1:
            stanje = False
        st+=1
    for i in range(len(pot)-1):
        if pot[i] == pot[i+1]:
            stanje = False
    while st2 < len(pot)-1:
        if pot[st2] not in zemljevid[pot[st2+1]]:
            stanje = False
        st2+=1
    return stanje
...
def navodila(pot, zemljevid):
    ...
    st = 0
    while st + 2 < len(pot):
        terka = (pot[st], pot[st+1], pot[st+2])
        seznam.append((zemljevid[terka[1]].index(terka[2]) - zemljevid[terka[1]].index(terka[0]))%len(zemljevid[terka[1]]))
        st += 1
    return seznam
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    st = 1
    st1 = 0
    while st1 < len(navodila):
        seznam.append(zemljevid[seznam[st]][(zemljevid[seznam[st]].index(seznam[st1])+navodila[st1])%len(zemljevid[seznam[st]])])
        st += 1
        st1 += 1
