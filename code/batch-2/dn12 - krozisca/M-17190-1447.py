...
def uredi(array):
    x = array.index(min(array))
    return array[x:] + array[:x]

def preberi(ime_datoteke):
    t = []
    x = []
    file = open(ime_datoteke).read().split('\n')
    for vrstica in file:
...
def hamiltonova(pot, zemljevid):
    krajisca = len(zemljevid)
    for i in range(len(zemljevid)):
        if len(zemljevid[i + 1]) == 1:
            krajisca -= 1
    if krajisca == len(pot) - 2 and mozna_pot(pot, zemljevid):
        return True
    else:
        return False
...
def prevozi(zacetek, navodila, zemljevid):
    t = [zacetek, zemljevid[zacetek][0]]
    krizisce = zemljevid[zacetek][0]
    i = 1
    for e in navodila:
        diff = ((zemljevid[krizisce].index(zacetek)) + e) % len(zemljevid[krizisce])
        krizisce = zemljevid[krizisce][diff]
        t.append(krizisce)
        zacetek = t[i]
        i += 1
    return t
