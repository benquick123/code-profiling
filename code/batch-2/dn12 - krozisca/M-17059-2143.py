...
def preberi(ime_datoteke):
    poti = open(ime_datoteke)
    a =dict()
    counter = 1
    for pot in poti:
        pot = pot.strip()
        pot = pot.split()
        pot = list(map(int, pot))
        b = pot[pot.index(min(pot)):]
        c = pot[:pot.index(min(pot))]
        a[counter] =  b + c
        counter += 1
    return  a
...
def mozna_pot(pot, zemljevid):
    ...
    for (idx,krizise1), krizise2 in zip(enumerate(pot),pot[1:]):
        if krizise1 not in zemljevid[krizise2]:
            return False
        if len(zemljevid[krizise2]) == 1 and idx != len(pot[1:]) - 1:
            return False
    return True
...
def hamiltonova(pot, zemljevid):
    ...
    if len(unikati(pot)) != len(pot):
        return False
    counter = 0
    cc = 0
    for potka in zemljevid:
        if len(zemljevid[potka]) == 1:
            counter += 1
            if counter > 2:
                cc += 1
    if len(zemljevid) - cc != len(unikati(pot)):
        return False
    return True

def navodila(pot, zemljevid):
    gg = []
    for enka, dvojka, trojka in zip(pot, pot[1:], pot[2:]):
        a = list(zemljevid[dvojka])
        a = a[a.index(enka):] + a[:a.index(enka)]
        counter = 0
        for i in a:
            if a[counter] == trojka:
                gg.append(counter)
...
def prevozi(zacetek, navodila, zemljevid):
    stev = zemljevid[zacetek][0]
    gg = [zacetek, stev]
    krozise = zemljevid[zemljevid[zacetek][0]]
    for n in navodila:
        a = krozise[krozise.index(zacetek):] + krozise[:krozise.index(zacetek)]
        gg.append(a[n])
        zacetek = stev
        krozise = zemljevid[a[n]]
        stev = a[n]
    return gg
