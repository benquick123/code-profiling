...
def preberi(ime_datoteke):
    ...
    for ključ in d:
        najmanjši = min(d[ključ])
        indeks_najmanjšega = d[ključ].index(najmanjši)
        d[ključ] = d[ključ][indeks_najmanjšega:] + d[ključ][:indeks_najmanjšega]
...
def hamiltonova(pot, zemljevid):
    if not mozna_pot(pot, zemljevid):
        ...
        for ključ in zemljevid:
            if ključ not in pot and len(zemljevid[ključ]) > 1 or pot.count(ključ) > 1:
                return False
    return True
...
def navodila(pot, zemljevid):
    ...
    for prvi, drugi ,tretji in zip(pot,pot[1:],pot[2:]):
        seznamdrugi = zemljevid[drugi]
        i1 = seznamdrugi.index(prvi)
        i2 = seznamdrugi.index(tretji)
        t.append((i2-i1) % len(seznamdrugi) )
    return t
...
def razdalja(x, y, zemljevid):
    ...
    doslej = sosedi(u, zemljevid)

    if y in doslej:
        return cnt

    for e in doslej:
        return razdalja(doslej, y, zemljevid) + 1
