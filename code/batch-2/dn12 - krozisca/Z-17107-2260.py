...
def preberi(ime_datoteke):
    ...
    i = 1
    for line in file:
        temporary = [int(num) for num in line.split("\n")[0].split(" ")]
        temporary = temporary[temporary.index(min(temporary)):] + temporary[0:temporary.index(min(temporary))]
        final_dict[i] = temporary
        i += 1
    file.close()
    return final_dict
...
def mozna_pot(pot, zemljevid):
    ...
    if len(zemljevid[pot[0]]) == 1 and len(zemljevid[pot[len(pot) - 1]]) == 1:
        ...
        return False
    for rondo in pot[1: len(pot) - 1]:
        if len(zemljevid[rondo]) != 1:
            pass
        else:
            return False
    i = 0
...
def hamiltonova(pot, zemljevid):
    if mozna_pot(pot, zemljevid):
        ...
        return False
    hamilton = []
    for i, rondo in zemljevid.items():
        if len(rondo) > 1:
            hamilton.append(i)
    already = []
    for num in pot:
...
def navodila(pot, zemljevid):
    ...
    i = 1
    while i < len(pot) - 1:
        index_1 = zemljevid[pot[i]].index(pot[i - 1])
        index_2 = zemljevid[pot[i]].index(pot[i + 1])
        if index_1 < index_2:
            result.append(index_2 - index_1)
        elif index_1 == index_2:
            result.append(0)
        else:
            result.append(len(zemljevid[pot[i]]) - index_1 + index_2)
        i += 1
    return result
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    for next in navodila:
        index = zemljevid[pot[-1]].index(pot[-2])
        if index + next < len(zemljevid[pot[-1]]):
            pot.append(zemljevid[pot[-1]][index + next])
        else:
            how_much = (index + next) - len(zemljevid[pot[-1]])
            pot.append(zemljevid[pot[-1]][how_much])
    return pot
