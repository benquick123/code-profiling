...
def mozna_pot(pot, zemljevid):
    if 1 < len([zemljevid[m] for m in pot if len(zemljevid[m]) == 1]) <= 2:
        for a, b in zip(pot, pot[1:]):
            if a == b:
                return False
            if a in zemljevid.keys():
                if b in zemljevid[a]:
                    continue
                else:
                    return False
...
def hamiltonova(pot, zemljevid):
    if len([pot[0]] + [m for m in zemljevid.keys() if len(zemljevid[m]) > 1] + [pot[-1]]) == len(pot):
...
def navodila(pot, zemljevid):
    rez = []
    for i, j, k in zip(pot, pot[1:], pot[2:]):
        zem = zemljevid[j][::]
        while zem[0] != i:
            temp = zem[0]
            zem.remove(temp)
            zem.append(temp)
        krozisca = list(enumerate(zem))
        for x, y in krozisca:
            if k == y:
                rez += [x]
    return rez
...
def prevozi(zacetek, navodila, zemljevid):
    tocka = zacetek
    rez = [tocka] + zemljevid[tocka]
    c = 1
    for i in navodila:
        mozne_poti = zemljevid[rez[c]]
        while rez[c - 1] != mozne_poti[0]:
            temp = mozne_poti[0]
            mozne_poti.remove(temp)
            mozne_poti.append(temp)
        b = mozne_poti[i]
        rez += [b]
        c += 1
...
def sosedi(doslej, zemljevid):
    ...
    for i in doslej:
        a |= set(zemljevid[i])
    [a.remove(k) for k in doslej if k in a]
    return a
