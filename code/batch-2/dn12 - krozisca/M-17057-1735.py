...
def preberi(ime_datoteke):
    ...
    for v in open(ime_datoteke).read().splitlines():
        spr = v.split()
        int_spr = [int(stevilo) for stevilo in spr]

        najmansa = min(int_spr)
        najmansa_indeks = int_spr.index(najmansa)

        for x in range(0, najmansa_indeks):
            seznam.append(int_spr[x])

        for y in seznam:
            int_spr.append(y)
            int_spr.remove(int_spr[0])
...
def mozna_pot(pot, zemljevid):
    ...
    for x in range(0, len(pot) - 1):
        if pot[x + 1] in zemljevid[pot[x]] and pot[x + 1] != zemljevid[pot[x]] and len(zemljevid[pot[0]]) == 1:
            i += 1

    for y in range(1, len(pot) - 1):
        if len(zemljevid[pot[y]]) == 1:
            i -= 1

    if i == len(pot) - 1 and len(zemljevid[pot[len(pot) - 1]]) == 1:
        return True
    else:
        return False
...
def hamiltonova(pot, zemljevid):
    ...
    for s in range(min(pot), max(pot)):
        if s in pot:
            i += 1
        if s not in pot and len(zemljevid[s]) != 1:
            i -= 1

    if mozna_pot(pot, zemljevid) and len(seznam) == len(pot) and i+1 == len(pot):
        return True
    else:
        return False
...
class Test08(unittest.TestCase):
    ...
    def test_01_prevozi(self):
        ...
        self.assertEqual(prevozi(15, [1, 2, 5, 2, 3, 2, 5, 2, 2], zemljevid),
                         [15, 16, 14, 11, 9, 16, 14, 11, 9, 16, 15])
