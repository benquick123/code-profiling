...
def zamenjaj(start_with, l):
    while start_with != l[0]:
        temp = l[0]
...
def preberi(ime_datoteke):
    ...
    with open(ime_datoteke) as f:
        for index, value in enumerate([line.strip() for line in f.readlines()]):
            zemljevid[index + 1].extend(map(lambda x: int(x), value.split()))
            while zemljevid[index + 1][0] != min(zemljevid[index + 1]):
                zemljevid[index + 1] = zamenjaj(min(zemljevid[index + 1]), zemljevid[index + 1])
        f.close()
    return zemljevid
...
def mozna_pot(pot, zemljevid):
    if len(zemljevid[pot[0]]) != 1 or len(zemljevid[pot[len(pot) - 1]]) != 1:
        return False
    for i, p in enumerate(zip(pot, pot[1:])):
        if p[1] not in zemljevid[p[0]] or len(zemljevid[p[1]]) == 1 and i != len(list(zip(pot, pot[1:]))) - 1:
            return False
    return True

def hamiltonova(pot, zemljevid):
    return len(pot) == len(zemljevid) - sum(map(lambda p: 1 if len(p) == 1 else 0, zemljevid.values())) + 2 \
           and mozna_pot(pot, zemljevid)
def navodila(pot, zemljevid):
    l = list(zip(pot, pot[1:]))
    navodila = []
    for index, value in enumerate(l):
        if index >= 1:
            z = zemljevid[l[index - 1][1]]
            while z[0] != l[index-1][0]:
                z = zamenjaj(l[index-1][0], z)
            navodila.append(z.index(value[1]))
    return navodila
...
def prevozi(zacetek, navodila, zemljevid):
    ...
    for i, navodilo in enumerate(navodila):
        pot.append(zamenjaj1(navodilo, zamenjaj(pot[i], zemljevid[pot[i + 1]]))[0])
    return pot


def sosedi(doslej, zemljevid):
    return {sosed for d in doslej for sosed in zemljevid[d]}.difference(doslej)

def razdalja(x, y, zemljevid):
    s = sosedi({x}, zemljevid)
    i = 1
    while y not in s:
...
def najkrajsa_navodila(x, y, zemljevid):
    ...
    kandidati = [y]
    od_kod = []
    for nasednji in kandidati:
        for naprej in zemljevid[nasednji]:
            if naprej not in kandidati:
                od_kod.append((naprej, nasednji))
