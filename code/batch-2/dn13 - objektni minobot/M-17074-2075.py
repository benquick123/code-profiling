...
class Minobot():
    def __init__(self):
        ...
        self.y = 0
        self.smer = 90
        self.data = [(self.x, self.y, self.smer)]

    def naprej(self, d):
        if self.smer % 360 == 0:
            self.y += d
        elif self.smer % 360 == 90:
            self.x += d
        elif self.smer % 360 == 180:
            self.y -= d
        else:
            self.x -= d
        self.update()
    ...
    def update(self):
        self.data.append((self.x, self.y, self.smer))

    def razveljavi(self):
        if len(self.data) > 1:
            self.data.pop(-1)
            self.x, self.y, self.smer = self.data[-1]
