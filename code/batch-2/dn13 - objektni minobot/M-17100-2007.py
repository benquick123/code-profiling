...
class Minobot:
    ...
    def naprej(self, d):
        dodatek_x = math.cos(math.radians(self.kot))
        dodatek_y = math.sin(math.radians(self.kot))

        if abs(dodatek_x) < 0.1 and abs(dodatek_x) > 0:
            dodatek_x = 0
        elif abs(dodatek_y) < 0.1 and abs(dodatek_y) > 0:
            dodatek_y = 0

        self.x = d*dodatek_x + self.x
        self.y = d*dodatek_y + self.y

    def desno(self):
        self.kot = self.kot - 90
