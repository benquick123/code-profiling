...
class Minobot:
    def __init__(self):
        ...
        self.y = 0
        self.angle = 0
        self.position = [(self.x, self.y, self.angle)]

    def naprej(self, d):
        if self.angle >= 360 or self.angle <= -360:
            phi = radians(self.angle % 360)
        else:
    ...
    def pomnilnik(self):
        self.position.append((self.x, self.y, self.angle))

    def razveljavi(self):
        if len(self.position) >= 2:
            self.position.pop(-1)
            self.x, self.y, self.angle = self.position[-1]
        if len(self.position) == 1:
            self.x, self.y, self.angle = self.position[0]
