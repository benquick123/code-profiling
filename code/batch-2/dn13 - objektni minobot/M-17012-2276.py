...
class Minobot:
    def __init__(self):
        self.tocka = (0, 0)
        self.zgodovina = []
        self.zgodovina.append(self.tocka)
        self.smer = [(1, 0), (0, 1), (-1, 0),(0, -1)]

    def naprej(self, d):
        self.zgodovina.append(self.tocka)
        x = self.tocka[0] + self.smer[0][0]*d
        y = self.tocka[1] + self.smer[0][1]*d
        self.tocka = (x, y)
        self.zgodovina.append(self.tocka)

    def desno(self):
        self.smer = self.smer[-1:] + self.smer[:-1]
        self.zgodovina.append("d")

    def levo(self):
        self.smer = self.smer[1:] + self.smer[:1]
        self.zgodovina.append("l")

    def koordinate(self):
        return self.tocka

    def razdalja(self):
        return abs(self.tocka[0])+abs(self.tocka[1])

    def razveljavi(self):
        if(len(self.zgodovina) > 1):
            if(self.zgodovina[-1] == "d"):
                self.zgodovina.pop()
                self.levo()
            elif(self.zgodovina[-1] == "l"):
                self.zgodovina.pop()
                self.desno()
            else:
                self.zgodovina.pop()
                self.tocka = self.zgodovina[-1]
            self.zgodovina.pop()
