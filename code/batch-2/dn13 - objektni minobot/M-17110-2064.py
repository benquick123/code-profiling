...
class Minobot():
    def __init__(self):
        ...
        self.y = 0
        self.angle = 0
        self.pozicije = [(self.x, self.y, self.angle)]

    def dodaj_pozicijo(self):
        self.pozicije.append((self.x, self.y, self.angle))

    def naprej(self, d):
        kot = math.radians(self.angle)
        self.x = round(self.x + d * math.cos(kot), 0)
        self.y = round(self.y + d * math.sin(kot), 0)
        self.dodaj_pozicijo()
    ...
    def razveljavi(self):
        st_pozicij = len(self.pozicije)
        self.x, self.y, self.angle = self.pozicije[st_pozicij-2]
        del self.pozicije[st_pozicij-1]
        if len(self.pozicije) == 0:
            self.pozicije = [(self.x, self.y, self.angle)]
...
class TestDodatna(unittest.TestCase):
    def test_undo(self):
        ...
        self.assertEqual(a.koordinate(), (3, -4))
        a.razveljavi()                                          # 2 nazaj
        self.assertEqual(a.koordinate(), (1, -4))
        a.naprej(1)                                             # 1 naprej
        self.assertEqual(a.koordinate(), (2, -4))
        a.razveljavi()                                          # 1 nazaj
        self.assertEqual(a.koordinate(), (1, -4))
        a.razveljavi()
