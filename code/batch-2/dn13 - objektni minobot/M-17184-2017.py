...
class Minobot:
    ...
    def naprej(self, d):
        self.ukazi.append(("N", d))
        x,y = 0,0
        dx, dy = self.premiki[self.smer]
        x += (dx * d) + self.postavitev[0]
        y += (dy * d) + self.postavitev[1]
        self.postavitev = (x, y)

    def desno(self):
        self.ukazi.append(("D", self.smer))
        self.smer = (self.smer + 1) % 4

    def levo(self):
        self.ukazi.append(("L", self.smer))
        self.smer = (self.smer - 1) % 4

    def koordinate(self):
        return self.postavitev

    def razdalja(self):
        return abs(self.postavitev[0])+abs(self.postavitev[1])
    ...
    def razveljavi(self):
        ...
        if self.ukazi:
            if self.ukazi[-1][0] == "D":
                self.levo()
            elif self.ukazi[-1][0] == "L":
                self.desno()
            else:
                self.naprej(-self.ukazi[-1][1])
            self.ukazi.pop()
            self.ukazi.pop()
