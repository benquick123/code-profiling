...
class Minobot:
    ...
    def naprej(self,d):
        self.premik(d, self.x, self.y, self.smer)
        self.ukazi.append(["naprej", d])

    def desno(self):
        self.premik("R", self.x, self.y, self.smer)
        self.ukazi.append(["desno"])

    def levo(self):
        self.premik("L", self.x, self.y, self.smer)
    ...
    def razveljavi(self):
        if not self.ukazi:
            ...
            last_cmd = self.ukazi.pop()
            if last_cmd[0] == "naprej":
                self.obrni180()
                self.premik(last_cmd[1], self.x, self.y, self.smer)
                self.obrni180()
            elif last_cmd[0] == "levo":
                self.premik("R", self.x, self.y, self.smer)
            elif last_cmd[0] == "desno":
                self.premik("L", self.x, self.y, self.smer)
