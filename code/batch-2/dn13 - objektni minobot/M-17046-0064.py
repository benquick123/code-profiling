...
class Minobot:
    def __init__(self):
        ...
        self.y = 0
        self.smer = 0
        self.stanja = [copy(self)]
    
    def naprej(self,d):
        self.stanja.append(copy(self))
        self.x += int(d * cos(radians(self.smer*90)))
        self.y += int(d * sin(radians(self.smer*90)))
    ...
    def razveljavi(self):
        if len(self.stanja) > 0:
            oldself = self.stanja[-1]
            self.stanja = self.stanja[:-1]
            self.x = oldself.x
            self.y = oldself.y
            self.smer = oldself.smer
