...
class Minobot:
    ...
    def naprej(self,d):
        if self.smer>360:
            self.smer=self.smer-360
        if self.smer==90 or self.smer==-270:
            self.x=self.x+d
        if self.smer==-90 or self.smer==270:
            self.x=self.x-d
        if self.smer==0 or self.smer==360:
            self.y=self.y+d
        if self.smer == -180 or self.smer==180:
            self.y=self.y-d
    def desno(self):
    ...
    def razdalja(self):
        razdalja = abs(0 - self.y) + abs(0 - self.x)
        return razdalja
    def koordinate(self):
