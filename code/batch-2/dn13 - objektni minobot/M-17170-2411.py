...
class Minobot:
    ...
    def naprej(self,d):
        if self.smer == 0:
            ...
            self.x -= d
        self.zadnji.append(['naprej', d])

    def desno(self):
        self.smer = (self.smer + 1)%4
        self.zadnji.append(['desno'])
    ...
    def razveljavi(self):
        if len(self.zadnji) != 0:
            if self.zadnji[-1][0] == 'naprej':
                self.naprej(-self.zadnji[-1][1])
                del self.zadnji[-1]
            elif self.zadnji[-1][0] == 'desno':
                self.levo()
                del self.zadnji[-1]
            else:
                self.desno()
