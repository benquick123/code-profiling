...
class Minobot:
    ...
    def desno(self):
        ...
        if "NESW".index(self.smer) == 3:
            self.smer = 'N'
        else:
            self.smer = "NESW"["NESW".index(self.smer) + 1]
    ...
    def koordinate(self):
        return tuple(self.koord)

    def razdalja(self):
        return abs(self.koord[0]) + abs(self.koord[1])

    def razveljavi(self):
        if len(self.ukazi) != 0:
            if self.ukazi[-1][0] == "naprej":
                self.naprej(- (self.ukazi[-1][1]))
            elif self.ukazi[-1][0] == "desno":
                self.levo()
            elif self.ukazi[-1][0] == "levo":
                self.desno()
