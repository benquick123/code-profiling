...
slovarSmeri = {0:[1,0] , 1:[0,-1] , 2:[-1,0] , 3:[0,1]}
...
class Minobot():
    def __init__(self):
        ...
        self.y = 0
        self.zgodovina = []
        self.belezi_zgodovino = True

    def naprej(self,d):
        if self.belezi_zgodovino:
            self.zabelezi_antikorak(self.naprej, [-d])
        self.belezi_zgodovino = True
        print(self.x,self.y)
        print(d)
        self.x = self.x + d * slovarSmeri[self.smer][0]
        self.y = self.y + d * slovarSmeri[self.smer][1]

    def desno(self):
        print("Desno")
        if self.belezi_zgodovino:
            self.zabelezi_antikorak(self.levo)
        self.belezi_zgodovino = True
    ...
    def levo(self):
        print("Levo")
        if self.belezi_zgodovino:
            self.zabelezi_antikorak(self.desno)
        self.belezi_zgodovino = True
        self.smer = (self.smer - 1) % 4
    ...
    def zabelezi_antikorak(self, funkcija,d=[]):
        self.zgodovina.append((funkcija,d))


    def razveljavi(self):
        if len(self.zgodovina) > 0:
            self.belezi_zgodovino = False
            self.zgodovina[-1][0](*self.zgodovina[-1][1])
