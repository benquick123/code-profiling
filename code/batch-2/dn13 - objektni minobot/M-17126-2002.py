...
class Minobot:
    ...
    def naprej(self, d):
        ...
        nx, ny = self.x + int(d * math.cos(angle)), self.y + int(d * math.sin(angle))
        self.x, self.y = nx, ny
    ...
    def desno(self):
        self.turn(90)
        self.history_list.append("desno")

    def levo(self):
        self.turn(-90)
        self.history_list.append("levo")
...
################################################### EXTRA ASSIGNMENT ###################################################
### With an addition of history_list in the previous methods as obligatory assignments.

    def razveljavi(self):
        if not self.history_list:
            pass
        else:
            if self.history_list[-1] == "desno":
                self.turn(-90)
                del self.history_list[-1]
            elif self.history_list[-1] == "levo":
                self.turn(90)
                del self.history_list[-1]
            else:
                angle = math.radians(90 - self.angle)
                nx, ny = self.x + int((- self.history_list[-1]) * math.cos(angle)), self.y + int((- self.history_list[-1]) * math.sin(angle))
                self.x, self.y = nx, ny
                del self.history_list[-1]
