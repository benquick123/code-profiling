...
class Minobot:
    ...
    def naprej(self, d):
        self.histor.append((self.x, self.y, self.kot))
        pi = radians(self.kot)

        newx = self.x + int(d * cos(pi))
        newy = self.y + int(d * sin(pi))
        self.x = newx
        self.y = newy
    ...
    def razveljavi(self):
        if len(self.histor) > 0:
            self.x,self.y,self.kot = self.histor[len(self.histor)-1]
            self.histor.pop(len(self.histor) - 1)
