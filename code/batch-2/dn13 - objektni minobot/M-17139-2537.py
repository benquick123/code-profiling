...
class Minobot:
    def __init__(self):
        self.x = 0
        self.y = 0
        self.direction = "Desno"
        self.history = []
        self.history_track = True

    def naprej(self, d):
        if self.direction == "Gor":
            self.y += d

        elif self.direction == "Desno":
            self.x += d

        elif self.direction == "Dol":
            self.y -= d

        elif self.direction == "Levo":
            self.x -= d

        if self.history_track:
            self.history.append((self.naprej, [-d]))
    ...
    def levo(self):
        ...
        if self.history_track:
            self.history.append((self.desno, []))
    ...
    def razveljavi(self):
        if len(self.history) != 0:
            self.history_track = False
            ukaz, arg = self.history[-1]
            del self.history[-1]
            ukaz(*arg)
            self.history_track = True
