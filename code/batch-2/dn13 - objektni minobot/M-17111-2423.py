...
class Minobot:
    def __init__(self):
        #Kje je? self.x je začetna x koordinata, self.y je začenta y koordinata, obedve sta 0 torej sta obe vrednosti 0
        self.x = 0
        self.y = 0
        #Kam je obrnjen? Ker je koordinatni sistem kot pri matematiki, vemo, da je pri kotu 0° nekaj usmerjeno desno, zato je tudi pri nas self.angle enak 0, ker minobot privzeto gleda v desno
        self.angle = 0

    #Najprej deklariram funkcji desno() in levo():
    def desno(self):
        self.angle += 90 #če si predstavljamo naš koordinatni sistem, se bomo z prištevanjem 90° premikali v smeri urinega kazalca, kar je pravilno

    def levo(self):
        self.angle -= 90 #če si predstavljamo naš koordinatni sistem, se bomo z odštevanjem 90° premikali v nasprotni smeri urinega kazalca,
                         # kar pomeni da bo naš minobot, kot primer iz pozicije desno, gledal navzgor in to je je pravilno

    def naprej(self, d):
        nov_x = self.x + d * round(cos(radians(self.angle))) # z roundom zaokrožim na celo število in izračunam nove koordinate pri premiku
        nov_y = self.y - d * round(sin(radians(self.angle)))

        self.x = nov_x
        self.y = nov_y

    def koordinate(self):
        return (self.x, self.y) #vrnem terko koordinat x in y

    def razdalja(self):
        """pozicija_minobota = (self.x, self.y)
        zacetno_izhodisce = (0, 0)"""
        return (abs(self.x - 0)+(abs(self.y - 0))) #vrnem pravokotno razdaljo do koordinat (0, 0)
