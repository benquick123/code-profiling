...
i = int(input("Kateri program želite izvesti? 1 za obvezni, 2 za dodatni. "))
...
if i == 1:
    ...
    for ime, x, y in kraji:
        if ime == kraj:
            xk = x
            yk = y
    print(kraj, "je na koordinatah", xk, yk)

    max_razdalja = 0
    max_kraj = ""
    for ime, x, y in kraji:
        razdalja = m.sqrt((x-xk) ** 2 + (y-yk) ** 2)
        if domet > razdalja > max_razdalja:
            max_razdalja = razdalja
            max_kraj = ime
    print("Iz kraja", kraj, "lahko zalijemo kraj", max_kraj, "na razdalji", max_razdalja)
...
# dodatna------------------------------------------------------------------------------------
    ...
    for ime, x, y in kraji:
        if ime == kraj1:
            xk1 = x
            yk1 = y
            # print(kraj1, "je na koordinatah", xk1, yk1)
        if ime == kraj2:
            xk2 = x
            yk2 = y
            # print(kraj2, "je na koordinatah", xk2, yk2)

    # ideja je poiskati vse kraje(tarce), ki jih lahko zalije posamezen kraj in nato poiskati presek
    # teh dveh množic (skupne člene)

    tarce1 = []
    tarce2 = []

    for ime, x, y in kraji:
        razdalja = m.sqrt((x - xk1) ** 2 + (y - yk1) ** 2)
        if domet > razdalja and ime != kraj1:
            tarce1.append(ime)

    for ime, x, y in kraji:
        razdalja = m.sqrt((x - xk2) ** 2 + (y - yk2) ** 2)
        if domet > razdalja and ime != kraj2:
            tarce2.append(ime)

    # print("Iz", kraj1, "lahko zalijemo: ", tarce1)
    # print("Iz", kraj2, "lahko zalijemo: ", tarce2)

    # list-e spremenimo v set-e, da nad njimi lahko uporabimo funkcijo intersection()
    skupne_tarce = set(tarce1).intersection(set(tarce2))

    # print("Skupne tarce so", skupne_tarce)

    for ime in skupne_tarce:
        print(ime)
