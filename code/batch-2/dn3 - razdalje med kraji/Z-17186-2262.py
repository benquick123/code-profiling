...
for ime, x, y in kraji:
    if ime == kraj:
        legaA = x
        legaB = y
print(kraj, 'je na koordinatah', legaA, legaB)

for kraj1, lega1, lega2 in kraji:
    razdalja = sqrt((lega1 - legaA)**2 + (lega2 - legaB)**2)
    if najvecja < razdalja <= domet:
        cilj = kraj1
        najvecja = razdalja
print('Iz kraja', kraj, 'lahko zalijemo kraj', cilj, 'na razdalji', najvecja)
...
for ime, z, q in kraji:
    if ime == kraj2:
        legaC = z
        legaD = q

for ime, b, c in kraji:
    razdalja1 = sqrt((b - legaA)**2 + (c - legaB)**2)
    razdalja2 = sqrt((b - legaC) ** 2 + (c - legaD) ** 2)
