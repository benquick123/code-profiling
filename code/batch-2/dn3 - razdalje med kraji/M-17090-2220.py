...
iskani_kraj = input("Vnesi kraj: ")
domet = int(input("Vnesi domet: "))
iskani_kraj_x = 0
iskani_kraj_y = 0

for kraj, x, y in kraji:
    if kraj == iskani_kraj: #ko najde vnešeni kraj si zapomni x in y koordinate
        print(iskani_kraj, "je na koordinatah", x, y)
        iskani_kraj_x = x
        iskani_kraj_y = y
        break

najbolj_oddaljeni_kraj = ""
razdalja = 0
max_razdalja = 0

for kraj, x, y in kraji:
    razdalja = sqrt(abs(iskani_kraj_x - x)**2 + abs(iskani_kraj_y - y)**2) #izračuna razdaljo med krajema
    if razdalja > max_razdalja and razdalja <= domet: #če je nova razdalja največja in v dometu, si shrani kraj
        max_razdalja = razdalja
        najbolj_oddaljeni_kraj = kraj
print("Iz kraja", iskani_kraj, "lahko zalijemo", najbolj_oddaljeni_kraj, "na razdalji", max_razdalja)


# Dodatna naloga

prvi_kraj = input("Vnesi prvi kraj: ")
drugi_kraj = input("Vnesi drugi kraj: ")
domet = int(input("Vnesi domet: "))
prvi_x = 0
prvi_y = 0
drugi_x = 0
...
for kraj, x, y in kraji:
    if kraj == prvi_kraj: #poišče koordinate prvega kraja
        prvi_x = x
        prvi_y = y
    if kraj == drugi_kraj: #poišče koordinate drugega kraja
        drugi_x = x
        drugi_y = y
...
for kraj, x, y in kraji:
    prvi_razdalja = sqrt(abs(prvi_x - x)**2 + abs(prvi_y - y)**2)
    drugi_razdalja = sqrt(abs(drugi_x - x)**2 + abs(drugi_y - y)**2)
    if prvi_razdalja <= domet and drugi_razdalja <= domet: #če sta obe razdalji v dometu, izpiše kraj
        print(kraj)