...
najvecja_oddaljenost = 0
ime_najbolj_oddaljenega = ""

for ime2, x2, y2 in kraji:
    razdalja = math.sqrt((x2 - x) ** 2 + (y2 - y) ** 2)
    if razdalja <= domet and razdalja > najvecja_oddaljenost:
        najvecja_oddaljenost = razdalja
        ime_najbolj_oddaljenega = ime2

print("Iz kraja", kraj, "lahko zalijemo kraj", ime_najbolj_oddaljenega, "na razdalji", najvecja_oddaljenost)
...
for kraj, x, y in kraji:
    if trenutni_kraj == kraj:
        trenutni_x = x
        trenutni_y = y
    elif kraj == ciljni_kraj:
        ciljni_x = x
        ciljni_y = y

razdalja = math.sqrt((ciljni_x - trenutni_x) ** 2 + (ciljni_y - trenutni_y) ** 2)

while trenutni_kraj != ciljni_kraj:

    najboljsa_pot = razdalja
    najboljsi_kraj = ""

    for kraj, x, y in kraji:

        razdalja_a = math.sqrt((trenutni_x - x) ** 2 + (trenutni_y - y) ** 2)
        nova_razdalja = math.sqrt((ciljni_x - x) ** 2 + (ciljni_y - y) ** 2)

        if razdalja_a <= domet and nova_razdalja <= najboljsa_pot:

            najboljsa_pot = nova_razdalja
            najboljsi_kraj = kraj

    for kraj, x , y in kraji:
        if kraj == najboljsi_kraj:
            trenutni_kraj = kraj
            trenutni_x = x
            trenutni_y = y
            pot.append(kraj)
            razdalja = najboljsa_pot
