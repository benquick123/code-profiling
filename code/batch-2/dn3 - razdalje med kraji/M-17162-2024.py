...
kraj = input('Ukucajte kraj u kome je vodeni top(Pravopisno upiši): ')
domet = float(input('Ukucajte domet vodenog topa: '))
priblizni_razmak = 0
x1 = 0
y1 = 0
pogodak = 0
...
for ime1, x1, y1 in kraji:
    if kraj == ime1:
        print(kraj, 'je na koordinatama', x1,',', y1)
        for ime2, x2, y2 in kraji:
            razmak = sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)
            if priblizni_razmak < razmak < domet:
                priblizni_razmak = razmak
                pogodak = ime2
                the_end = 1
if the_end == 1:
    print('Iz kraja', kraj, 'lako zalijemo kraj', pogodak, 'na razdaljini od', priblizni_razmak)
else:
    print('Ukucali ste pogrešno kraj. PS Sljedeći put ne zaboravite napisati prvo veliko slovo.')
