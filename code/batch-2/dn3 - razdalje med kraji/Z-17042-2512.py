...
x=0
y=0
#Poiščemo kraj med kraji
for i in kraji:
    if i[0] == kraj:
        x = i[1]
        y = i[2]
print(kraj + " se nahaja na koordinatah " + str(x) + " " + str(y))

#Za začetno tarčo nastavimo domač kraj, potem pa iščemo bolj oddaljene v dometu
najdlje=0
tarca=kraj
for i in kraji:
    razdalja = ((x - i[1])**2 + (y - i[2])**2)**(0.5)
    if (razdalja > najdlje) & (razdalja <= domet):
        tarca = i[0]
        najdlje = razdalja
print("Iz kraja " + kraj + " lahko zalijemo kraj " + tarca + " na razdalji " + str(najdlje))
...
vdosegu=[]
for i in kraji:
    razdalja1 = ((x1 - i[1]) ** 2 + (y1 - i[2]) ** 2) ** (0.5)
    razdalja2 = ((x2 - i[1]) ** 2 + (y2 - i[2]) ** 2) ** (0.5)
    if (razdalja1 <= domet) & (razdalja2 <= domet):
        vdosegu.append(i[0])
#Izpišemo vse
for i in vdosegu:
    print(i)
