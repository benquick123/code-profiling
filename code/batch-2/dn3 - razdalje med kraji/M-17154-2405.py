...
for mesto in kraji:
    ...
    if ime == kraj:
        ...
        for mesto2 in kraji:
            ime2, x2, y2 = mesto2
            razdalja =((((x2 - x)**2)) + (((y2 - y)**2)))**(1/2)

            if (razdalja < domet) and (razdalja > najdlje):
                najdlje = razdalja
                mestodalec = ime2

        print('Iz', ime, 'lahko še zalivamo', mestodalec, 'na razdalji', najdlje)
...
for mesto in kraji:
    ...
    if ime == kraj1:
        ...
        for mesto2 in kraji:
            ime2, x2, y2 = mesto2
            razdalja1 =(((x2 - x)**2)) + (((y2 - y)**2))**(1/2)
            najdlje = 0

            if (razdalja1 < domet) and (razdalja1 > najdlje):
                najdlje = razdalja1
                for mesto3 in kraji:
                    ime3,x3,y3=mesto3
                    razdalja2 = (((x3 - x2) ** 2)) + (((y3 - y2) ** 2)) ** (1 / 2)
                    najdlje=razdalja2
                    if ime3 == kraj2:
                        if razdalja2 < domet or razdalja2 <=razdalja1:
                          obazalijeta=ime2
                          print(ime2)
