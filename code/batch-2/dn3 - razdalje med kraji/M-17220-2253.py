...
for nameofcity,latitude,longitude in kraji:
    if nameofcity == city:
        a = latitude
        b = longitude
        print(city," je na koordinatah ",a ,b)

for nameofcity,latitude,longitude in kraji:
    if sqrt((a-latitude)**2 + (b -longitude)**2) > x and sqrt((a-latitude)**2 + (b -longitude)**2) <= radius:
        x = sqrt((a-latitude)**2 + (b -longitude)**2)
        city2 = nameofcity
if y == True:
    print("Iz kraja", city, "lahko zalijemo kraj", city2, "na razdalji", x)
else:
