...
print("Odgovor obvezne naloge: ")
ime = x = y = najvecja_raz = razdx = razdy = x_kraj = y_kraj = 0
for mesto, x, y in kraji:
    if mesto == kraj:
        for mesto1, x1, y1 in kraji:

            if x != abs(x) and x1 == abs(x1):
                razdx = x1 + abs(x)
            if (x == abs(x) and x1 == abs(x1)) or (x != abs(x) and x1 != abs(x1)):
                razdx = abs(x1 - x)
            if x == abs(x) and x1 != abs(x1):
                razdx = x + abs(x1)

            if y != abs(y) and y1 == abs(y1):
                razdy = y1 + abs(y)
            if (y == abs(y) and y1 == abs(y1)) or (y != abs(y) and y1 != abs(y1)):
                razdy = abs(y1 - y)
            if y == abs(y) and y1 != abs(y1):
                razdy = y + abs(y1)

            odd = sqrt(razdx**2 + razdy**2)
            if odd <= domet and odd > najvecja_raz:
                najvecja_raz = odd
                ime = mesto1
                x_kraj = x
                y_kraj = y

print(kraj, "je na koordinatah", x_kraj, y_kraj,)
print("Iz kraja", kraj,"lahko zalijemo kraj", ime ,"na razdalji", najvecja_raz)
...
print(" ")
print("Odgovor dodatne naloge: ")
x1 = x2 = x3 = y1 = y2 = y3 = 0
drugi_do_x = razdx = prvi_do_y = prvi_do_x = drugi_do_y = n = 0

for kraj_1, x1, y1 in kraji:
    if kraj_1 == kraj1:
        for kraj_2, x2, y2 in kraji:
            if kraj_2 == kraj2:
                for kraj_3, x3, y3 in kraji:
                    a = x1
                    b = x3
                    n = 0
                    while n < 4:
                        if a != abs(a) and b == abs(b):
                            razdx = b + abs(a)
                        if (a == abs(a) and b == abs(b)) or (a != abs(a) and b != abs(b)):
                            razdx = abs(b - a)
                        if a == abs(a) and b != abs(b):
                            razdx = a + abs(b)
                    ...
                    odd_1_3 = sqrt(prvi_do_x**2 + prvi_do_y**2)
                    odd_2_3 = sqrt(drugi_do_x**2 + drugi_do_y**2)
                    if odd_1_3 <= domet and odd_2_3 <= domet:
                        print(kraj_3)