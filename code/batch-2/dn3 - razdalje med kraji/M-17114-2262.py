...
for ime, x, y in kraji :
    if ime == kraj:
        x0 = x
        y0 = y
print(kraj, 'je na kordinatah', x0, y0)
for ime, x, y in kraji:
    razdalja = sqrt((x-x0)**2 + (y-y0)**2)
    if  naj_domet < razdalja < domet:
        naj_domet = razdalja
        naj_ime = ime
print('Iz kraja', kraj, 'lahko zalijemo kraj', naj_ime, 'na razdalji', naj_domet)
...
for ime, x, y in kraji :
    if ime == kraj1:
        ...
        x2, y2 = x, y
k = sqrt((x1-x2)**2 + (y1-y2)**2)/domet
seznam=[[kraj1]]
a=1
while a:
    for seznamcek in seznam:
        if a:
            for ime, x, y in kraji:
                if ime == seznamcek[-1]:
                    x0, y0 = x, y
            for ime, x, y in kraji:
                if sqrt((x-x0)**2 + (y-y0)**2) < domet:
                    if ime == kraj2:
                        print('\n'.join(i for i in seznamcek)+ '\nMaribor')
                        a=0
                        break
                    razdalja =  sqrt((x-x2)**2 + (y-y2)**2)
                    if ime in seznamcek or razdalja/domet > k - len(seznamcek) - 1:
                        continue
                    seznam1=seznamcek.copy()
