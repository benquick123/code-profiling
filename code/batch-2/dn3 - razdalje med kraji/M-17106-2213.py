...
for mesto, x2, y2 in kraji:
    ...
    if(razdalja<=domet and razdalja2<=razdalja):
        kraj2 = mesto
        razdalja2 = razdalja
print("Iz kraja ", kraj, " lahko zalijemo kraj ",kraj2," na razdalji ", razdalja2)
...
for mesto, x3, y3 in kraji:
    razdalja = sqrt((x3-x1)**2+(y3-y1)**2)
    if(razdalja<=domet and not mesto == kraj2):
        razdalja2 = sqrt((x3 - x2) ** 2 + (y3 - y2) ** 2)
        if(razdalja2 <= domet and not mesto == kraj):
            print(mesto)