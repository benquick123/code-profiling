...
# Obvezna naloga
kraj = str(input("Vpišite ime kraja, kjer so kupili top: "))
domet = int(input("Vpišite domet, topa: "))
...
max = 0
for i in range(velikost):
    razdalja = sqrt((x2-kraji[i][1])**2 +(y2-kraji[i][2])**2)
    if razdalja >= max and razdalja < domet:
        max= razdalja
        krajZalitja = kraji[i][0]
print("kraj",kraj," je na koordianatah",x2," ",y2)
print("lahko zalijemo",krajZalitja,"ki je odaljen ",max)
...
kraj1 = str(input("Vpišite ime kraja, kjer so kupili top: ")) #Ljubljana
kraj2 = str(input("Vpišite ime kraja, kjer so kupili top: ")) #Bled
domet = int(input("Vpišite domet, topa: ")) #30
...
for i in range(velikost):
    razdalja1 = sqrt((Kraj1X2 - kraji[i][1]) ** 2 + (Kraj1Y2 - kraji[i][2]) ** 2)
    razdalja2 = sqrt((Kraj2X2 - kraji[i][1]) ** 2 + (Kraj2Y2 - kraji[i][2]) ** 2)
    if razdalja1 < 30 and razdalja2 < 30:
        print(kraji[i][0])
