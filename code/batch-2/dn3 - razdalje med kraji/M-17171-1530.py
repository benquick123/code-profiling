...
for ime,x,y in kraji:
    if ime == kraj:
        ...
        iz_x = x
        iz_y = y
        print(kraj, "je na koordinatah", iz_x, iz_y)
        for mesto,mesto_x,mesto_y in kraji:
            koliko = sqrt((iz_x - mesto_x) ** 2 + (iz_y - mesto_y) ** 2)

            if koliko <= domet:
                if zadnji_domet < koliko:
                    zadnji_domet = koliko

                    shrani_ime = mesto
                    shrani_x = mesto_x
                    shrani_y = mesto_y
        print("Iz kraja", kraj, "lahko zalijemo kraj", shrani_ime, "na razdalji", zadnji_domet)
...
for ime, x, y in kraji:
    if kraj1 == ime:
        ...
        for a1, x1, y1 in kraji:
            d1 = sqrt((kraj1_x - x1) ** 2 + (kraj1_y - y1) ** 2)
            if d1 <= domet:
                sez1.add(a1)
    ...
    if kraj2 == ime:
        ...
        for b1, x2, y2 in kraji:
            d2 = sqrt((kraj2_x - x2) ** 2 + (kraj2_y - y2) ** 2)
            if d2 <= domet:
                sez2.add(b1)
