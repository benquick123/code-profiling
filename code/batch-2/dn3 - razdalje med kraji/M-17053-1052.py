...
kraj = input("Vnesi ime kraja iz seznama: ")
...
for i in kraji:
    if i[0] == kraj:
        x = i[1]
        y = i[2]
print("Kraj", kraj, " je na koordinatah", x , y)

"""
Ne razumem, zakaj program v nadaljevanju ne deluje pravilno,
 če namesto zgornjega, dela rešitve uporabimo spodnjega, zakomentiranega?
...
naj_razdalja = 0
for naj_ime, nx, ny in kraji:
        razdalja = sqrt((x - nx)**2 + (y - ny)**2)
        if razdalja > naj_razdalja and razdalja <= domet:
            naj_razdalja = razdalja
            novo_naj_ime = naj_ime

print("Iz kraja", kraj, "lahko zalijemo kraj", novo_naj_ime, "na razdalji", naj_razdalja)
