...
while y < len(kraji):
    if kraji[y][0] == kraj:
        Prva_koordinata = kraji[y][1]
        Druga_koordinata = kraji[y][2]
        break
    y = y + 1


while x < len(kraji):
    OddaljenostKraja = sqrt((kraji[x][1] - Prva_koordinata)**2 + (kraji[x][2] - Druga_koordinata)**2)
    if OddaljenostKraja > NajOddaljenKraj and OddaljenostKraja <= domet:
        NajOddaljenKraj = OddaljenostKraja
        ImeNajOddaljenegaKraja = kraji[x][0]
    x = x + 1

print(kraj,"je na koordinatah" ,Prva_koordinata,Druga_koordinata,". Iz kraja",kraj,"lahko zalijemo kraj",ImeNajOddaljenegaKraja,"na razdalji",NajOddaljenKraj)
...
while y < len(kraji):
    oddaljenostkraja1 = sqrt((kraji[y][1] - kraj1x)**2 + (kraji[y][2] - kraj1y)**2)
    oddaljenostkraja2 = sqrt((kraji[y][1] - kraj2x)**2 + (kraji[y][2] - kraj2y)**2)
    if oddaljenostkraja1 <= domet and oddaljenostkraja2 <= domet:
        print(kraji[y][0])
    y = y + 1