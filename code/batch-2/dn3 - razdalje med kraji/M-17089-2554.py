...
kraj = input("Ime kraja? ")
domet = int(input('Domet v km? '))
dis_city = 0
...
for ime, p, q in kraji:
    razdalja = sqrt((x - p) ** 2 + (y - q) ** 2)
    if domet >= razdalja and razdalja > dis_city:
        dis_city = razdalja
        iskan_kraj = ime
print('Iz kraja', kraj, 'lahko zalijemo kraj', iskan_kraj, 'na razdalji', dis_city)
...
for ime3, p, q in kraji:
    dis_city_x = g - p
    dis_city_y = h - q
    razdalja1 = sqrt(dis_city_x ** 2 + dis_city_y ** 2)
    if domet >= razdalja1:
        if razdalja1 > domet_low:
            for ime4, x, y in kraji:
                dis_city_x = a - x
                dis_city_y = b - y
                razdalja2 = sqrt(dis_city_x ** 2 + dis_city_y ** 2)
                if domet >= razdalja2:
                    if ime3 == ime4:
                        print(ime3)
                        break
