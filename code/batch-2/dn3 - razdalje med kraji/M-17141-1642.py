...
# vhodni podatki
shootFrom = "Ljubljana"  # kraj1
shootFrom2 = "Bled"  # kraj2
domet = 30
...
# fishamo podatke za nasa dva(og) kraja
for ime, lon, lat in kraji:
    if ime == shootFrom:
        ogZ = math.sqrt(lon ** 2 + lat ** 2)
        ogLat = lat
        ogLon = lon
    if ime == shootFrom2:
        ogZ2 = math.sqrt(lon ** 2 + lat ** 2)
        ogLat2 = lat
        ogLon2 = lon

# primerjamo z OG krajema vse ostale kraje
for ime, lon, lat in kraji:

    razdalja = math.sqrt((lon - ogLon) ** 2 + (lat - ogLat) ** 2)
    razdalja2 = math.sqrt((lon - ogLon2) ** 2 + (lat - ogLat2) ** 2)
    ...
    # obvezna naloga
    if razdalja <= domet and razdalja > maxDomet:
        maxDomet = razdalja
        maxKraj = ime

print("Iz kraja", shootFrom, "lahko zalijemo kraj", maxKraj, "na razdalji", maxDomet)
