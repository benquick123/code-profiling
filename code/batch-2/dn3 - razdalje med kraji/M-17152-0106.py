...
for kraj1, x1, y1 in kraji:
    if (kraj1 == kraj): # Preveri, če je kraj na seznamu
        print(kraj + " je na koordinatah " + str(x1) + " " + str(y1)) # Izpiše koordinate kraja s topom
        for kraj2, x2, y2 in kraji:
            razdalja = sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2) # Izračuna razdaljo od kraja s topom do drugih krajev
            if (razdalja <= domet and razdalja > max_domet): # Preveri, če je razdalja manjša od dometa ter ali je večja od trenutne največje razdalje
                max_domet = razdalja
                max_kraj = kraj2
print("Iz kraja " + kraj + " lahko zalijemo kraj " + max_kraj + " na razdalji " + str(max_domet))
