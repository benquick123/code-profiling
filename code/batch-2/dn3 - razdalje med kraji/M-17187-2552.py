...
for kraj_b, x, y in kraji:
    pot = sqrt((x - ax) ** 2 + (y - ay) ** 2)
    if (pot <= domet) and (pot > najdlje):
        najdlje = pot
        kraj_naj = kraj_b
        x_naj = x
        y_naj = y
...
print(kraj_a, "je na koordinatah", ax, ay)
print("iz kraja", kraj_a, "lahko zalijemo kraj", kraj_naj, "na razdalji", najdlje)
...
for kraj_c, x, y in kraji:

    pot_a = sqrt((x - ax) ** 2 + (y - ay) ** 2)
    pot_b = sqrt((x - bx) ** 2 + (y - by) ** 2)

    if (pot_a <= domet) and (pot_b <= domet):
        kraj_naj = kraj_c
        stevec += 1
        x_naj = x
        y_naj = y
        print(kraj1, "in", kraj2, "lahko družno zalivata:", kraj_naj)

if stevec == 0:
    print("Kraja ne moreta družno zalivata nobenega mesta")
elif stevec == 1:
    print("kraja lahko družno zalivata 1 kraj")
elif stevec == 2:
    print("kraja lahko družno zalitava 2 kraja")
elif stevec == 3 or stevec == 4:
    print("kraja lahko družno zalivata", stevec, "kraje")
else:
    print("Kraja lahko družno zalivata", stevec, "krajev")
