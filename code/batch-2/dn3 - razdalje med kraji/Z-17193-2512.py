...
for x in kraji:
    ime, kx, ky = x
    if ime == kraj:
        print(ime, 'se nahaja na koordinatah', kx, ky)

        najvisja = 0
        for x1 in kraji:
            ime2, xk, yk = x1
            razdalja = sqrt((pow((kx - xk),2)) + (pow((ky - yk),2)))
            if (razdalja < domet) and (razdalja > najvisja):
                najvisja = razdalja
                najkraj = ime2
        print('iz', ime, 'lahko zalijemo', najkraj, 'na razdalji', najvisja)
...
for x1 in kraji:
    ...
    if ime1 == kraj1:
        for x2 in kraji:
            ime2, xk, yk = x2
            razdalja1 = sqrt((pow((kx - xk),2)) + (pow((ky - yk),2)))
            najvisja = 0
            if (razdalja1 < domet) and (razdalja1 > najvisja):
                najvisja = razdalja1
                for x3 in kraji:
                    ime3, kxx, kyy = x3
                    if ime3 == kraj2:
                        razdalja2 = sqrt((pow((kxx - xk), 2)) + (pow((kyy - yk), 2)))
                        skupnamax = ((razdalja1 + razdalja2) / 2 < 30)
                        if (razdalja2 < domet) and (skupnamax == True):
                            najvisja = razdalja1
                            najkraj = ime2
                            print(ime2)
