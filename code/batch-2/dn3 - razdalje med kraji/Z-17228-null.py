...
for ime, x, y in kraji:
    print(ime)
mesto = input("Izberite ime kraja ki je kupilo vodni top.")
domet = int(input("Kako daleč nese vodni top?"))

for ime, x, y in kraji:
    if ime == mesto:
        izbrani_x = x
        izbrani_y = y
        print(mesto,"je na koordinatah", izbrani_x, izbrani_y)
for ime, x, y in kraji:
    d = sqrt((x - izbrani_x)**2 + (y - izbrani_y)**2)
    if d <= domet and ime != mesto and d > max_doseg:
        max_doseg = d
        max_kraj = ime
print("Iz kraja", mesto,"lahko zalijemo kraj", max_kraj, "na razdalji",max_doseg)
