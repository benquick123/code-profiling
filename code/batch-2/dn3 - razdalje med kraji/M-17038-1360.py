...
for mesto, x , y in kraji:
    razdalja = sqrt (pow(abs(x - mesto1x),2) + pow(abs(y - mesto1y),2))
    if razdalja <= domet and razdalja > najbolje:
        najbolje = razdalja
        najmesto = mesto
print ( kraj , "je na koordinatah", mesto1x , mesto1y)
print ("Iz kraja" , kraj , "lahko zalijemo mesto" , najmesto, "z razdalje" , najbolje )
...
for mesto, x, y in kraji:
    if mesto != kraj1 or mesto != kraj2:
        razdalja1 = sqrt(pow(abs(x - mesto1x), 2) + pow(abs(y - mesto1y), 2))
        razdalja2 = sqrt(pow(abs(x - mesto2x), 2) + pow(abs(y - mesto2y), 2))
        if razdalja1 <= domet and razdalja2 <= domet:
            skupno = mesto
            print ( "Skupaj lahko zalivata:" , skupno )
