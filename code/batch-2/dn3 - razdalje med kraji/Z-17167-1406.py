...
kraj= input("V katerem kraju stoji vodni top? ")
for (ime, a1, b1) in kraji:
    if kraj==ime:
        break
print(kraj, "je na koordinatah", a1, "in", b1,".")


domet= int(input("Kolikšen je domet visokodometnega vodnega topa? "))
najvisja = 0
for(ime_k, a_k, b_k) in kraji:
    iskanoime= ime_k
    c = sqrt ((a1 - a_k) ** 2 + (b1 - b_k) ** 2)
    if (c < domet) and (c > najvisja):
        najvisja = c
        zalitikraj = iskanoime

print("Iz kraja", kraj, "lahko zalijemo kraj", zalitikraj, "na razdalji", najvisja,".")
