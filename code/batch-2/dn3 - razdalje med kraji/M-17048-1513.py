...
# Find the coordinates of desired place (kraj)
found = False
...
if found:
    print(kraj, "je na koordinatah", place_x, place_y)
else:
    print(kraj, " ni v seznamu!")
    sys.exit(-1)

# Find the first place in range of our desired place
found = False
desired_place_range = -1
desired_place_name = ""
for name, x, y in kraji:
    distance = sqrt((x - place_x)**2 + (y - place_y)**2)
    if distance <= domet and distance > desired_place_range:
        found = True
        desired_place_range = distance
        desired_place_name = name

if found:
    print("Iz kraja", kraj, "lahko zalijemo kraj", desired_place_name, "na razdalji", desired_place_range)
else:
    print("Noben izmed krajov na seznamu ni v dometu", domet, "kraja", kraj)
...
place1_x, place1_y, place2_x, place2_y = -1, -1, -1, -1

# Find the coordinates of desired places (kraj1, kraj2)
...
if not found1:
    print(kraj1, "ni v seznamu")
    sys.exit(-1)
elif not found2:
    print(kraj2, "ni v seznamu")
    sys.exit(-1)

# Print out all places in range of our 2 places
for name, x, y in kraji:
    distance1 = sqrt((x - place1_x)**2 + (y - place1_y)**2)
    distance2 = sqrt((x - place2_x) ** 2 + (y - place2_y) ** 2)
