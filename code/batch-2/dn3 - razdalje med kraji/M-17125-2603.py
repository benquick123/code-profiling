...
for ime1, x1, y1 in kraji:
    if ime1 == kraj:
        xkraja = x1
        ykraja = y1
        print (kraj + " je na koordinatah " + str(xkraja) + " " + str(ykraja))

for ime, x, y in kraji:
    razdalja = sqrt((xkraja-x) **2 + (ykraja - y) ** 2)
    if razdalja <= domet and razdalja > naj_razdalja:
        naj_razdalja = razdalja
        naj_kraj = ime

print("Iz kraja " + kraj + " lahko zalivamo kraj " + naj_kraj + " na razdalji: " + str(naj_razdalja))
