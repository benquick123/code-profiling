...
for mesto, x, y in kraji:
    if mesto == kraj:
        ...
        break
for mesto, x, y in kraji:
    if sqrt((x-x_kraj)**2 + (y-y_kraj)**2)<domet and sqrt((x-x_kraj)**2 + (y-y_kraj)**2)>največja_razdalja_od_kraj:
        največja_razdalja_od_kraj=sqrt((x-x_kraj)**2 + (y-y_kraj)**2)
        najbolj_oddaljen_kraj=mesto

print ("Iz kraja", kraj, "lahko zalijemo kraj", najbolj_oddaljen_kraj, "na razdalji" ,največja_razdalja_od_kraj)
...
for mesto, x, y in kraji:
    if mesto == kraj2:
        ...
        break
for mesto, x, y in kraji:
    if domet >sqrt((x-x_kraj1)**2 + (y-y_kraj1)**2) and domet >sqrt((x-x_kraj2)**2 + (y-y_kraj2)**2):
        kraj=mesto
        print(kraj)