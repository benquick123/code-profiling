...
zalit_kraj = "Kraja ni na seznamu"
naj_razdalja = 0.0
...
for ime, x, y in kraji:
    trenutna_razdalja= sqrt((x - x_kraja) ** 2 + (y - y_kraja) ** 2)
    if naj_razdalja < trenutna_razdalja <= domet:
        zalit_kraj = ime
        naj_razdalja = trenutna_razdalja

print(kraj, "je na koordinatah", x_kraja, y_kraja)
print("Iz kraja", kraj, "lahko zalijemo kraj", zalit_kraj, "na razdalji", naj_razdalja)
...
for ime, x, y in kraji:
    razdalja1 = sqrt((x - x_kraja1) ** 2 + (y - y_kraja1) ** 2)
    razdalja2 = sqrt((x - x_kraja2) ** 2 + (y - y_kraja2) ** 2)
