...
print('Iz kraja', kraj, 'lahko zalijemo kraj', maxKraj, 'na razdalji', maxRazdalja, '\n')
...
for i in range(len(kraji)):
    x2        = kraji[i][1]
    y2        = kraji[i][2]
    razdalja1 = sqrt(pow(x2 - k1x1, 2) + pow(y2 - k1y1, 2))
    razdalja2 = sqrt(pow(x2 - k2x1, 2) + pow(y2 - k2y1, 2))

    if razdalja1 <= domet and razdalja1 != 0:
        vDometuKraja1.append(kraji[i][0])
    if razdalja2 <= domet and razdalja2 != 0:
        vDometuKraja2.append(kraji[i][0])

def izpisiSkupneKraje(kraji1, kraji2, stKrajev):
    stSupnihKrajev = 0
    for i in range(stKrajev):
        kraj = kraji1[i]
        if kraj in kraji2:
            print(kraj)
            stSupnihKrajev += 1
    ...
    if stSupnihKrajev == 0:
        print('Kraja', kraj1, 'in', kraj2, 'ne moreta družno zalivati nobenega kraja')

if len(vDometuKraja1) <= len(vDometuKraja2):
    izpisiSkupneKraje(vDometuKraja1, vDometuKraja2, len(vDometuKraja1))
else:
    izpisiSkupneKraje(vDometuKraja2, vDometuKraja1, len(vDometuKraja2))
