...
for ime,x,y in kraji:
    if ime==kraj:
        ...
        for ime,x,y in kraji:
            drugaX=x
            drugaY=y
            daljava=sqrt((prvaX-drugaX)**2+(prvaY-drugaY)**2)
            if daljava<domet:
                if daljava>maxDaljava:
                    maxDaljava=daljava
                    drugiKraj=ime
print(kraj+" je na koordinatah "+ str(prvaX) + " "+ str(prvaY))
print("Iz kraja "+kraj+" lahko zalijemo kraj "+ drugiKraj+" na razdalji "+str(maxDaljava))
...
print("-----------------")
#obrazec za razdaljo:
#sqrt((koordinataX1-koordinataX2)**2+(koordinataY1-kordinataY2)**2)
...
dometKraj1=[]
dometKraj2=[]
for ime,x,y in kraji: #vsi kraji od kraj1
    if ime==kraj1:
        prvaX=x
        prvaY=y
        for ime,x,y in kraji:
...
for ime,x,y in kraji: #vsi kraji od kraj1
    if ime==kraj2:
        ...
        for ime,x,y in kraji:
            ...
            daljava=sqrt((prvaX-drugaX)**2+(prvaY-drugaY)**2)
            if daljava<=domet:
                dometKraj2.append(ime)
for element in dometKraj1:
    if element in dometKraj2:
        print(element)
