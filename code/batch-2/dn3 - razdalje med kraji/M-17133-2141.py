...
for ime, koordinata1, koordinata2 in kraji:
    oddaljenost = math.sqrt(pow(koordinata1, 2)+pow(koordinata2, 2))
    print(ime, oddaljenost)


najoddaljen = ""
zadnji_najoddaljeni = 0
for i, k1, k2 in kraji:
    odd = math.sqrt(pow(k1, 2)+pow(k2, 2))
    if odd > zadnji_najoddaljeni:
        zadnji_najoddaljeni = odd
        najoddaljen = i
print(najoddaljen, zadnji_najoddaljeni)

'''

dn = int(input("Vnesite 1, da zaženete obvezno nalogo ali 2, da zaženete dodatno nalogo: "))
...
if dn == 1:
    ...
    while True:
        if ponoven_vpis == "y":
            kraj = input("Vnesite ime kraja (primer: Ljubljana): ")
            domet = float(input("Vnesite domet vodnega topa (primer: 30): "))
            ...
            for i, t1, t2 in kraji:
                if i == kraj:
                    koordinata1 = t1
                    koordinata2 = t2
                    print(kraj, "je na koordinatah", t1, t2)
                    break

            se_v_dometu = 0
            se_v_dometu_kraj = ""

            for ime, k1, k2 in kraji:
                odd = math.sqrt(pow(koordinata1 - k1, 2) + pow(koordinata2 - k2, 2))
                if odd <= domet and se_v_dometu < odd:
                    se_v_dometu = odd
                    se_v_dometu_kraj = ime
            print("Iz kraja", kraj, "lahko zalijemo kraj", se_v_dometu_kraj, "na razdalji", se_v_dometu)

        else:
            break

        ponoven_vpis = input("Če želite ponovno vnesti kraj in domet vnesite y: ")
    ...
    for ime, k1, k2 in kraji:
        odd1 = math.sqrt(pow(kraj1_k1 - k1, 2) + pow(kraj1_k2 - k2, 2))
        odd2 = math.sqrt(pow(kraj2_k1 - k1, 2) + pow(kraj2_k2 - k2, 2))
        if odd1 <= domet and odd2 <= domet:
            print(ime)

else:
    print("Niste vnesli pravilne številke. Prosim vnesite številko 1 za zagon obvezne naloge oziroma 2 za zagon dodatne naloge.")
