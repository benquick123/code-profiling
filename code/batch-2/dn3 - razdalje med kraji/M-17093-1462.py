...
for izhodisce, x1, y1 in kraji:
    if izhodisce == kraj1:
        print(kraj1, "je na koordinatah", x1, y1)
        break

for izhodisce, x2, y2 in kraji:
    if izhodisce == kraj2:
        print(kraj2, "je na koordinatah", x2, y2)
        break

najdaljsa = 0
for tarca, x3, y3 in kraji:
    razdalja0 = sqrt(((x3 - x1) ** 2) + ((y3 - y1) ** 2))
    razdalja1 = sqrt(((x3 - x2) ** 2) + ((y3 - y2) ** 2))
    if razdalja0 < domet and razdalja1 < domet and razdalja0 != 0 and razdalja1 != 0:
        print("Kraja", kraj1, "in", kraj2, "lahko skupaj zalivata kraj", tarca)
