...
for x in kraji:
    ime_zacetni, j_zacetni, k_zacetni = x
    if(ime_zacetni=='Ljubljana'):
      print('Kraj ', ime_zacetni, 'je na koordinatah', j_zacetni, k_zacetni)
      break

for y in kraji:
    ime, j, k = y
    razdalja=sqrt(pow(j-j_zacetni,2)+pow(k-k_zacetni,2))
    if(razdalja>najbolj_oddaljen and razdalja<=domet):
        najbolj_oddaljen=razdalja
        ime_no=ime

print('Najbolj oddaljen kraj je', ime_no, 'ki je od kraja', ime_zacetni, 'oddaljen', najbolj_oddaljen)
