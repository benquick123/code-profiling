...
for kraj1,x,y in kraji:
    ...
    if kraj1 == kraj:  #zapis koordinat vpisanega kraja
        x1=x
        y1=y
        break

else:
    print("Kraja ni na seznamu.")
    ne_nadaljuj=False
...
if ne_nadaljuj:
    ...
    for kraj2 ,x2, y2 in kraji:
        ...
        razdalja_med_krajema = sqrt((x2-x1)**2+(y2-y1)**2)
        if(razdalja_med_krajema < domet and razdalja_med_krajema > najvecja_razdalja):
            trenuten_kraj=kraj2
            najvecja_razdalja = razdalja_med_krajema

    print(kraj, " je na koordinatah ",x1, y1)
    print("Iz kraja" , kraj,"lahko zalijemo kraj " , trenuten_kraj ,  "na razdalji " , najvecja_razdalja)
...
for kraj,x,y in kraji:
    ...
    if kraj1 == kraj:  #zapis koordinat 1. kraja
        x1=x
        y1=y

    if kraj2 == kraj: #zapis koordinat 2. kraja
        x2=x
        y2=y

print(kraj1, " in ", kraj2 , " lahko skupaj zalivata naslednje kraje: ")

for kraj3,x,y in kraji:


    if(sqrt(((x1-x)**2) + ((y1-y)**2))) < domet2 and  sqrt(((x2-x)**2) + ((y2-y)**2)) < domet2:
        print(kraj3)
