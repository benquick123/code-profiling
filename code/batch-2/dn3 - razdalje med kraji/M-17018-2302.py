...
print(k0_ime, "je na koordinatah", x0, y0)
razdalja = 0
for k1 in kraji:
    k1_ime, x1, y1 = k1
    razdalja_temp = sqrt((x1 - x0)**2 + (y1 - y0)**2)
    if razdalja_temp > razdalja and razdalja_temp <= domet:
        razdalja = razdalja_temp
print("Iz kraja", k0_ime, "lahko zalijemo kraj", k1_ime, "na razdalji", razdalja)
...
for k2 in kraji:
    k2_ime, x2, y2 = k2
    razdalja0 = sqrt((x2 - x0)**2 + (y2 - y0)**2)
    razdalja1 = sqrt((x2 - x1)**2 + (y2 - y1)**2)
    if razdalja0 <= domet and razdalja1 <= domet:
        print(k2_ime)
