...
#razdalja in ime najbolj oddaljenega kraja
max_razdalja = 0
max_kraj = ""

#osnovni kraj
for kraj1 in kraji:
    ime, sirina, visina = kraj1
    if ime == kraj:
        visina_1 = visina
        sirina_1 = sirina
        print(kraj, "je na koordinatah", visina_1, sirina_1)

#preverjanje kateri kraj lahko špricamo
for kraj2 in kraji:
    ime, sirina_2, visina_2 = kraj2

    if (ime != kraj):

        abs_sirina = abs(sirina_1 - sirina_2)
        abs_visina = abs(visina_1 - visina_2)

        razdalja = sqrt( abs_sirina ** 2 + abs_visina ** 2 )

        if (razdalja > max_razdalja and razdalja <= domet):
            max_razdalja = razdalja
            max_kraj = ime

print("Iz kraja", kraj, "lahko zalivamo kraj", max_kraj, "na razdalji", max_razdalja)
...
#prirejanje vrednosti spremenljivk za oba kraja
for kraj in kraji:
    ime, sirina, visina = kraj
    if ime == kraj1:
        visina_1 = visina
        sirina_1 = sirina
    elif ime == kraj2:
...
for kraj in kraji:
    ...
    if (ime != kraj1 and ime != kraj2):

        #razdalja od prvega kraja
        abs_sirina_1 = abs(sirina_1 - sirina)
        abs_visina_1 = abs(visina_1 - visina)
        razdalja_1 = sqrt(abs_sirina_1 ** 2 + abs_visina_1 ** 2)

        #razdalja od drugega kraja
        abs_sirina_2 = abs(sirina_2 - sirina)
        abs_visina_2 = abs(visina_2 - visina)
        razdalja_2 = sqrt(abs_sirina_2 ** 2 + abs_visina_2 ** 2)

        if (razdalja_1 <= domet and razdalja_2 <= domet):
            skupni_kraji.append(kraj)

#izpis skupnih krajev
for kraj in skupni_kraji:
    ime, sirina, visina = kraj
    print(ime)