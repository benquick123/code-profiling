...
kraj_s_topom = "Ljubljana"
domet = 30

for kraj, x, y in kraji:
    if kraj == kraj_s_topom:
        x0, y0 = x, y
        break
...
for kraj, x, y in kraji:
    ...
    if razdalja <= domet and razdalja > začasna_razdalja:
        začasna_razdalja = razdalja
        ime = kraj

print(kraj_s_topom, "je na koordinatah", x0, y0)
print("Iz kraja", kraj_s_topom, "lahko zalijemo kraj", ime, "na razdalji", začasna_razdalja)
...
for kraj_0, x_0, y_0 in kraji:
    razdalja_0 = sqrt((x0 - x_0) ** 2 + (y0 - y_0) ** 2)
    razdalja_1 = sqrt((x1 - x_0) ** 2 + (y1 - y_0) ** 2)
    if razdalja_0 <= domet and razdalja_1 <= domet:
        print(kraj_0)