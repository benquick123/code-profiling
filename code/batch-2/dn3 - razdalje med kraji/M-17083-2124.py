...
def izpis_razdalj(kraji):
    for kraj in kraji:
        d = sqrt(abs(kraj[1])*abs(kraj[1]) + abs(kraj[2])*abs(kraj[2]))
        print('{} {}'.format(kraj[0], d))

#izpis_razdalj(kraji)

def izpis_najvecje_razdalje(kraji):
    max = 0.0
    ime_kraja = ""
    for kraj in kraji:
        d = sqrt(abs(kraj[1])*abs(kraj[1]) + abs(kraj[2])*abs(kraj[2]))
        if d > max:
            max = d
            ime_kraja = kraj[0]
    print('{} {}'.format(ime_kraja, max))

#izpis_najvecje_razdalje(kraji)
...
for a in kraji:
    if a[0] == kraj:
        x1 = a[1]
        y1 = a[2]
print('{} je na koordinatah {} {}'.format(kraj, x1, y1))

max = 0.0
ime = ""
for a in kraji:
    if a[0] != kraj:
        d = sqrt(pow(a[1] - x1, 2) + pow(a[2] - y1, 2))
        if d <= domet and d > max:
            max = d
            ime = a[0]
print("Iz kraja {} lahko zalijemo kraj {} na razdalji {}".format(kraj, ime, (str)(max)))
print()
...
imena = []
for a in kraji:
    if a[0] != kraj1 and a[0] != kraj2:
        d1 = sqrt(pow(a[1] - x1, 2) + pow(a[2] - y1, 2))
        if d1 <= domet:
            d2 = sqrt(pow(a[1] - x2, 2) + pow(a[2] - y2, 2))
            if d2 <= domet:
                imena.append(a[0])
for ime in imena:
    print(ime)
