...
kraj_1 = input("vnesi ime kraja: ")
kraj_2 = input("vnesi ime kraja 2: ")
domet = eval(input("vnesi domet: "))
...
for kraj, x, y in kraji:
    if kraj == kraj_1:
        ...
        y1 = y
        for kraj, x ,y in kraji:
            razdalja = sqrt((x - x1)*(x - x1) + (y - y1)*(y - y1))
            if razdalja <= domet:


                seznam_1.append(kraj)
...
for kraj, x, y in kraji:
    if kraj == kraj_2:
        ...
        y2 = y
        for kraj, x ,y in kraji:
            razdalja = sqrt((x - x2)*(x - x2) + (y - y2)*(y - y2))
            if razdalja <= domet:

                seznam_2.append(kraj)
...
for element in seznam_1:
    if element in seznam_2:
        seznam_3.append(element)
...
#print(kraj_1, "je na kordinatah", x1, y1)
#print("Iz kraja", kraj_1, "lahko zalijemo kraj",kraj_2, "na razdalji", razdalja_1)
