#Kenan Mušić Domačo nalogo Vodeni top
kraji = [
...
from math import *
Kraj1 = str(input("kraji =" ))
domet = float(input("Prosimo vnesite domet topa"))
for x in kraji :
    ime, duljina, sirina = x
    if Kraj1 == str(ime):
       a = ime
       b = float(duljina)
       c = float(sirina)
       break
print ("Vodeni top je na ",a, "na",b,",", c, "i ima domet ",domet)
blize= 0
for x2 in kraji :
    ime2,duljina2,sirina2 = x2
    d = sqrt(pow(duljina2 - b,2) + pow(sirina2 - c,2))
    if d <= domet and d >= blize:
        blize= d
        dalec = blize
        a1= ime2
        b1= duljina2
        c1= sirina2
print("Iz kraja",a,"lahko zalijemo kraj",a1,"na razdalji",dalec)
