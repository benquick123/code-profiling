...
kraj2 = "Bled"
domet = 30
oddaljenost_zrtve_max = 0

for iskalnik_kraja in kraji:
    ime_napadalca1, x_os_napadalca1, y_os_napadalca1 = iskalnik_kraja

    if kraj1 == ime_napadalca1:
        break

for iskalnik_kraja in kraji:
    ime_napadalca2, x_os_napadalca2, y_os_napadalca2 = iskalnik_kraja

    if kraj2 == ime_napadalca2:
        break

for iskalnik_kraja in kraji:
    ime_zrtve, x_os_zrtve, y_os_zrtve = iskalnik_kraja
    oddaljenost_zrtve1 = sqrt(pow(x_os_zrtve - x_os_napadalca1, 2) + pow(y_os_zrtve - y_os_napadalca1, 2))
    oddaljenost_zrtve2 = sqrt(pow(x_os_zrtve - x_os_napadalca2, 2) + pow(y_os_zrtve - y_os_napadalca2, 2))

    if oddaljenost_zrtve1 < domet and oddaljenost_zrtve2 < domet:
       print(ime_zrtve)