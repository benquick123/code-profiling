...
def sosedov(x, y, mine):
    vsota= 0
    for x_mina, y_mina in mine:
        for i in range(-1, 2):

            for l in range(-1, 2):

                if( (x_mina, y_mina) == (x+i, y+l) and not ( i == 0 and l == 0 ) ):
                    vsota+=1
...
def najvec_sosedov(mine, s, v):
    ...
    for i in range(0,s):
        ...
        for j in range(0,v):
            ...
            trenutnoSosedov = sosedov(i,j,mine)

            if trenutnoSosedov > max_sosedov:

                max_sosedov=trenutnoSosedov
                trenutnaTerka=(i,j)

    return trenutnaTerka
...
def po_sosedih(mine, s, v):
    ...
    for i in range(0,s):
        ...
        for j in range(0,v):
            trenutnoSosedov = sosedov(i, j, mine)
            terka = (i, j)
            slovar[trenutnoSosedov].add(terka)
