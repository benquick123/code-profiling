...
def sosedov(x, y, mine):
    ...
    for i in range(3): 
        for j in range(3):
            if (i,j)!=(1,1):
                polja.add((x-1+i , y-1+j))
    return len(mine & polja)
 
def varna_pot(pot, mine):
    neki = []
    if len(pot) > 1:
        for (x0, y0), (x1, y1) in zip(pot, pot[1:]):
            neki.append(varen_premik(x0, y0, x1, y1, mine))
    else:
        for (x0, y0) in pot:
            neki.append((x0,y0) not in mine)
    return all(neki)

def polje_v_mine(polje):

    s = len(polje.split(" ")[0])
    v = len(list(filter(None, polje.split(" "))))
    for i, vrstica in enumerate(polje.split(" ")):
        print(vrstica)
        print(i, "Y vrstica")
        for x, j in enumerate(vrstica):
            print()
            if j == "X"
    mine = {(x,y) for y, vrstica in enumerate(polje.split(" ")) for x, crka in enumerate(vrstica) if crka == "X"}
    return (mine,s,v)
