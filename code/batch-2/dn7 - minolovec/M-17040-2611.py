...
def sosedov(x, y, mine):
    ...
    for mina in mine:
        a = mina[0]
        b = mina[1]
        if a == x-1 and b == y-1 or a == x+1 and b == y-1 or a == x and b == y-1:
            st_sosedov += 1
        elif a == x-1 and b == y or a == x+1 and b == y:
            st_sosedov += 1
        elif a == x-1 and b == y+1 or a == x+1 and b == y+1 or a == x and b == y+1:
            st_sosedov += 1
...
def najvec_sosedov(mine, s, v):
    ...
    """
    st_sosedov = 0
    koordinati_polja = (0,0)
    for x in range(s):
        for y in range(v):
            if sosedov(x,y,mine) > st_sosedov:
                st_sosedov = sosedov(x,y,mine)
                koordinati_polja = (x,y)
    return tuple(koordinati_polja)
