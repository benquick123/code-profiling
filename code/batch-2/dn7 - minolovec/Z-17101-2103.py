...
def sosedov(x, y, mine):
    ...
    v = 0
    for m in mine:
        if m[0]  <= x+1 and m[1]  <= y+1 and m[0] >= x -1 and m[1] >= y -1:
            v += 1
        if m[0] == x and m[1] == y:
            v -= 1
    return v
...
def najvec_sosedov(mine, s, v):
    ...
    for i in vsa_polja(s,v):
        seznam.append(i)
        for e,y in seznam:
            slovar[i] = sosedov(e,y,mine)
            max_key = max(slovar, key = lambda k: slovar[k])
    print(max_key)
    return max_key
...
def po_sosedih(mine, s, v):
    ...
    for m in vsa_polja(s, v):
        numines = sosedov(m[0], m[1], mine)
        print(numines)
        slovar[numines].update({m})

    print(slovar)
    return slovar
#how to update elements in a set
