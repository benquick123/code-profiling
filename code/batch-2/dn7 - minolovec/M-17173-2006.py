...
def najvec_sosedov(mine, s, v):
    ...
    """
    koordinate = (0, 0)
    najvec = sosedov(0, 0, mine)
    for i in range(0, s):
        for n in range(0, v):
            if sosedov(i, n, mine) > najvec:
                koordinate = (i, n)
                najvec = sosedov(i, n, mine)
    return koordinate
...
def po_sosedih(mine, s, v):
    ...
    sosedi = dict.fromkeys(i for i in range(0, 9))
    for st_min in range(0, 9):
        mnozice_koordinat = set()
        for x, y in vsa_polja(s, v):
            if sosedov(x, y, mine) == st_min:
                mnozice_koordinat.add((x, y))
            sosedi[st_min] = mnozice_koordinat
    return sosedi
