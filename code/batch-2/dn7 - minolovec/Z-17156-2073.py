...
def sosedov(x, y, mine):
    ...
    Returns:
        int: število sosedov
    """
    return len({(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1), (x + 1, y - 1), (x + 1, y + 1), (x - 1, y + 1), (x - 1, y - 1)} & mine)
...
def najvec_sosedov(mine, s, v):
    ...
    """
    return max([(sosedov(a, b, mine), (a, b)) for a, b in vsa_polja(s, v)])[1]
...
def po_sosedih(mine, s, v):
    ...
    return {i: {(a,b) for a, b in vsa_polja(s,v) if sosedov(a, b, mine) == i} for i in range(9)}
...
def dolzina_poti(pot):
    ...
    return sum([abs(e2[0] - e1[0]) if e2[0] != e1[0] else abs(e2[1] - e1[1]) for e1, e2 in zip(pot, pot[1:])])
...
def varen_premik(x0, y0, x1, y1, mine):
    ...
    return not (({(x0, e) for e in range(min(y0, y1), max(y0, y1) + 1) if x0 == x1} | {(i, y0) for i in range(min(x0, x1),max(x0, x1) + 1) if y0 == y1}) & mine)
...
def varna_pot(pot, mine):
    ...
    return all([True if varen_premik(e1[0], e1[1], e2[0], e2[1], mine) else False for e1, e2 in zip(pot, pot[1:])] + [not (set(pot) & mine)])
...
def polje_v_mine(polje):
    ...
    s = set()
    for indeks, element in enumerate(polje.split()):
        for indeks2, crka in enumerate(element):
            if crka == "X":
                s.add((indeks2, indeks))
