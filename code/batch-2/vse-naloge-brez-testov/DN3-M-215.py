...
for kraj, dol, sir in kraji:
    if ime_kraja == kraj:
        print(ime_kraja, "je na koordinatah", dol, sir)

for ime, korx1, korx2 in kraji:
    if ime == ime_kraja:
        x1 = korx1
        x2 = korx2
trenutni_najvecji = 0
trenutni_kraj = ""
for ime_2, kory1, kory2 in kraji:
    y1 = kory1
    y2 = kory2
    razdalja = abs(sqrt(((x1-y1)**2) + ((x2-y2)**2)))
    if razdalja <= domet:
        if razdalja > trenutni_najvecji:
            trenutni_najvecji = razdalja
            trenutni_kraj = ime_2

print("Iz kraja", ime_kraja, "lahko zalijemo kraj", trenutni_kraj, "na razdalji", trenutni_najvecji)
