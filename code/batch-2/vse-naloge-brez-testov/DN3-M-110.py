...
kraj = input("Vnesi kraj: ")
domet = int(input("Vnesi domet: "))
...
if najden:
    for ime, x2, y2 in kraji:
        dolzina = sqrt((x2 - x1) ** 2 + (y2 - y1)**2)
        if(dolzina < domet and dolzina > najDolzina):
            najDolzina = dolzina
            najIme = ime
    print(kraj, "je na koordinatah", x1, x2)
    print("Iz kraja ", kraj, "lahko zalijemo kraj", najIme, "na razdalji", najDolzina)
else:
    print("Kraj ne obstaja.")
...
kraj1 = input("Vnesi prvi kraj: ")          # vnos prvega kraja
kraj2 = input("Vnesi drugi kraj: ")         # vnos drugega kraja
domet = int(input("Vnesi domet: "))         # vnos dometa
...
for ime, x, y in kraji:         # iskanje vnešenih krajev v seznamu kraji[]
    if(ime == kraj1):               # če se trenutno ime ujema s prvim krajem,
        x1 = x                          # shrani koordinate tega kraja
        y1 = y
        stNajdenih += 1
    elif (ime == kraj2):            # če se trenutno ime ujema s drugim krajem,
        x2 = x                          # shrani koordinate tega kraja
        y2 = y
        stNajdenih += 1
    if stNajdenih > 1:              # če imamo koordinate obeh krajev, prekini zanko
        break
else:
    najden = False

if najden:
    for ime, x, y in kraji:         # računanje dolžin med...
        dolzina1 = sqrt((x - x1) ** 2 + (y - y1)**2)    # ... prvim krajem in tarčo, ...
        dolzina2 = sqrt((x - x2) ** 2 + (y - y2) ** 2)  # ... drugim krajim in tarčo
        if(dolzina1 < domet and dolzina2 < domet and kraj1 != ime and kraj2 != ime):  # če sta obe dolžini manji od dometa,
            trenutni = ime                              # potem izpiši trenuten kraj
            print(ime)
    if not trenutni:
        print(kraj1, "in", kraj2, "ne moreta skupaj zaliti nobenega kraja pri dometu", domet)
else:
    print("Eden izmed vnešenih krajev ne obstaja.")
