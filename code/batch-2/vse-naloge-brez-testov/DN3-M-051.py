...
#kraj1 =(input("vpiši kraj"))

kraj1 = "Ljubljana"
domet = 30
oddaljenost1=0
# Na tem mestu dopiši program
for kraj,razdalja1,razdalja2 in kraji:
    if kraj == kraj1:
        kraj1=[kraj,razdalja1,razdalja2]
        print(kraj,"je na koordinatah", razdalja1, razdalja2)
for kraj, razdalja1, razdalja2 in kraji:
    oddaljenost=(((razdalja1-kraj1[1])**2) + (razdalja2-kraj1[2])**2)**0.5
    if oddaljenost <= domet and kraj != kraj1[0]:
        if oddaljenost1 < oddaljenost:
            oddaljenost1= oddaljenost
            kraj2 = kraj
print("Iz kraja", kraj1[0], "lahko zalijemo kraj", kraj2, "na razdalji",oddaljenost1)
...
for kraj,razdalja1,razdalja2 in kraji:
    if kraj == kraj1:
        ...
        kraj2=[kraj,razdalja1,razdalja2]
for kraj,razdalja1,razdalja2 in kraji:
    if (((razdalja1 - kraj1[1]) ** 2) + (razdalja2 - kraj1[2]) ** 2) ** 0.5 <= domet and kraj != kraj1[0]:
        if (((razdalja1 - kraj2[1]) ** 2) + (razdalja2 - kraj2[2]) ** 2) ** 0.5 <= domet and kraj != kraj2[0]:
            print(kraj)
