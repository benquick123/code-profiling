...
for ime, x, y in kraji:
    if ime == kraj:
        ...
        y1 = y
for ime, x, y in kraji:
    razdalja = math.sqrt(math.pow((x1 - x), 2) + math.pow((y1 - y), 2))
    if razdalja <= domet:
        if razdalja > max_razdalja:
            max_razdalja = razdalja
            max_ime = ime
print(kraj, "je na koordinatah", x1, y1, "\nIz kraja", kraj, "lahko zalijemo kraj", max_ime, "na razdalji", max_razdalja)
...
for ime, x, y in kraji:
    if ime == kraj1:
        ...
        y2 = y
for ime, x, y in kraji:
    razdalja1 = math.sqrt(math.pow((x1 - x), 2) + math.pow((y1 - y), 2))
    razdalja2 = math.sqrt(math.pow((x2 - x), 2) + math.pow((y2 - y), 2))
