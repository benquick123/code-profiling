...
for ime1, kordinata1, kordinata2 in kraji:
    if ime1 == kraj1:
        # shranjevanje kordinat prvega kraja:
        ax = kordinata1
        ay = kordinata2
        for ime_1, kordinata_1, kordinata_2 in kraji:
            c1 = sqrt((kordinata_1 - ax) ** 2 + (kordinata_2 - ay) ** 2)
            if c1 <= domet:
                seznam1.append(ime_1)
                name1 = ime_1

        for ime2, kordinata3, kordinata4 in kraji:
            if ime2 == kraj2:
                # shranjevanje kordinat drugega kraja:
                bx = kordinata3
                by = kordinata4
                for ime_2, kordinata_3, kordinata_4 in kraji:
                    c2 = sqrt((kordinata_3 - bx) ** 2 + (kordinata_4 - by) ** 2)
                    if c2 <= domet:
                        seznam2.append(ime_2)
                        if ime_2 in seznam1:
                            print(ime_2)
