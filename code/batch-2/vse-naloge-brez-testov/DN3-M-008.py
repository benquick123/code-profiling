...
kraj = "Ljubljana"  # Dela tudi z Litija, domet 60
domet = 30
...
for j in range(len(kraji)):
    ime, x, y = kraji[j]
    razdalja = sqrt((x - x1) ** 2 + (y - y1) ** 2)
    if razdalja < domet:
        razdalje_Domet.insert(0, razdalja)
        if razdalja == max(razdalje_Domet):
            maxIme = ime
razdalja = max(razdalje_Domet)
print(kraj, "je na koordinatah X:", x1, "Y:", y1)
print("Kraj", maxIme, "je oddaljen", razdalja, "kilometrov")
...
for k in range(len(kraji)):
    ime, x, y = kraji[k]
    razdaljaLjubljana = sqrt((x - x1) ** 2 + (y - y1) ** 2)
    razdaljaBled = sqrt((x - x2) ** 2 + (y - y2) ** 2)

    if razdaljaLjubljana < domet and razdaljaBled < domet:
        print(ime)
