...
naj_ime = ""
for ime, x, y in kraji:
    razdalja= sqrt((xlj - x) ** 2 + (ylj - y) ** 2)
    if razdalja < domet and razdalja > naj_razdalja :
        naj_razdalja = razdalja
        naj_ime = ime
print("Iz kraja", kraj,"lahko zalijemo mesto", naj_ime, "na razdalji" ,naj_razdalja)

krajj = str(input("Vpišite kraj:"))
domet2 = float(input("Vpišite domet:"))
xx = 0
yy = 0
...
for ime, x, y in kraji:
    if ime == krajj:
        xx = x
        yy = y
        print(krajj, "je na koordinatah",x, y)

naj_razdalja = 0
naj_ime = ""
for ime, x, y in kraji:
    razdalja = sqrt((xx - x) ** 2 + (yy - y) ** 2)
    if razdalja < domet2 and razdalja > naj_razdalja:
        naj_razdalja = razdalja
        naj_ime = ime
print("Iz kraja", krajj, "lahko zalijemo kraj", naj_ime, "na razdalji", naj_razdalja)
...
kraj1 = str(input("Vpišite kraj 1:"))
kraj2 = str(input("Vpišite kraj 2:"))
skupen_domet = float(input("Vpišite domet:"))
x1 = 0
y1 = 0
x2 = 0
y2 = 0
...
for ime, x, y in kraji:
    ...
    if razdalja < skupen_domet:
        razdalja1.append(razdalja)
        ime1.append(ime)

ime2 = []
for ime,x,y in kraji:
    razdaljaa = sqrt((x2 - x) ** 2 + (y2 - y) ** 2)
    if razdaljaa < skupen_domet:
        razdalja2.append(razdaljaa)
        ime2.append(ime)

skupni_kraji = set(ime1).intersection(ime2)

print("Zalijeta lahko:", skupni_kraji)
