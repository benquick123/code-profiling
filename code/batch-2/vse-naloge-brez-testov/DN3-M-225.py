...
for začasno_mesto, začasno_x, začasno_y in kraji:
    if začasno_mesto == mesto:
        b = True
        x = začasno_x
        y = začasno_y
        break
...
for začasno_mesto, začasno_x, začasno_y in kraji:
    if sqrt((x-začasno_x)**2 + (y -začasno_y)**2) > m and sqrt((x-začasno_x)**2 + (y -začasno_y)**2) <= domet:
        m = sqrt((x-začasno_x)**2 + (y -začasno_y)**2)
        ime = začasno_mesto

if b == True:
    print (mesto, "je na koordinatah", x, y)
    print ("Iz kraja", mesto, "lahko zalijemo kraj", ime , "na razdalji", m)
else:
    print("Mesto ni bilo najdeno ali domet je majnši od 0")