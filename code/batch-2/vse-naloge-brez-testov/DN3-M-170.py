...
v_seznamu = False
najbolj_oddaljen_razdalja = 0
najbolj_oddaljen_ime = ""
...
if v_seznamu:
    print(kraj, "je na koordinatih", kraj_x, kraj_y)
    for ime, x, y in kraji:
        razdalja = abs(sqrt((x - kraj_x)** 2 + (y - kraj_y) ** 2))
        if razdalja <= domet and razdalja > najbolj_oddaljen_razdalja:
            najbolj_oddaljen_razdalja = razdalja
            najbolj_oddaljen_ime = ime
    print("Iz kraja", kraj, "lahko zalijemo kraj", najbolj_oddaljen_ime, "na razdalji", najbolj_oddaljen_razdalja)

else:
    print("Tega mesta ni v seznamu.")
...
if v_seznamu1 and v_seznamu2:
    for ime, x, y in kraji:
        razdalja1 = abs(sqrt((x - kraj1_x)** 2 + (y - kraj1_y) ** 2))
        razdalja2 = abs(sqrt((x - kraj2_x)** 2 + (y - kraj2_y) ** 2))
        if razdalja1 <= domet and razdalja2 <= domet:
            seznam_mest_v_dometu.append(ime)
    for ime in seznam_mest_v_dometu:
        print(ime)
else:
    print("Teh mest ni na seznamu.")
