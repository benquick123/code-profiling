...
# Most far away city vars
najdaljsa_razdalja = 0
najdaljse_mesto = ""

# Get City one x and y 
for kraji in kraji_list:
    if kraj == kraji[0]:
        kraj_x1 = kraji[1]
        kraj_y1 = kraji[2]
...
for kraji in kraji_list:
    razdalja = math.sqrt( \
              (math.pow((kraji[1] - kraj_x1), 2)) + \
              (math.pow((kraji[2] - kraj_y1), 2)))

    if  razdalja < domet and razdalja > najdaljsa_razdalja and kraj != kraji[0]:
        najdaljsa_razdalja = razdalja
        najdaljse_mesto = kraji[0]

# Print result
print(kraj, "je na kordinatah", kraj_x1, "in", kraj_y1)
print("Iz kraja", kraj, "lahko zalijemo kraj", najdaljse_mesto, "na razdalji", najdaljsa_razdalja)
