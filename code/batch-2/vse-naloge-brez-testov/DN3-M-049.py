...
for mesto, x, y in kraji:
    if mesto == kraj:
        koordinate = str(x) + " " + str(y)
        koorX = x
        koorY = y

for mesto, x2, y2 in kraji:
    racun = math.sqrt((x2 - koorX) ** 2 + (y2 - koorY) ** 2)
    if racun > najRacun and racun < domet:
        najRacun = racun
        najMesto = mesto

print(kraj, "je na koordinatah", koordinate)
print("Iz kraja ", kraj," lahko zalijemo kraj ", najMesto," na razdalji", najRacun)
