...
for mesto, x, y in kraji:
    if mesto == kraj:
        ...
        for mesto, x, y in kraji:
            ...
            while (razdalja <= domet):
                naj_odd = razdalja
                odd_mesto = mesto
                sezn.append(naj_odd)
                sez2.append(odd_mesto)
                skupaj = list(zip(sez2, sezn))
                for ime,razdalja in skupaj:
                    if razdalja > naslednji:
                        naslednji = razdalja
                        pravi = ime
                break
print(kraj, 'je na koordinatah', a, b)
print('Iz kraja', kraj, 'lahko zalijemo kraj', pravi, 'na razdalji', naslednji)
...
for mesto, x, y in kraji:
    ...
    if mesto == kraj2:
        ...
        b2 = y
        for mesto, x, y in kraji:
            razdalja2 = sqrt((x - a2) ** 2 + (y - b2) ** 2)
            if razdalja2 <= domet:
                mes2.append(mesto)

presek = list(set(mes1).intersection(mes2))
for mesto in presek:
    print(mesto)
