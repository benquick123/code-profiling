...
for i in kraji:
        if kraj == i[0]:
            print (kraj, " je na koordinatah ", i[1], " " ,i[2])
            vneseniKraj = i
            for j in kraji:
                a = vneseniKraj[1]-j[1]
                b = vneseniKraj[2]-j[2]
                c = sqrt((a**2)+(b**2))
                if c <= domet and c > najdaljšaRazdalja:
                    najdaljšaRazdalja = c
                    najboljOddaljenKraj = j[0]
            print ("Iz kraja ", kraj, "lahko zalijemo ",najboljOddaljenKraj, "na razdalji ", najdaljšaRazdalja)
