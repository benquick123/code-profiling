...
naloga = int(input('Katero nalogo hočete zagnati (1: obvezna, 2: dodatna): '))
if naloga==1:
    # Obvezna naloga
    kraj = "Ljubljana"
    domet = 30
    ...
    #iz krajev dobim cel kraj (s koordinatami) z zgornjim imenom
    for k in kraji:
        if k[0]==kraj:
            kraj=k
    ...
    max_razdalja = 0
    for k in kraji:
        #preveri če je trenutni kraj v dometu
        if (k[1] - kraj[1])**2 + (k[2] - kraj[2])**2 < domet**2:
            #izračuna razdaljo med obema krajema
            razdalja_od_kraja = sqrt(((kraj[1]-k[1])**2)+((kraj[2]-k[2])**2))
            #poišče največjo izmed razdalj
            if razdalja_od_kraja > max_razdalja:
                max_razdalja=razdalja_od_kraja
                najdlje_kraj=k

    print( kraj[0],'je na koordinatah',kraj[1], kraj[2])
    print('Iz kraja',kraj[0],'lahko zalijemo kraj',najdlje_kraj[0],'na razdalji',max_razdalja)
if naloga==2:
    ...
    kraji_blizu = []
    for k in kraji:
        razdalja_do1 = sqrt((k[1] - kraj1[1]) ** 2 + (k[2] - kraj1[2]) ** 2)
        razdalja_do2 = sqrt((k[1] - kraj2[1]) ** 2 + (k[2] - kraj2[2]) ** 2)
        if  razdalja_do1<domet and razdalja_do2<domet:
            kraji_blizu.append(k)

    for k in kraji_blizu:
        print(k[0])
'''
...
kraj = []
for k in kraji:
    razdalja_do_izhodisca = sqrt((k[1]**2)+(k[2]**2))
    if razdalja_do_izhodisca>max_razdalja:
        max_razdalja=razdalja_do_izhodisca
        kraj=k
