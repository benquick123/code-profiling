...
def obveznaNaloga():
    ...
    if domet == 0:
        print("Če je domet 0, voda ne doseže nobene druge točke, torej je edini logični odgovor da ostane v cevi...")
    else:
        for kraj1, x1, y1 in kraji:

            if mesto == kraj1:
                print (kraj1, "je na koordinatah", x1, y1)

                for kraj2, x2, y2 in kraji:
                    r = sqrt((x1 - x2)** 2 + (y1 - y2)** 2)
                    if domet_max < r and r < abs(domet):
                        domet_max = r
                        domet_max_kraj = kraj2
                try:
                    print ("Iz kraja", kraj1, "lahko zalijemo kraj", domet_max_kraj, "na razdalji", round(domet_max, 2))
                except:
                    print("Iz mesta", kraj1, "in dometom", domet, "ne moremo zaliti nobebenga drugega kraja.")
...
def dodatnaNaloga():
    ...
    print("Izbrana sta kraja", kraj1, "in", kraj2 + ". Skupaj lahko zalivata naslednja mesta:")
    topDomet(kraj1, domet, kraj1_domet)
    topDomet(kraj2, domet, kraj2_domet)
    primerjavaDometov(kraj1_domet, kraj2_domet, skupni_domet, kraj1, kraj2, domet)

def primerjavaDometov(kraj1_domet, kraj2_domet, skupni_domet, kraj1, kraj2, domet):
    for x in kraj1_domet:
        for y in kraj2_domet:
            if x == y:
                skupni_domet.append(x)
                print(x)
    if skupni_domet == []:
        print("Ne obstaja mesto, ki bi ga lahko mesto", kraj1, "in", kraj2, "lahko skupaj zalivali z dometom", domet,".")

def topDomet(kraj, domet, seznam):
    for _kraj, _x, _y in kraji:
        if _kraj == kraj:
            for kraj_, x_, y_ in kraji:
                razdalja = sqrt((_x - x_)**2 + (_y - y_)**2)
                if razdalja < domet:
                    seznam.append(kraj_)

def privzetZagon():
    print("Dobrodošli v programu, kjer se mesta zalivajo z ekstremnim vodnim topom!")
    print("Izberite kateri del domače naloge želite zagnati (vnesite številko):")
    print("1 - Obvezna naloga")
    print("2 - Dodatna naloga")
    x = input()
    ...
    try:
        if int(x) == 1:
            ...
            print("Vnos nepravilen.")
            privzetZagon()
    except:
        print("Vnos nepravilen.")
        privzetZagon()

    restart = input("Želite poizkusiti še drugi del programa (da/drugi vnosi program zaprejo)? ")
    if restart == "da":
        privzetZagon()
    else:
        quit("Program je bil zaprt.")

########################################################################################################################
#KOMENTAR: Pri vseh primerih je bilo predpostavljeno, da so podatki vnešeni pravilno.
#Odstranite # pred imenom funkcije, ki jo želite zagnati:
#Samo ena izmed funkcij spodaj je lahko de-komentirana naekrat!

#Privzet zagon (Uporabnik izbira med Obvezno in Dodatno nalogo:
privzetZagon()
