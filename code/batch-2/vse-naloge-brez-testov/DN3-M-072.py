...
for ime, x, y in kraji:
    if ime == kraj:
        xkraj = x
        ykraj = y
print(kraj,"je na koordinatah", xkraj, ykraj)
for ime, x, y in kraji:
    razdalja = sqrt((xkraj-x)**2+(ykraj-y)**2)
    razlika = domet - razdalja
    if razlika > 0 and razlika < najvecja:
        najvecja = razlika
        a = ime
        b = razdalja
print ("iz kraja", kraj, "lahko zalijemo kraj", a, "na razdalji", b)# Na tem mestu dopiši program
...
for ime, x, y in kraji:
    razdalja1 = sqrt((xkraj1-x)**2+(ykraj1-y)**2)
    razdalja2 = sqrt((xkraj2-x)**2+(ykraj2-y)**2)
