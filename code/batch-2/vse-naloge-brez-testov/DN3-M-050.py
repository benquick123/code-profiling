...
for i,x1,y1 in kraji:
    raz = sqrt(pow((x1-x_kraj),2)+pow((y1-y_kraj),2))


    if(raz<=domet and raz>=razdalja):
        naj = i
        razdalja= raz

print(kraj,"je na koordinatah",x_kraj,y_kraj)
print("Iz kraja",kraj,"lahko zalijemo kraj",naj,"na razdalji",razdalja)
...
kraj1x = 0 #x koordinata kraja1
kraj1y = 0 #y koordinata kraja1

kraj2x = 0 #x koordinata kraja2
kraj2y = 0 #y koordinata kraja2

kraji1=[] #kraji kateri so kraju1 v dometu 30
kraji2=[] #kraji kateri so kraju2 v dometu 30




for K1,x3,y3 in kraji: #pridobimo koordinate kraja1
    if(kraj1 == K1):
        kraj1x=x3
        kraj1y=y3

for K2,x4,y4 in kraji: #v seznam kraji1 dodamo vse kraje ki so od kraja1 oddaljeni 30 ali manj
    raz1=sqrt(pow((x4-kraj1x),2)+pow((y4-kraj1y),2))

    if(raz1<=domet):
        kraji1.append(K2)

for K3, x5,y5 in kraji:
    if(kraj2== K3):
        kraj2x=x5
        kraj2y=y5

for K4, x6,y6 in kraji:
    raz2 = sqrt(pow((x6 - kraj2x), 2) + pow((y6 - kraj2y), 2))
