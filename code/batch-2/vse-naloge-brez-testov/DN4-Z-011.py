...
def zalijemo(ime, domet, kraji):
    ...
    for kraj, X, Y in kraji:
        if kraj == ime:
            d = v_dometu(kraj, domet, kraji)
            for mesto in d:
                n = najbolj_oddaljeni(mesto, d, kraji)
    return n
...
def presek(s1, s2):
    ...
    for elementi1 in s1:
        if elementi1 in s2:
            seznam.append(elementi1)
    for elementi2 in s2:
        if elementi2 in s1 and elementi2 not in seznam:
            seznam.append(elementi2)
    return seznam
...
def skupno_zalivanje(ime1, ime2, domet, kraji):
    ...
    for kraj, X, Y in kraji:
        if kraj == ime1:
            zaliti1 = v_dometu(kraj, domet, kraji)
        elif kraj == ime2:
            zaliti2 = v_dometu(kraj, domet, kraji)
    resitev = presek(zaliti1, zaliti2)
    return resitev
