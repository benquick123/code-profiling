...
for ime,x,y in kraji:
    if ime==kraj:
        x0=x
        y0=y
        print (ime, ' je na kordinatah', x, y)

tarca = "Rogaška Slatina"

for ime1,x1,y1 in kraji:
    if ime1==tarca:
        hipotenuza = sqrt((x1 - x0) ** 2 + (y1 - y0) ** 2)
        if hipotenuza < domet:
            print('Iz kraja ' ,kraj,' lahko zalijemo ',ime1,' na razdalji ',hipotenuza)
        else:
            print('Kraj ',tarca,'ni v dometu vodnega topa,')
