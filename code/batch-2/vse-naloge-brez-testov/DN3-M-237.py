...
for ime, x, y in kraji:
    ...
    y_2 = y_1-y
    ime_holder = ime
    fake_domet=sqrt(pow(x_2,2)+pow(y_2,2))
    if fake_domet <= domet:
        if fake_domet_placeholder<fake_domet:
            fake_domet_placeholder=fake_domet
            fake_ime=ime_holder

print("Iz kraja",kraj , "lahko zalijemo kraj",fake_ime , "na razdalji ",fake_domet_placeholder)
...
for ime, x, y in kraji:
    ...
    if ime == kraj2:
        ...
        y_dodatna_1 = y
for ime, x, y, in kraji:
        x_dodatna2  = x_dodatna-x
        y_dodatna2  = y_dodatna-y
        x_dodatna_2 = x_dodatna_1-x
        y_dodatna_2 = y_dodatna_1-y
        ime_holder_1 = ime
        ime_holder_2 = ime
        fake_domet_1 = sqrt(pow(x_dodatna2,2)+pow(y_dodatna2,2))
        fake_domet_2 = sqrt(pow(x_dodatna_2,2)+pow(y_dodatna_2,2))
        if fake_domet_1 <= domet and fake_domet_2 <= domet:
            if ime_holder_1==ime_holder_2:
                print(ime)
