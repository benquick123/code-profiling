...
kraj = "Ljubljana"
domet = 30
kordinataX_izhodisce,kordinataY_izhodisce=(0.,0.)
razdalja_max=0
mesto_domet=""
razdalja=0.
# Na tem mestu dopiši program
for kraj_temp in kraji:
    imeKraja,kordinataX,kordinataY=kraj_temp
    if imeKraja==kraj:
        kordinataX_izhodisce=kordinataX
        kordinataY_izhodisce=kordinataY
        print(imeKraja,kordinataX_izhodisce,kordinataY_izhodisce)
        break

for kraj_temp in kraji:
    imeKraja,kordinataX,kordinataY=kraj_temp
    razdalja=sqrt(pow(abs(kordinataX_izhodisce-kordinataX),2)+pow(abs(kordinataY_izhodisce-kordinataY),2))
    print(razdalja)
    if razdalja_max<razdalja and razdalja<domet:
      mesto_domet=imeKraja
      razdalja_max=razdalja
print("Iz kraja ",kraj ,"lahko zalijemo kraj", mesto_domet ,"na razdalji",razdalja_max)
