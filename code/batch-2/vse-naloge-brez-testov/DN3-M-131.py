...
for ime_kraja, x, y in kraji:
    if ime_kraja == kraj:
        ...
        for ime_kraja, x, y in kraji:
            razdalaja = sqrt((x - x_zero)**2 + (y - y_zero)**2)
            if razdalaja <= domet:
                if razdalaja > max_razdalja:
                    max_razdalja = razdalaja
                    max_ime_kraja = ime_kraja

print (kraj, "je na koordinatah", x_zero,y_zero)
print("Iz kraja", kraj, "lahko zalijemo", max_ime_kraja, "na razdalji", max_razdalja)
