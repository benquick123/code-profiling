...
d = 0
for ime_kraja, x, y in kraji:
    if sqrt((x - x_topa ) ** 2 + (y - y_topa) ** 2) > d and sqrt((x - x_topa) ** 2 + (y - y_topa) ** 2) < domet:

        d = sqrt((x - x_topa ) ** 2 + (y - y_topa) ** 2)
        najdlje = ime_kraja


print("Iz kraja", kraj, "lahko zalijemo kraj", najdlje , "na razdalji", d,)
...
for ime_kraja_1, x, y in kraji:
    if ime_kraja_1 == kraj1:
        print(ime_kraja_1, "je na koordinatah", x, y)
        x_topa_1 = x
        y_topa_1 = y


for ime_kraja_2, x_2, y_2 in kraji:
    if ime_kraja_2 == kraj2:
        print(ime_kraja_2, "je na koordinatah", x_2, y_2)
        x_topa_2 = x_2
        y_topa_2 = y_2


print("S topi dometa", domet, "km, lahko skupaj lahko zalivata kraje:")

spremenljivka = 1

for ime_kraja_3, x_3, y_3 in kraji:
    if sqrt((x_3 - x_topa_1) ** 2 + (y_3 - y_topa_1) ** 2) < domet and sqrt((x_3 - x_topa_2) ** 2 + (y_3 - y_topa_2) ** 2) < domet:
        spremenljivka *= 0
        if ime_kraja_3 != kraj1 and ime_kraja_3 != kraj2:
            print(ime_kraja_3)

if spremenljivka:
    print("Topova nimata zadostnega dometa.")
