...
kraj = input("vpisite ime kraja")
domet = int(input("vpisite dolzino dometa"))
...
for terka in kraji:
    x_kraja = terka[1]
    y_kraja = terka[2]
    razdalja = sqrt((x_ljubljana-x_kraja)**2+(y_ljubljana-y_kraja)**2)
    if(razdalja>max_dometek and razdalja<=domet and terka[0]!="Ljubljana"):
        max_kraj = terka[0]
        max_dometek = razdalja
print(kraj, "je na koordinatah", x_ljubljana, y_ljubljana)
print("Iz kraja", kraj, "lahko zalijemo kraj", max_kraj, "na razdalji", max_dometek)
# Dodatna naloga (kdor želi)

kraj1 = kraj
kraj2 = input("vpisite ime drugega kraja")

domet1=int(input("vpisite domet1"))
...
for terka in kraji:
    x_kraja = terka[1]
    y_kraja = terka[2]
    razdalja1 = sqrt((x_kraj1-x_kraja)**2+(y_kraj1-y_kraja)**2)
    razdalja2 = sqrt((x_kraj2-x_kraja)**2+(y_kraj2-y_kraja)**2)
    if(terka[0] not in [kraj1,kraj2]):
        if(razdalja1<=domet):
            doseg1.add(terka[0])
        if(razdalja2<=domet):
            doseg2.add(terka[0])

abc = set.intersection(doseg1, doseg2)
for element in abc:
    print(element)
