...
from math import*
kraj = input("V katerem kraju smo ?")
domet = float(input("Kolikšen je domet vodnega topa ?"))
x0 =  float(input("Koordinata x ?"))
y0 = float(input("Koordinata y ?"))
najvecija_razdalja = 0

for ime, x, y in kraji:
    razdalja = sqrt(((x0 - x)**2)+ ((y0 - y)**2))
    if  razdalja > najvecija_razdalja:
        if domet >= razdalja:
            najvecija_razdalja = razdalja
            naselbina = ime
print(kraj,"je na koordinatah", x0, y0,".")
print("Iz kraja", kraj, "lahko zalijemo kraj", naselbina,"na razdalji", najvecija_razdalja)