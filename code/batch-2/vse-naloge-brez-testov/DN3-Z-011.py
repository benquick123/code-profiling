...
dalec = 0
najbolj_oddaljen = ""
for ime, smerX, smerY in kraji:
    if ime == kraj:
        koorX = smerX
        koorY = smerY
        break
for ime, smerX, smerY in kraji:
     razdalja = sqrt(((koorX-smerX) ** 2 + (koorY-smerY) ** 2))
     if razdalja <= domet:
        dolzina = razdalja
        v_dometu.append((ime, dolzina))

for ime, dolzina in v_dometu:
    if dolzina >= dalec :
       dalec = dolzina
       najbolj_oddaljen = ime
print(kraj, "je na koordinatah", koorX, koorY,
"Iz kraja", kraj, "lahko zalijemo kraj", najbolj_oddaljen, "na razdalji", dalec)
...
for ime, smerX, smerY in kraji:
    razdalja1 = sqrt(((koorX1 - smerX) ** 2 + (koorY1 - smerY) ** 2))
    razdalja2 = sqrt(((koorX2 - smerX) ** 2 + (koorY2 - smerY) ** 2))
