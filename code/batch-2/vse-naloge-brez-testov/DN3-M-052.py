...
for i in range(0, len(kraji)):
    if kraji[i][0] == kraj:
        print("Ljubljana je na koordinatah", kraji[i][1], kraji[i][2])
        for j in range(0, len(kraji)):
            xRazdalja = kraji[i][1] - kraji[j][1]
            yRazdalja = kraji[i][2] - kraji[j][2]
            razdalja = sqrt(xRazdalja**2 + yRazdalja**2)
            if razdalja <= domet:
                if razdalja > najRazdalja:
                    najRazdalja = razdalja
                    najOddaljen = j
        print("Iz kraja", kraj,"lahko zalijemo kraj", kraji[najOddaljen][0],"na razdalji", najRazdalja)
        ...
        for j in kraji:
            if j[0] == kraj2:
                for k in kraji:
                    ...
                    xRazdalja2 = j[1] - k[1]
                    yRazdalja2 = j[2] - k[2]
                    razdalja1 = sqrt(xRazdalja1**2 + yRazdalja1**2)
                    razdalja2 = sqrt(xRazdalja2 ** 2 + yRazdalja2 ** 2)
