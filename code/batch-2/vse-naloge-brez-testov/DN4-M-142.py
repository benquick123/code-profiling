...
def v_dometu(ime, domet, kraji):
    vsi_kraji =[]
    for imek, x, y in kraji:
        if razdalja(ime, imek, kraji) <= domet and ime != imek:
            vsi_kraji.append(imek)
    return vsi_kraji
...
def najbolj_oddaljeni(ime, imena, kraji):
    najbolj_oddaljen = ("", 0)
    for imek in imena:
        if razdalja(ime, imek, kraji) > najbolj_oddaljen[1]:
            najbolj_oddaljen = imek,razdalja(ime, imek, kraji)
    return najbolj_oddaljen[0]

def zalijemo(ime, domet, kraji):
    kraji_v_dometu= v_dometu(ime,domet,kraji)
    naj_oddljen=("", 0)
    for imek in kraji_v_dometu:
        if razdalja(ime,imek,kraji)>naj_oddljen[1]:
            naj_oddljen=imek,razdalja(ime,imek,kraji)
    return naj_oddljen[0]
