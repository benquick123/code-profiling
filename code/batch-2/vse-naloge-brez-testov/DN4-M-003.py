...
def v_dometu(ime, domet, kraji):
    tarce = []
    for mesto, x, y in kraji:
        d = razdalja(ime, mesto, kraji)
        if mesto != ime and d <= domet:
            tarce.append(mesto)
    return tarce
...
def zalijemo(ime, domet, kraji):
    tarce = v_dometu(ime, domet, kraji)
    cilj = najbolj_oddaljeni(ime, tarce, kraji)
    return cilj
...
def skupno_zalivanje(ime1, ime2, domet, kraji):
    prve_tarce = v_dometu(ime1, domet, kraji)
    druge_tarce = v_dometu(ime2, domet, kraji)
    skupne_tarce = presek(prve_tarce, druge_tarce)
    return skupne_tarce
