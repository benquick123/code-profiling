...
for place, kor1, kor2 in kraji:
    if place == kraj:
        print(place, 'je na koordinatah', kor1, kor2)
        ...
        for place2, x, y in kraji:
            ...
            if distance <= domet:
                ...
                if far > num:
                    num = far
                    naj = place2
        print('Iz kraja', place, 'lahko zalijemo kraj', naj, 'na razdalji', num)
...
for kr1, cr11, cr12 in kraji:
    if kr1 == kraj1:
        for city1, a, b in kraji:
            razdalje1 = sqrt((cr11 - a) ** 2 + (cr12 - b) ** 2)
            if razdalje1 <= domet:
                far1 = city1
                for kr2, cr21, cr22 in kraji:
                    if kr2 == kraj2:
                        for city2, aa, bb in kraji:
                            razdalje2 = sqrt((cr21 - aa) ** 2 + (cr22 - bb) ** 2)
                            if razdalje2 <= domet:
                                far2 = city2
                                if far1 == far2:
                                    print(far1)