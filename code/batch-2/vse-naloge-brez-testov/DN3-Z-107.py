...
razdalja = 0
for ime, x,y in kraji:
    razdalja = abs(sqrt(((y-y1)**2)+((x-x1)**2)))
    if razdalja < domet and razdalja > najvecja_razdalja:
        najvecja_razdalja = razdalja
        najvecji = ime
print(kraj,"je na koordinatah",x1, y1)
print("Iz kraja",kraj,"lahko zalijemo kraj",najvecji,"na razdalji",najvecja_razdalja)
...
druznozal_kraj = kraj
for ime, x,y in kraji:
    razdalja1 = abs(sqrt(((y-y1)**2)+((x-x1)**2)))
    razdalja2 = abs(sqrt(((y-y2)**2)+((x-x2)**2)))
    if razdalja1 < domet and razdalja2 < domet:
        druznozal_kraj = ime
        print(druznozal_kraj)
