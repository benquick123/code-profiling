...
for i in kraji:
    if(sqrt(pow(x-(i[1]),2) + pow(y-(i[2]),2))> distanca and sqrt(pow(x-(i[1]),2) + pow(y-(i[2]),2)) <= domet) :
        distanca = sqrt(pow(x-(i[1]),2) + pow(y-(i[2]),2))
        kraj1 = i[0]

print(kraj , "je na kodrdinatah" , str( x) , str( y))

print("Iz kraja" , kraj , "lahko zalijemo kraj" ,kraj1 , "na razdalji" ,distanca)
...
for i in kraji:
    if(sqrt(pow(x1 - (i[1]), 2) + pow(y1 - (i[2]), 2)) <= domet):
        s_kraji1.append(i[0])
    if (sqrt(pow(x2 - (i[1]), 2) + pow(y2 - (i[2]), 2)) <= domet):
        s_kraji2.append(i[0])
