...
def naj_oddaljen_kraj_v_km(kraji,kraj,domet):
    for x,y,z in kraji:
        if x == kraj:
            y1 = y
            z1 = z
    ...
    for x,y,z in kraji:
        razdalja = math.sqrt((y1 - y)** 2 + (z1 - z)**2)
        if razdalja <= domet:
            bliznji.append((x,razdalja))
    najvec = 0
    kateri = None
    for x,y in bliznji:
        if y > najvec:
            najvec  = y
            kateri = x
    return "Iz kraja {kraj} lahko zalijemo kraj {kateri} na razdalji {najvec}".format(kraj = kraj,kateri = kateri, najvec = najvec)

print(naj_oddaljen_kraj_v_km(kraji,kraj,domet))
...
oba_zalivata = []
for x,y,z in kraji:
    razdalja1 = math.sqrt((y1 - y)** 2 + (z1 - z)**2)
    razdalja2 = math.sqrt((y2 -y)**2 + (z2 - z)**2)

    if razdalja1 <= domet and razdalja2 <= domet:
        oba_zalivata.append(x)


for skupni in oba_zalivata:
        print(skupni)
