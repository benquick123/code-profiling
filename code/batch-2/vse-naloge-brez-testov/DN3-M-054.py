...
if not dodatna_naloga:
    ...
    for k in kraji:
        ...
        if max_distance <= distance <= domet:
            max_distance = distance
            max_name = n
        if max_name == kraj:
            print(kraj, 'je na koordinatah', x1, y1)
            print('Iz kraja', kraj, 'lahko zalijemo kraj', kraj, 'na razdalji', max_distance)
            break
    else:
        print(kraj,'je na koordinatah', x1, y1)
        print('Iz kraja', kraj, 'lahko zalijemo kraj', max_name, 'na razdalji', max_distance)
else:
    find1 = True
    find2 = True

    while find1 or find2:
        for k in kraji:
            n, x, y = k
            if find1 and n == kraj:
                x1 = x
    ...
    for k in kraji:
        n, x, y = k
        distance1 = sqrt((x - x1)**2 + (y - y1)**2)
        distance2 = sqrt((x - x2)**2 + (y - y2)**2)
