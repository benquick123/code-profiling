...
def v_dometu(ime, domet, kraji):
    ...
    for ime1, x1, y1 in kraji:
        raz = razdalja(ime1, ime, kraji)
        if ime != ime1 and raz <= domet:
            sez.append(ime1)
    return sez
...
def zalijemo(ime, domet, kraji):
    sezz = v_dometu(ime,domet,kraji)
    naj_odd = najbolj_oddaljeni(ime, sezz,kraji)
    return naj_odd

def presek(s1, s2):
    sez = []
    for ime in s1:
        if ime in s1 and ime in s2:
            sez.append(ime)
    return sez
