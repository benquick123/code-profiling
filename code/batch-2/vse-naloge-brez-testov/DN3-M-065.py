...
def najboljOddaljenKraj(kraj, domet):
    retKraj = (kraj[0], 0)

    zacetniKraj = [x for x in kraji if x[0] == kraj][0]
    #filtriraniKraji = [x for x in kraji if x[0] != kraj][0]

    for kraj in kraji:#filtriraniKraji:
        razdalja = sqrt((zacetniKraj[1] - kraj[1])**2 + (zacetniKraj[2] - kraj[2])**2)

        if zacetniKraj[0] != retKraj[0] and retKraj[1] < razdalja <= domet:
            retKraj = (kraj[0], razdalja)

    return retKraj


def krajiVDometu(kraj, domet):
    retKraji = []
    ...
    for kraj in kraji:
        ...
        if zacetniKraj[0] != kraj[0] and razdalja <= domet:
            retKraji.append(kraj[0])
...
def main():
    ...
    resitev = najboljOddaljenKraj(kraj, domet)
    print("Iz kraja %s lahko zalijemo kraj %s na razdalji %s" % (kraj, resitev[0], resitev[1]))

    kraj1 = "Ljubljana"
    kraj2 = "Bled"
    domet = 30

    resitev1 = krajiVDometu(kraj1, domet)
    resitev2 = krajiVDometu(kraj2, domet)

    for x in [x for x in resitev1 if x in resitev2]:
        print(x)
