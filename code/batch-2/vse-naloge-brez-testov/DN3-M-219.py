...
for kraj, x, y in kraji:
    if kraj == vnos_kraj:
        vnos_kraj_x = x
        vnos_kraj_y = y
        print("{} je na koordinatah {} {}".format(kraj, x, y))

    if vnos_kraj_y != 0:
        for kraj, x, y in kraji:
            razd1 = sqrt(pow(x - vnos_kraj_x, 2) + pow(y - vnos_kraj_y, 2))

            if razd1 > razd and razd1 <= domet:
                razd = razd1
                krajdom = kraj

print("Iz kraja {} lahko zalijemo kraj {} na razdalji {}".format(vnos_kraj, krajdom, razd))
