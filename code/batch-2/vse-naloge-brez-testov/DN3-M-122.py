...
max = 0
for ime, x, y in kraji:
    razdalja = sqrt((x - koordinate[0])**2 + (y - koordinate[1])**2)
    if max < razdalja <= domet:
        max = razdalja
        podatek = ime

print("Iz kraja", kraj, "lahko zalijemo kraj", podatek, "na razdalji", max)
...
for ime, x, y in kraji:
    razdalja1 = sqrt((x - koordinate1[0]) ** 2 + (y - koordinate1[1]) ** 2)
    razdalja2 = sqrt((x - koordinate2[0]) ** 2 + (y - koordinate2[1]) ** 2)
