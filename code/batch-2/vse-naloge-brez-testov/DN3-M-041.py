...
for ime1, x1, y1 in kraji:
    ...
    for ime2, x2, y2 in kraji:
        if ime1 == kraj:
            v_dometu = sqrt((x2-x1) ** 2 + (y2 - y1) ** 2)
            if v_dometu < domet:
                if v_dometu > max_v_dometu:
                    max_v_dometu = v_dometu
                    max_kraj = ime2
                    zacetni_kraj = ime1
print("Iz kraja", zacetni_kraj, "lahko zalijemo kraj", max_kraj, "na razdalji", max_v_dometu)
