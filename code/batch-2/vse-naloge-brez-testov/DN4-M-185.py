...
def v_dometu(ime, domet, kraji):
    ...
    for im,x,y in kraji:
        ...
        if razdalja <= domet and im!= ime_t:
            xs.append(im)

    return xs
print(v_dometu("Ljubljana", 30, kraji))
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    for kraj in imena:
        ...
        for ime_t,x,y in kraji:
            if kraj == ime_t:
                break
        razdalja = sqrt((x0 - x) ** 2 + (y0 - y) ** 2)


        if razdalja > trenutna:
            trenutna = razdalja
            naj_kraj = kraj
...
def zalijemo(ime, domet, kraji):
    ...
    for im, x, y in kraji:
        razdalja = sqrt((x0 - x) ** 2 + (y0 - y) ** 2)
        if razdalja <= domet and im != ime_t and razdalja > trenutna:
            trenutna = razdalja
            naj_kraj = im
...
def skupno_zalivanje(ime1, ime2, domet, kraji):
    ...
    for ime_x, x, y in kraji:
        razdalja1 = sqrt((x0 - x) ** 2 + (y0 - y) ** 2)
        razdalja2 = sqrt((x1 - x) ** 2 + (y1 - y) ** 2)
