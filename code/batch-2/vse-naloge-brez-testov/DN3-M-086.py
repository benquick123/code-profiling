...
kraj = "Ljubljana"
domet = 30
for ime, x_os, y_os in kraji:
    if ime == kraj:
        x_koordinata = x_os
        y_koordinata = y_os
        print(kraj, "je na koordinatah", x_koordinata, y_koordinata)

razdalja = 0
razdalja_max = 0
for ime, x_os, y_os in kraji:
    razdalja = sqrt((x_koordinata - x_os)**2 + (y_koordinata - y_os)**2)
    if razdalja <= domet and razdalja > razdalja_max:
        razdalja_max = razdalja
        ime_max = ime
print("Iz kraja",  kraj, "lahko zalijemo kraj", ime_max, "na razdalji", razdalja_max, ".")
...
razdalja_max = 0
for ime, x_os, y_os in kraji:
    razdalja1 = sqrt((x_koordinata1 - x_os)** 2 + (y_koordinata1 - y_os)** 2)
    razdalja2 = sqrt((x_koordinata2 - x_os)** 2 + (y_koordinata2- y_os)** 2)
