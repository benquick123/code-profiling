...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    for ime_kraja in imena:
        x2, y2 = lokacija(ime_kraja, kraji)
        dolžina = sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2)
        if dolžina > max_dolžina:
            max_dolžina = dolžina
            kraj = ime_kraja
...
def skupno_zalivanje(ime1, ime2, domet, kraji):
    x1, y1 = lokacija(ime1, kraji)
    x2, y2 = lokacija(ime2, kraji)
    poplavljenci=[]
    for ime, x, y in kraji:
        radius2 = sqrt((x2 - x) ** 2 + (y2 - y) ** 2)
        radius1 = sqrt((x1 - x) ** 2 + (y1 - y) ** 2)
        if radius1 <= domet and radius2 <= domet:
            poplavljenci.append(ime)
    return poplavljenci

print(najbolj_oddaljeni("Lenart", ["Brežice", "Rateče"], kraji))
print("")
print(v_dometu("Ljubljana", 40, kraji))
print("")
print(zalijemo("Ljubljana", 40, kraji))
print("")
print(presek(["banana", "jabolka", "hruška", "avtomobil"], ["avtomobil", "jagoda", "butelka", "jabolka"]))
print("")
print(skupno_zalivanje("Ljubljana", "Kranj", 40, kraji))
