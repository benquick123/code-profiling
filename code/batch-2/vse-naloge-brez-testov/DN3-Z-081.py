...
for podatki in kraji:
    ...
    if ime == kraj:
            ...
            izhodisce_x=kx
            izhodisce_y=ky
print(kraj, "je na koordinatah", izhodisce_x, izhodisce_y)
najbolj_oddaljen=0
...
for podatki in kraji:
    ...
    x=kx-izhodisce_x
    y=ky-izhodisce_y
    razdalja_med_kraji=sqrt((x**2)+(y**2))
    #print(ime, razdalja_med_kraji)

    if razdalja_med_kraji <= domet:
        if razdalja_med_kraji>najbolj_oddaljen:
            najbolj_oddaljen=razdalja_med_kraji
            najbolj_oddaljen_kraj=ime
print("iz kraja", kraj, "lahko zalijemo kraj", najbolj_oddaljen_kraj, "na razdalji", najbolj_oddaljen)
...
for podatki in kraji:
    ...
    x=kx-kraj1x
    y=ky-kraj1y
    razdalja_do_kraja1=sqrt((x**2)+(y**2))
    # kraj2
    x = kx - kraj2x
    y = ky - kraj2y
    razdalja_do_kraja2 = sqrt((x ** 2) + (y ** 2))

    if razdalja_do_kraja1 <=domet and razdalja_do_kraja2<=domet:
        print(ime)
