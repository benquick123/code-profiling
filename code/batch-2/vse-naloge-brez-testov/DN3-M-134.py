...
kraj = str(input("Vpiši kraj:"));
domet = int(input("Vpiši domet topa:"));
najdaljsi = 1;
kraj3 = "Ljubljana"
kordx=0;
kordy=0;
for takraj, x, y in kraji:
	if takraj==kraj:
		kordx=x;
		kordy=y;
		for cilj, novx, novy in kraji:
			dolzina=sqrt(pow(x-novx, 2)+pow(y-novy, 2));
			if dolzina<=domet and dolzina>najdaljsi:
				najdaljsi=dolzina;
				kraj3=cilj;	
print();	
print(kraj,"je na koordinatah",kordx,kordy);
print("Iz kraja",kraj,"lahko zalijemo kraj",kraj3,"na razdalji",najdaljsi);

print();
print("DODATNA!");
print();

kraj1 = str(input("Vpišite prvi kraj:"));
kraj2 = str(input("Vpišite drugi kraj:"));
domet = int(input("Vpišite domet topa:"));
print();
razdalja1=1;
razdalja2=1;
for prvikraj, x, y in kraji:
	if prvikraj==kraj1:
		for drugikraj, kx, ky in kraji:
			if drugikraj==kraj2:
				for ciljni, ciljx, ciljy in kraji:
					razdalja1=sqrt(pow(x-ciljx, 2)+pow(y-ciljy, 2));
					razdalja2=sqrt(pow(kx-ciljx, 2)+pow(ky-ciljy, 2));
