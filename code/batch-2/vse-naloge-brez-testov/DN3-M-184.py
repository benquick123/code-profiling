...
for ime, x, y in kraji:
    dist = sqrt(((x - kraj[1]) ** 2) + ((y - kraj[2]) ** 2))
    if najvecji < dist <= domet:
        najvecji = dist
        najbolj_oddaljen = [ime, dist]

print(kraj[0] + " je na koordinatah " + str(kraj[1]) + " " + str(kraj[2]) + "\nIz kraja", kraj[0], "lahko zalijemo kraj", najbolj_oddaljen[0], "na razdalji", najvecji)
...
for ime, x, y in kraji:
    dist1 = sqrt(((x - kraj1[1]) ** 2) + ((y - kraj1[2]) ** 2))
    dist2 = sqrt(((x - kraj2[1]) ** 2) + ((y - kraj2[2]) ** 2))
    if dist1 <= domet and dist2 <= domet:
        kraji_v_dometu.append(ime)

print("Med krajema", kraj1[0], "in", kraj2[0], "so v dometu kraji:")
for kraj in kraji_v_dometu:
    print(kraj)
