...
print(kraj , "se nahaja na koordinatah", kx, ky)

max = ("", 0)
for ime, k1, k2 in kraji:
    razdalja = sqrt(pow(k1 - kx, 2) + pow(k2 - ky, 2))
    if razdalja < domet:
        if max[1] < razdalja:
            max = ime, razdalja
print("Iz kraja", kraj, "lahko zalijemo kraj", max[0], "na razdalji", max[1])
...
for ime, k1, k2 in kraji:
    razdalja = sqrt(pow(k1 - k1x, 2) + pow(k2 - k1y, 2))
    if razdalja < domet:
        kraji1tab.append(ime)

kraji2tab = []

for ime, k1, k2 in kraji:
    razdalja = sqrt(pow(k1 - k2x, 2) + pow(k2 - k2y, 2))
    if razdalja < domet:
        kraji2tab.append(ime)
