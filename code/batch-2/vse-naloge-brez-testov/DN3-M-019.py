...
#na tem mestu sem dopisal program
#def sp
iksanikraj = ""
maxd = 0
...
for ime, x, y, in kraji:
    tempd = sqrt(pow((x1-x),2)+ pow((y1 - y),2))
    if tempd <= domet and tempd > maxd:
        maxd = tempd
        iksanikraj = ime

print("Iz kraja", kraj, "lahko zalijemo", iksanikraj, "na razdalji", maxd)
...
kraj2 = "Idrija"
domet = 30
print("Dodatna naloga (kdor želi)")
...
for ime, x , y in kraji:
    d1 = sqrt(pow((xx1-x),2)+ pow((yx1 - y),2))
    d2 = sqrt(pow((x2-x),2)+ pow((y2 - y),2))
...
for ime, x, y in kraji:
    ...
    if ime == kraj2:
        x2 = x
        y2 = y
dtemp = sqrt(pow((x2 - x1), 2) + pow((y2 - y1), 2))
print("Razdalja med kraji je", dtemp)
