...
if domet == 0:
    print("Vodni top je pokvarjen. Preveri domet.")
    exit()
...
for kraj_sez, x, y in kraji:
    if kraj == kraj_sez:
        print(kraj_sez, "je na koordinatah", x, y)
        x1 = x
        y1 = y
        break
...
if naj_razdalja == 0 or naj_kraj == "":
    print("Mesto je šparalo pri nakupu vodnega topa, saj je kupilo vodni top s katerim ni mogoče zadeti nobenega mesta!")
else:
    print("Iz kraja", kraj, "lahko zalijemo kraj", naj_kraj, ", ki je na razdalji", naj_razdalja, ".")
...
if domet == 0:
    ...
    exit()
if kraj1 == kraj2:
    print("Napaka, podana kraja sta enaka. Eno mesto nima dovolj velikega proračuna, za nakup dveh topov.")
    exit()
...
ali_zadet_kraj = False

for kraj_sez, x, y in kraji:
    razdalja1 = sqrt((x - x1) ** 2 + (y - y1) ** 2)
    razdalja2 = sqrt((x - x2) ** 2 + (y - y2) ** 2)
    if razdalja1 <= domet and razdalja2 <= domet and kraj_sez != kraj1 and kraj_sez != kraj2:
        print(kraj1, "in", kraj2, "lahko skupaj zalivata", kraj_sez, ".")
        ali_zadet_kraj = True

if ali_zadet_kraj == False:
    print("Obe mesti sta projektu namenili premalo €€€, saj sta kupili vodna topa s prekratkim dometom, da bi skupaj zalivali isto mesto.")
