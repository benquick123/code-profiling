    ...
    for kraj, x, y in kraji:
        if ime == kraj:
            for kraj1, x1, y1 in kraji:
                ...
                if razdalja <= domet and kraj1 != ime:
                    tmp.append(kraj1)
    return tmp
...
def najbolj_oddaljeni(ime, imena, kraji):
    ...
    for kraj, x, y in kraji:
        if ime == kraj:
            for kraj1, x1, y1 in kraji:
                razdalja = sqrt((x1 - x) ** 2 + (y1 - y) ** 2)
                if kraj1 in imena and razdalja > max_razdalja:
                    max_kraj = kraj1
    return max_kraj
...
def zalijemo(ime, domet, kraji):
    ...
    for kraj, x, y in kraji:
        if ime == kraj:
            for kraj1, x1, y1 in kraji:
                razdalja = sqrt((x1 - x) ** 2 + (y1 - y) ** 2)
                if max_razdalja < razdalja <= domet:
    ...
    for kraj1, x1, y1 in kraji:
        if ime1 == kraj1:
            for kraj2, x2, y2 in kraji:
                if ime2 == kraj2:
                    for tmpkraj, tmpx, tmpy in kraji:
                        razdalja1 = sqrt((tmpx - x1) ** 2 + (tmpy - y1) ** 2)
                        razdalja2 = sqrt((tmpx - x2) ** 2 + (tmpy - y2) ** 2)
                        if razdalja1 <= domet and razdalja2 <= domet and tmpkraj != ime1 and tmpkraj != ime2:
                            tmp.append(tmpkraj)
    return tmp
