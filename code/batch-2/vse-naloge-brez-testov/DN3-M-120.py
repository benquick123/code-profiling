...
for ime_prvega_kraja, X1, Y1 in kraji:
    if kraj == ime_prvega_kraja:
        print(kraj,"je na koordinatah",X1,Y1)
        break

for ime_drugega_kraja, X2, Y2 in kraji:
    razdalja = sqrt((X2-X1)**2 + (Y2-Y1)**2)
    if razdalja < domet and razdalja > najdaljsa_razdalja:
        najdaljsa_razdalja = razdalja
        ime_kraja = ime_drugega_kraja

print ("Iz kraja",kraj,"lahko zalijemo kraj",ime_kraja,"na razdalji",najdaljsa_razdalja)
...
for ime_prvega_kraja, X1, Y1 in kraji:
    if kraj1 == ime_prvega_kraja:
        break

for ime_drugega_kraja, X2, Y2 in kraji:
    if kraj2 == ime_drugega_kraja:
        break

for ime_kraja, X3, Y3 in kraji:
    razdalja1 = sqrt((X3 - X1) ** 2 + (Y3 - Y1) ** 2)
    razdalja2 = sqrt((X3 - X2) ** 2 + (Y3 - Y2) ** 2)
